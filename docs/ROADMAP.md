...
- **Gallring = a, slå på nu. ✅ Gjort samma dag (A6).** pg_cron aktiverat av Mikael. Torrkörningen mot prod fällde originalmigrationen innan den kördes: jobb 4 anropade `execute_account_deletion_immediate(uuid)` som inte finns (funktionen är argumentlös och läser `auth.uid()`, NULL under cron), jobb 2 raderade `auth.users` rakt av trots nio FK:er utan CASCADE, och `COMMENT ON SCHEMA cron` ger 42501. Ny migration `20260912150000_retention_cron_rattad.sql`: två definer-funktioner (per konto: auditrad → `audit_logs.user_id` nullas → `DELETE auth.users`, fel loggas och nästa konto körs), stängda för anon/authenticated (`lint:grants` grön), fyra jobb i `cron.job`. Verifierat: manuell körning raderade den raderingsbegäran som legat förfallen sedan 6 augusti (testkonto), auditrad finns. Öppet: 18-månadersvarningen hamnar i `email_queue` men inget triggar `send-inactivity-warning` (pg_net är av) — aktuellt först 2027-08. Perioder märkta "bekräftas" i RETENTION-POLICY väntar på beslut.
- **Lagringstider = b (2026-09-12).** Alla "bekräftas"-perioder bekräftade som de stod, utom konsulentjournalen som sätts till **5 år efter avslutat uppdrag** (Mikael angav ingen tid; rekommendationen antogs — ändra i RETENTION-POLICY om annat). ✅ **Byggt samma dag:** nio pg_cron-jobb i `20260912170000_retention_bekraftade.sql`, torrkörda mot prod först (allt 0 utom 20 utgångna pilotinbjudningar). Två tabeller saknar slutdatum (`sta_enrollments`, `consultant_placements`) och "avslutat uppdrag" härleds ur `consultant_consents.revoked_at` eftersom kopplingsraden raderas vid uppsägning — fail closed där datum saknas; en överlämnad deltagare (KS2 b) har aktiv koppling och rörs inte. 13 jobb totalt i `cron.job`, `execute_consultant_journal_retention()` stängd för anon/authenticated. Avstämning med AI-jurist (A2) kan justera nedåt i efterhand.
- **Inloggning för kommuner = a, SSO för personalen (2026-09-12).** Konsulenter och chefer loggar in med kommunens Microsoft-konto (Entra ID) via Supabase Auth Azure-provider; deltagarna behåller e-post/lösenord/Google. BankID för deltagare väntar tills en kommun kräver det. Att bygga (nytt spår **SSO1**): provider i Supabase (kräver app-registrering i kommunens Entra — testas mot en egen Entra-tenant först), knappen "Logga in med kommunens konto" på inloggningen, koppling e-postdomän → organisation vid första inloggningen, och att avslutat Entra-konto stänger åtkomsten (sessionslängd + `organization_members`-status). PUB-avvikelse 4 stryks när det är verifierat.
- **STA upphör som projekt och arkiveras (2026-09-12).** Mikaels ord: "sta som projekt ska upphöra, så det ska arkiveras, återanvänd den del av koden som kan vara användbar för rusta och matcha samt kommunkonsulter (det är de kunder jag kan se just nu)". **Kundbilden är alltså två: Rusta och matcha-leverantörer och kommunernas arbetskonsulenter.** Att göra (nytt spår **STA-ARK**): (1) inventera STA-koden mot R&M- och kommunbehov innan flytt — kandidater: arbetsplatsuppföljningar (`sta_workplace_followups` → placeringsuppföljningarna i AG1), frånvaro/närvaro (`sta_absences` → KM-närvaron), snabbanteckningar (`sta_quick_notes` → konsulentjournalen), självskattningar (`sta_assessments` → R&M-progressionsmätning), dokumentarbetsytan (`StaDocumentWorkspace` → R&M-rapportering till AF); (2) flytta `pages/sta/`, `services/staApi.ts`, `staAiApi.ts`, `hooks/useSta.ts`, `FocusStaWizard`, STA-edge-funktionerna och `e2e/sta.spec.ts` till `archive/2026-09-sta/` med README; (3) `MODULES.STA`-flaggan och `VITE_STA_ENABLED` bort; (4) STA-tabellerna: gallring 2 år efter avslutad inskrivning enligt beslutet ovan, ingen radering av schemat nu; (5) CLAUDE.md, DESIGN.md och `docs/sta-*` märks som arkiverade. ✅ **Klart 2026-09-12 (STA-ARK):** 63 filer flyttade med `git mv` till `archive/2026-09-sta/` (~24 000 rader inkl. sex `docs/sta-*`), återbrukskarta i README (uppföljningar → AG1, frånvaro → KM-närvaro, snabbanteckningar → journal, självskattningar → R&M-progression, dokumentarbetsyta → AF-rapportering), flaggan `MODULES.STA`/`VITE_STA_ENABLED` borta, nåbarhet −10 260 rader (BEHÅLL 30 filer → 4), `typecheck:ceiling` 352 → 340. Levande kod utbruten först: `services/konsulentKopplingApi.ts` (KS3-dialogen + Min konsulent, 5 nya tester). `e2e/sta.spec.ts` kvar som 30-raders regressionsvakt (ci.yml räknar upp filen). ✅ **STA-resten klar 2026-09-12:** `_prompts/sta.js` (3 promptar), validatorn, exemptlistan, `generateDoaSummary` + Sta/Doa-scheman, 16 locale-nycklar och projektväljarens "Steg till arbete" borttagna; sex testfiler omskrivna utan sänkta assertions, 245/245 gröna. PROMPTS = 17 (CLAUDE.md sa 20). Kvar med flit: `steg_till_arbete` som **värde** i `profiles.program` (20 rader i prod, CHECK orörd) och i inbjudningars/samtyckens `program` — data, inte kod; de 20 ser "Inget projekt" i väljaren tills de väljer om.
...
- [x] **PG2** ✅ 2026-09-12 — embed mot `consultant_dashboard_participants` kan inte disambigueras (fyra relationer, hint hjälper inte; 300/PGRST201 reproducerat i prod). Nu två steg: mål med `select('*')`, namn ur vyn med `.in('user_id', …)`; bevisat 200/200 mot prod. Grind i `consultantInsights.test.ts` mutationsbevisad. ✅ InsightsPanel skiljer nu fast (PostgREST/schema/4xx: ingen "försök igen", ingen knapp) från tillfälligt (`felklass.ts`, 3 tester). *(Var:)* Rapporter → AI-insikter felar varje gång: `PGRST201` (tvetydig relation) i embed:et `participant:consultant_dashboard_participants!inner(...)` i `consultantInsights.ts:189`; texten säger "försök igen om en stund" om ett fast fel. Fix: välj relation explicit eller läs namnen separat · S
- [x] **PG3** ✅ **Körd 2026-09-12 22:30 (ja 1–4) och verifierad:** som demo@jobin.se ger vyn nu Anna=true/72, Omar=true/58, Lisa=true/81, Erik/Fatima=false — konsulenter ser CV-status och ATS-poäng. *(Var:)* **större än demot:** vyn är `security_invoker` och `cvs` har bara "own CV"-policy, så **ingen konsulent i prod ser CV-status/ATS-poäng** (mätt: has_cv=false för alla fem som demo@jobin.se, `cvs` → []). Migration `20260912210000_pg3_cvs_konsulent_lasratt.sql` (SELECT via `har_aktiv_relation`) bevisad i rollback 0 → 3 av 5 och nu körd. Kortet "Kräver uppmärksamhet" räknar nu samma mängd som listan, med skälen i undertexten (test). Seedningen var rätt. *(Var:)* Demots Översikt: "Kräver uppmärksamhet **0**" bredvid en lista med fem "CV saknas" trots tre seedade CV:n och CV-poäng i deltagarlistan — nyckeltalen motsäger sig i det första en köpare ser. Undersök `has_cv`-källan i `consultant_dashboard_participants` vs korten · S
- [x] **PG4** ✅ 2026-09-12 — premissrättelse: TopBar hade redan språkvalet men renderas inte på mobil; `MobileTopBar` (i `Layout.tsx`) har nu samma `LanguageSwitcher`, och Inställningar → Tillgänglighet har Svenska/Lätt svenska/English som eget val (`SprakVal.tsx`). Vakt `Layout.mobilsprak.test.tsx`, mutationsbevisad. Kvar: lätt svenska sparas per enhet (localStorage), inte på kontot. *(Var:)* Ingen språkväxling på mobil — "Välj språk" finns bara i desktop-toppnaven; mobilhuvud, Min profil-dialog och Inställningar saknar den. Portalens engelska läsare är nyanländ och på mobil · S
- [x] **PG5** ✅ byggt 2026-09-12 (F1): "Jag kan inte komma" på kommande pass (orsak + rad till konsulenten), notis till konsulenten via definer-trigger, "Anmäld frånvaro" i konsulentens PassRad; migration `20260913002000_f1_franvaroanmalan.sql` (tre nullable kolumner, guarden utvidgad, notis-trigger) bevisad i rollback (`e2e/f1-franvaro-prov.sql`) och **körd 2026-09-12 (ja 1–4). Verifierad i prod** med demodeltagaren: anmälan på morgondagens pass → 200 `absence_reason=sick`, demokonsulentens notiser 0 → 1 (`aktivitet_franvaro`, "Anmäld frånvaro"), ångra → 200 null, titelbyte som deltagare → 403. Ångra ger ingen ny notis. *(Var:)* Aktivitetskravet utan väg för deltagaren att anmäla frånvaro: "Frånvaro" på passet utan orsak, utan knapp; sjukanmälan bara via Min konsulent. Oanmäld frånvaro kan påverka försörjningsstödet. Se förslag F1 · M
...
- [x] **PG7** ✅ 2026-09-12 — saldoraden förklarar konsulentens mål och lagens ram, inplanerat/återstår ("det gör din konsulent"), jobbsökstimmarna utöver, länk till guiden; bara tal ur planen och passen (`weekly_hours_target`, `veckosaldo()`). *(Var:)* "Du har 15 av 30 timmar" i Min vecka är oförklarat (vad räknas, vad är kravet, vad händer) — länka guiden och räkna jobbsökartimmar ur sparade jobb/ansökningar (`pages/MinVecka.tsx`) · S
...
- [x] **PG15** ✅ 2026-09-12 — loggen bar aldrig "vad" (16 rader, alla `participant`). `laslogg` tar nu avsnitt (`participant.<avsnitt>`), `ParticipantDetailPage` loggar per aktiv flik, och kortet i Min konsulent visar "din journal"/"dina mål"/…; äldre rader = "din sida". *(Var:)* Läsloggen säger "dina uppgifter" — inte vad som öppnades (journal, CV, sparade jobb) · S

**Viktigt — konsulenten (Karin)**
- [x] **PG16** ✅ 2026-09-12 (F14) — Tidslinjen byggd av det som finns: journal, mål (satt/uppnått), möten, platser, pass med utfall — `tidslinjeApi.ts` + `Tidslinje.tsx`, RLS avgör, tomt = invit, fallen källa nämns. Läsloggen ingår inte (deltagarens egen vy). *(Var:)* Tidslinje-sektionen säger bara "Aktivitetshistorik kommer" — bygg av läslogg + närvaro + journal i tidsordning (F14) eller ta bort ur sektionsraden · S
- [x] **PG17** ✅ 2026-09-12 — mål = titel + deadline (obligatorisk), SMART som utfällning; "Ta bort mål" i menyn med bekräftelse via ConfirmDialog. Testat. *(Var:)* Nytt mål kräver sju fält; ett skapat mål gick inte att ta bort i UI:t (raderades via SQL) · S
- [x] **PG18** ✅ 2026-09-12 — premissrättelse: katalogen var tom för ALLA organisationer (0 rader), inte bara Testkommun; "Jobbsökarverkstad" är en schemamall. Seedad ur schemamallarna för Testkommun (1) och Demokommun (4), migration `20260913000000_pg18_katalog_seed_testorgs.sql` körd. Premissen "reset raderar katalogen" föll 2026-09-13: posterna ägs av demokonsulenten och överlever reset (4 före/4 efter, mätt). Kvar: `seed_demo_org()` bygger inte katalogen från noll; riktiga kommuner behöver "Hämta ur schemamall". *(Var:)* Aktivitetskatalogen i Resurser är tom för Testkommun trots att planen "Jobbsökarverkstad 15 h" finns som schemamall — katalog per org saknar de aktiviteter planen använder · S
- [x] **PG19** ✅ **Körd 2026-09-12 (ja 1–4) och verifierad i prod:** km-konsulent (chef) slår av → 1 rad `ai_enabled=false`, byter namn → 42501 "Chef får bara ändra…", slår på → 1 rad; utomstående konsulent → 0 rader. *(Var:)* UI byggt (`AiBrytare` i Din organisation, bara chef/admin, bekräftelse med sann beskrivning ur `checkOrgAiEnabled`, `orgApi.setOrgAiEnabled` kastar vid 0 rader); migration `20260913001000_pg19_org_ai_brytare_chef.sql` (UPDATE-policy + guard-trigger som bara släpper `ai_enabled`) bevisad i rollback (`e2e/pg19-ai-brytare-prov.sql`) och nu körd. *(Var:)* Chefen har ingen UI för organisationens AI-brytare (kräver SQL) — bekräftar uppdragets "självservice för AI-brytaren" · S
- [x] **PG20** ✅ 2026-09-12 — konsulentens avsnitt först i mobilmenyn, hubbarna hopfällda under "Deltagarvyn"; profil-dialogen säger Konsulent/Admin. Menyn bor i `Layout.tsx`, inte Sidebar. *(Var:)* Mobil: konsulenten får deltagarens hela hubbmeny med "Konsultportal" sist, och profil-dialogen kallar henne "Deltagare" trots `active_role = CONSULTANT` · S
- [x] **PG21** ✅ 2026-09-12 — fliken heter "Journal" med raden "Deltagarens egen dagbok är privat och syns inte här". *(Var:)* Fliken "Dagbok" på deltagardetaljen är konsulentjournalen, medan B2B lovar en dagbok bara deltagaren ser — döp om till "Journal" med en rad om att dagboken är privat · XS
- [x] **PG22** ✅ 2026-09-12 — "Rapportutkast (AI)" göms med förklaring när organisationens eller konsulentens AI är av (`useOrgAiSparr` + `profile.ai_enabled`). *(Var:)* "Rapportutkast (AI)" visas och öppnar dialog fast organisationens AI är av (bannern säger motsatsen) — läs `my_ai_policy`, göm med förklaring · XS
- [x] **PG23** ✅ 2026-09-12 — **premissen föll:** PDF laddas ner (jsPDF.save), bevisat i prod 20 381 byte via Förhandsgranska → Ladda ner; persona-skriptet lyssnade inte på download-eventet. Ingen ändring. *(Var:)* "PDF-rapport" och "Exportera rapport" gav ingen nedladdning i Playwright (Excel gjorde det) — verifiera manuellt: ny flik eller inget? · XS
- [x] **PG24** ✅ 2026-09-13 — en regel och ett ordval (`lib/kontaktText.ts`): "Aldrig kontaktad" när kontakt saknas, "Ej kontaktad på N dagar" annars — Min dag, Kräver uppmärksamhet och deltagarkortet. *(Var:)* "Aldrig kontaktad" (listan) vs "Ej kontaktad på 7+ dagar" (Översikt) om samma person · XS
- [x] **PG25** ✅ 2026-09-13 — premissen höll helt: ingen kod läser `consultant_settings.notifications`; kortet med sex val borttaget (inte omskrivet) tills en leveransväg finns. *(Var:)* Två "kommande"-löften i Inställningar (R&M-sidor; sex notisval som inte skickar) · XS

**Viktigt — köparen (Per)**
- [x] **PG26** ✅ 2026-09-12 (F15) — demodeltagare `anna.exempel@example.com` i prod (`20260912210000_demo_deltagare.sql`): alltid pass i dag + i morgon, 3 ansökningar, AI av; bevisat i prod (Översikt, Min vecka med "Jag är här", Min konsulent). Demobannern för deltagare: `my_ai_policy` fick `is_demo` (`20260913003000`) och `DemoBanner` läser vyn — verifieras i prod efter deploy. *(Var:)* Demot kan inte visa deltagarens sida — konsulentens egen deltagarvy är tom. Förslag F15: demodeltagarkonto med samma reset · M
- [x] **PG27** ✅ 2026-09-12 (F16) — "Så kommer ni igång" i fem steg med tider på kommunsidan, före FAQ; bara sådant som finns i dag. Plus block "Demokonton" med kopiera-knappar och "Prova som konsulent/deltagare" (`/#/login?email=…`, strikt validerad förifyllning) (F18). *(Var:)* B2B saknar "så kommer ni igång" (avtal → chefskonto → kollegor → katalog/mall → första planen, med tid) · S
- [x] **PG28** ✅ 2026-09-12 — startsidans per-konsulent-kort räknar upp det som finns (meddelanden, journal/mål, möten, rapporter + IVO, ATS per deltagare, praktikplatser); "delad kalender" och "jobbmatchning" fanns inte (belägg i koden). *(Var:)* Startsidans per-konsulent-kort räknar upp "Delad kalender" och "Jobbmatchning" som inte kunde beläggas i demot — kontrollera varje punkt mot koden (ärlighetsregeln) · XS
...
- **AG6 (2026-09-13):** raden räknade med att 43 RLS-policyer måste gås igenom för en roll `EMPLOYER`. Byggt som organisationstyp i stället: företagets person är USER på profilnivå och ingen befintlig policy rörs. Mätt samma dag: 20 policyer nämner en profilroll (inte 43), och ingen av dem påverkas.
- **AG10/mockupen (2026-09-13):** "företaget äger sin plats i samma tabell som konsulentens platsflik" höll inte — `consultant_work_placements.participant_id` är NOT NULL, raden är en person på en plats. Företagets platser fick tabellen `employer_places`, placeringen pekar dit med `place_id`.
- **AG7 (2026-09-13):** "utöka `spontaneous_companies`" — raderna är deltagarens privata (`user_id NOT NULL`). Företagets text bor i `employer_profiles`; spontaneous_companies fick bara `company_account_id`.
- **AG5 (2026-09-13):** guarden `guard_share_proposal_after_decision` stoppade varje UPDATE efter beslut utom `withdrawn` — företagets svar och visningsräkningen hade fällts. Omskriven med `samtycke_lika`.
- **Demomigrationerna (2026-09-13):** `.gitignore` hade `*demo*.sql`, så fyra migrationer körda i prod (`demo_org`, `demo_deltagare`, `my_ai_policy_is_demo`, `ag_demo_foretag`) låg oversionerade. Undantag `!supabase/migrations/*demo*.sql` tillagt och filerna committade.
...
- **Röktestet `e2e/mejl-inbjudan-prod-rok.cjs` läste inte miljövariabler**, bara filerna — så den första "spärrkontrollen" av demokontot loggade in som claude-playwright-consultant och skickade ett riktigt mejl (raderat efteråt). Rättat: `process.env` vinner över filerna.
### Rättelser mot förra versionen
...
- **KO4-raden beskrev en startsida som redan ändrats av ON2** — hjälten talade till den arbetssökande;
  kvar var sektionsordningen, VD-kortet, "Populärast" och slut-CTA:ns tilltal.
- **RM4 kunde byggas utan migration** tack vare KM-tabellerna; fysisk/digital är dock en härledning
  (`workplace` eller plats), inte en egen kolumn.
...
| **N4** | Toppraden: fyra ikoner i stället för åtta (sök, notiser, "nästa steg", avatar); språk, tema, skanna och hjälp in i avatarmenyn; hjärtat blir en flik under Resurser. **Nästa steg-knappen** öppnar "Ett bra nästa steg" från alla sidor (återanvänder `nastaStegRegler.ts`). *(Hette "Lykta-knappen" i första versionen — maskoten utgick med stilbeslutet 2026-09-10, se nedan)* | `TopNav.tsx`, ny `NastaStegPopover` | M |
| **N5** | Rådgivarnas foto får 2 px ring i rådgivarens accent; Lugnare läge får en bild (ark 22 cell 3, lyktan) | `RadgivarPanel.tsx`, `LugnarePanel.tsx` | S — kräver ark 22 |

**Stilbeslut för all ny grafik (Mikael 2026-09-10):** "jag gillar inte bolibompa barn stilen
… jag vill ha en modern realistisk grafik" och "tänk snarare EA Sports än Bolibompa". Maskoten
Lykta och den platta vektorstilen utgår ur `docs/BILDPROMPTER-SIDOR.md`, som är omskriven i
fotorealistisk render-stil (dramatiskt ljus, hubbfärgen som kantljus). **Först ett test:**
Översikt i två stilar (A mjuk realism, B EA Sports), fyra prompter, jämförs i drift innan
...
## Spår AG (Arbetsgivare) 2026-08-31 — mötesplatsen, utan kandidatdatabas

> **Beslut Mikael 2026-08-31.** Portalen ska öppnas för företag, så att den blir en mötesplats
> mellan arbetsgivare och deltagare på riktigt — inte via Arbetsförmedlingen. Företag betalar en
> mindre summa för sin närvaro.
>
> **Fyra val gjorda samma dag, och de styr allt nedan:**
> **(1)** Kombination — stöd- och praktikdelen bär betalningsviljan, företagsprofilen ligger bredvid.
> **Ingen kandidatsökning i något läge.**
> **(2)** Inga företagskunder ännu — bygg det minsta som går att visa i ett säljsamtal.
> **(3)** Deltagaren godkänner varje delning. Inget företag ser någon innan dess.
> **(4)** Praktik och arbetsträning är ingången.

### Premissgranskning före planen — tre mätningar som styr utformningen

**1. En kandidatdatabas går inte att sälja i dag, och det är inte en åsikt.** Mätt i prod
2026-08-31: 93 konton, 90 deltagare, **26 CV totalt varav 17 med kompetenser ifyllda**, 6 personer
har någonsin sparat ett jobb, **5 konton aktiva senaste 30 dagarna**. En arbetsgivare som betalar
för att söka bland kandidater möter alltså 17 profiler. Hen kommer inte tillbaka — och lokala
arbetsgivare är precis den grupp portalen behöver ha förtroende hos längst. Valet att inte bygga
kandidatsökning är därför inte bara juridiskt motiverat, det är det enda som fungerar med dagens
volym.

**2. Portalens egna villkor förbjuder premissen ordagrant — och det är en riskreducerande
åtgärd, inte en formulering.** `Terms.tsx:162`: *"Arbetsgivare och rekryterare får inte använda
portalen för att utvärdera, screena eller jämföra externa kandidater."* Och `:170`: *"AI-output får
inte vidareföras till tredje part för att fatta arbetsrelaterade beslut om någon annan."*
**Den texten är det som håller fyra AI-funktioner utanför högriskklassning** — `AI-ACT-CLASSIFICATION.md:83-84,138,166`
anger uttryckligen villkorsförbudet som mitigering för RIASEC, kompetensgap, `cv-analysis` och
`learning-analyze-gap`. Samma rad bär DPIA:ns R9 (`DPIA-PORTAL.md:140`). **Att riva den utan att
ersätta den river samtidigt riskbedömningen för fyra funktioner.** Terms måste därför skrivas om
*före* varje rad kod — inte som ett sista städsteg.

**3. Byggstenarna finns redan, och tre av dem är rätt formade.** Mätt:
- `spontaneous_companies` (1 rad i prod) bär org.nr, företagsdata från Bolagsverket, status,
  kontaktperson och uppföljningsdatum — men är i dag en **privat anteckningsbok** åt deltagaren,
  filtrerad `.eq('user_id', user.id)`. Ingenting landar hos företaget. Utöka med ett
  företagskonto-id; bygg inte om.
- `SharedProfile` + RPC:n `get_shared_profile` är **exakt rätt mönster**: strikt opt-in per fält
  (`show_contact`, `show_skills`, `show_experience`, `show_education`, `show_documents`,
  `show_summary`), utgångsdatum, visningstak. Ingen ATS-poäng, ingen Hollandkod, ingen hälsodata
  finns ens i funktionen. 0 rader i prod, men det är den färdiga formen för vad ett företag får se.
...
användaren — men kolumnen är en fälla för nästa läsare som antar att den fungerar. Ta bort den
eller implementera den när delningen ändå byggs om.

### Utformningen, och varför just den

Juridiken pekar ut tre möjliga former, och bara en av dem är billig:

| Form | AI-akten | Villkoren | Bedömning |
|---|---|---|---|
| (a) Företaget söker bland profiler | Troligen utanför Annex III om ingen AI rangordnar | **Bryter mot Terms + DPIA R9** | Kräver volym vi inte har |
| (b) Portalen matchar med AI åt företaget | **Annex III 4(a) — högrisk.** Hela kap. III aktiveras, inklusive mänsklig granskning som *inte finns* (`AI-ACT-CLASSIFICATION.md:131-137`) | Bryter mot Terms | Uteslutet |
| **(c) Konsulenten väljer, AI skriver bara text** | **Stannar i lågrisk** — människan är beslutsfattaren | Kräver omskrivning, men i förstärkande riktning | **Vald** |

Mikaels produktval och den juridiska analysen landade oberoende av varandra på (c). Den bygger
dessutom vidare på det som redan finns, och den fungerar med 17 profiler eftersom den handlar om
**en matchning i taget**, inte om volym.

**Vad företaget ser:** sina egna platser, förslag konsulenten skickat, och vilka anställningsstöd
som kan gälla. **Vad företaget aldrig ser:** en kandidatlista, en sökruta, mående, dagbok,
anpassningsbehov, ATS-poäng eller Hollandkod. De fyra sista ska vara **strukturellt uteslutna ur
schemat** som serialiseras mot en företagsvy — en vitlista, inte en samtyckeskryssruta.

### Domänsvar från Mikael 2026-08-31 — det som styr utformningen

> Mikael är själv erfaren arbetskonsulent. Svaren nedan är hans, inte research, och de har ändrat
> designen på tre punkter. **De är värda mer än någon marknadsanalys i det här spåret** — skriv
> inte om dem, och gissa inte vidare på dem.

**1. Vad som faktiskt går fel när en placering spricker.** Ordagrant: *"Främst är det att
arbetsplatsen inte har tid med handledning, så matchningen måste vara bra och anpassningarna
tydliga. När en deltagare väl är på plats brukar det fungera bra, så det är mest att arbetsgivaren
behöver vara positivt inställd till tanken och att praktikanten är en tillgång och att det kan leda
till en framtida anställning som täcker ett behov."*

Tre konsekvenser, alla inbyggda i AG1:
- **Handledningskapacitet är inte en av fyra jämbördiga dimensioner — den är den kritiska.**
  Krocken mellan hög `participant_supervision_need` och låg `workplace_supervision_capacity` ska
  synas **innan** placeringen startar, inte i en uppföljning efteråt.
- **Anpassningarna måste vara tydliga för arbetsgivaren — men får inte avslöja varför.** Det här är
  spårets känsligaste designpunkt: anpassningsbehov och diagnos är art. 9-data och får aldrig nå en
  arbetsgivare (`DPIA-PORTAL.md:51-54,140`), men utan att veta *vad* som ska göras spricker
  placeringen. **Lösningen ligger i Mikaels egen formulering: arbetsgivaren behöver veta vad, inte
  varför.** Schemat skiljer därför praktiska anvisningar ("skriftliga instruktioner", "paus var
  90:e minut", "en arbetsuppgift i taget") från konsulentens interna anteckning om bakgrunden.
  Det första får delas; det andra lämnar aldrig portalen.
- **Arbetsgivarens motivation är en faktor, inte en förutsättning.** Vad har arbetsgivaren för
  framtida behov som gör praktikanten till en tillgång? Det är säljargumentet, och det avgör om
  placeringen leder någonstans.

**2. Periodlängderna skiljer sig per insatstyp**, och det är vägledning — aldrig en spärr:
| Insatstyp | Riktvärde |
|---|---|
| Praktik | Bör inte vara längre än **en månad** |
| Arbetsträning | Upp till **ett år** |
| Arbetsprövning | Cirka **3–6 månader** |
| Subventionerad anställning | Ingen uppgift — sätt ingen gräns |

En praktik på sex månader är ett varningstecken värt att se, men **blockera aldrig** — Mikael vet
när ett undantag är rätt.

**3. Uppföljningstakten är milstolpar, inte en veckoserie.** Ordagrant: *"Varje vecka är för tätt.
Vanligtvis vecka 1, 5, 12 och 24."* Den ursprungliga modellen (lånad från `sta_workplace_followups`)
antog veckovis uppföljning. Kolumnen `week_number` behålls — det är användningen som ändrats: de
fyra tillfällena förbereds från startdatum, systemet visar nästa förfallande, och konsulenten kan
lägga till extra när hon vill.

**4. Lönebidraget — regel och erfarenhet, som inte får blandas ihop.** Mikaels uppgift:
bidraget sätts **individuellt, max 80 % av bruttokostnaden** upp till taket — och landar **i
praktiken på ungefär 30–50 % av lönen** beroende på lönenivå. **Det första är en regel, det andra
en erfarenhet.** Erfarenheten är värdefull när en konsulent ska sätta förväntningar hos en
arbetsgivare, men får aldrig visas som ett förväntat utfall eller räknas fram till ett belopp.
...
> Eftersom det inte finns någon företagskund ännu byggs först det som har värde **oavsett**.
...
- [x] **AG5** ✅ **Körd i prod 2026-09-13** (Mikael: "kör, gå på de föreslagna besluten"). Bevisad i rullad transaktion tillsammans med AG6 innan körning. `AUTH_TAK` 29 → 30 med rad i takloggen. Klient: `services/delningsforslagApi.ts` (konsulent skapar/redigerar/raderar utkast, deltagare svarar via RPC:n, tråd företag↔konsulent) och deltagarens sektion "Förfrågningar från din konsulent" i Min konsulent (`components/participant/Delningsforslag.tsx`, sv+en, 10 tester): kortet visar BARA de fält som är true, raden "Det här ser företaget aldrig", nej-meddelande som företaget aldrig ser, "Sluta dela" via ConfirmDialog. Guarden är omskriven i AG6 (`samtycke_lika`) så företagets svar och visningsräkningen släpps igenom efter beslut — den ursprungliga stoppade allt utom withdrawn. *(Var:)* **Omscopad 2026-09-02 — premissen höll inte.** Raden sa "ny samtyckestyp i
...
- [x] **AG6** ✅ **Byggt 2026-09-13 som ORGANISATIONSTYP, inte roll** (beslut 1 i AG10). Migration `20260913100000_ag6_foretagskonto.sql` körd i prod: `organizations.kind` får `arbetsgivare`, `organization_members.role` får `arbetsgivare` (kind-guard: bara den rollen i företagskonton, aldrig utanför), självbetjäningstriggern får en företagsgren, tabellerna `employer_profiles`/`employer_places`/`employer_checkins`/`employer_messages`, vyerna `employer_proposals` (bara accepted, bara egna, fält per show_*, `cvs.summary` — aldrig ai_summary) och `employer_placements` (bara placeringar med accepterat förslag), INSTEAD OF-triggers för svar/visningsräkning och inbjudan (`employer_invitations`), notistriggers (deltagaren får frågan, konsulent+företag får ja/nej/svar/meddelande/avstämning), medlemskap vid kontoskapande på e-post. **Premissen om 43 policyer föll**: personen är USER på profilnivå, inga befintliga policyer rörs — bara nya, additiva. `20260913120000` ger personal läsrätt på företagskontons namn (konsulentagentens fynd). Klient: `hooks/useForetagskonto.ts`, eget skal i `Layout.tsx` (sju länkar, inga hubbar, demobannern kvar), `/foretag/*` med sju flikar (`pages/foretag/`, `services/foretagApi.ts`, 60 tester), notistyper `foretag_*`. Konsulenten: "Bjud in företaget" från platsen (mejl via `send-invite-email`, ny företagsmall, "Logga in" när kontot finns), "Föreslå deltagaren" (opt-in per fält, DEFAULT false, ingen dokumentruta, AI används inte), `ForslagPanel` + tråd. Demoföretag `Nordfrakt Logistik (demo)` (`20260913110000`, foretag.demo@example.com — lösenord i `.env.test.local`, reset nattligen). DPIA §1.6/R12–R14, Art 30 B22, AI-akt-avsnitt och RETENTION-rad (❌ regel saknas) uppdaterade. **Kvar:** gallringsregel för `employer_*`, rättslig grund för företagskontaktens uppgifter (AG9), publicera demoföretaget på B2B-sidan eller inte (Mikael), AI-formulerad presentationstext (etapp 2). *(Var:)* **Företagskonto och roll `EMPLOYER`.** `profiles.role` är en **text**-kolumn, inte
  ...
  *Mätt i prod 2026-09-02 som startpunkt:* **13 policyer namnger en roll ordagrant** i `qual`/`with_check`
  ...
  ~1 vecka + migration
- [x] **AG7** 🟡 **Delvis 2026-09-13 — premissen föll.** `spontaneous_companies` är deltagarens privata anteckningsbok (`user_id NOT NULL`); företagets egen text kan inte bo där. Byggt: `employer_profiles` (beskrivning, tar emot praktikanter, vad ni brukar behöva, webb, ort, bransch, storlek) som fliken "Om företaget" + kollegor med inbjudan; `spontaneous_companies.company_account_id` (nullbar FK) som krok. **Kvar:** deltagaren ser inte företagsprofilen i Spontanansökan än — kräver en läspolicy för deltagare och ett UI-beslut. *(Var:)* **Företagsprofilen, byggd på det som finns.** Utöka `spontaneous_companies` med
  `company_account_id` i stället för en ny tabell. Företaget fyller i vad de gör, om de tar emot
  praktikanter, och vad de brukar söka. Deltagare kan hitta profilen och spontanansöka — flödet
  finns redan, men i dag landar ingenting hos företaget · ~1 vecka
- [x] **AG8** ✅ **Byggt 2026-09-13.** Konsulenten väljer plats (företagets egna ur `employer_places` förifyller formuläret) och deltagare → förslag → deltagaren får notis och svarar i Min konsulent → först vid ja syns namn + valda fält i företagets "Förslag" (vyn räknar visningar mot `max_views`, döljer efter `expires_at`) → företaget svarar "Vi vill gå vidare"/"Tacka nej" (bara konsulenten ser skälet) → "Pågående" med vecka X av Y, VAD-instruktioner, sjukanmälan, avstämning vecka 12/24, tråd med konsulenten. Bevisat i rullad transaktion mot prod: 0 rader före ja, 1 efter, 0 efter återkallelse; **prod-röktest `e2e/foretag-prod-rok.cjs` 19/19 gröna 2026-09-13 01:50** (företaget: sju flikar, Omars e-post dold, intern anteckning osynlig, stödfliken utan belopp; konsulenten: företagskonto-chip + företagets svar; Anna: "Det du delat"); konsulenten kan inte svara åt företaget; deltagaren ser inga företagsplatser. **Ingen AI-formulering i etapp 1** (textarea med förklaring); när den införs är det text, inte urval (AI-ACT-CLASSIFICATION uppdaterad med villkoret). *(Var:)* **Förslagsflödet — kärnan.** Konsulenten väljer en deltagare för en plats →
  **deltagaren får frågan och kan säga nej** → först vid ja går namn och valda fält till det
  namngivna företaget. Använd `SharedProfile`-mönstret rakt av: opt-in per fält, utgångsdatum,
...
- [x] **AG10** ✅ **Verkställt 2026-09-13** — Mikael: "kör, gå på de föreslagna besluten": (1) organisationstyp, (2) ingen direktkontakt företag→deltagare i etapp 1 (trådar går per förslag till konsulenten), (3) tre företagssamtal före AG9 — **Mikaels egen punkt, kvar**. Se AG5/AG6/AG7/AG8 för utfallet. *(Var:)* 📝 **Designförslag för företagskontot, 2026-09-13** (Mikael: "nu vill jag få in företagen, på ett företagskonto, ge förslag på hur den vyn ska se ut och vilka funktioner den ska innehålla"). Publicerat som artefakt "Företagskontot i Jobin": en **mottagarvy** — Översikt, Våra platser, Förslag, Pågående, Meddelanden (via konsulenten), Stöd och regler, Om företaget — där företaget äger sin plats (samma fält som `consultant_work_placements`), ser ett förslag först när deltagaren godkänt (AG5), fyller i 3/6-månadersavstämningen, och aldrig ser en lista, en poäng eller ett nej. **Rekommendation:** bygg företaget som organisationstyp `arbetsgivare` i `organizations` (ärver inbjudan, medlemskap, AI-brytare, demobanner) i stället för rollen `EMPLOYER` i AG6; inbjudan går från platsens kontaktperson (mejl fungerar sedan DE1). **Tre beslut väntar:** organisation vs roll, direktkontakt företag→deltagare (rek. nej i etapp 1), och tre företagssamtal före AG6 (rek. ja). Mätt 2026-09-13: 0 placeringar, 1 företag i `spontaneous_companies`, 2 organisationer (kommun), 43 policyer att läsa.

### Etapp 2 — betalning, när någon sagt ja

- [ ] **AG9** 🔴 **beslut — prismodell och avtal för företagskunder.** Mätt: portalen tar i dag
  2 990 kr/mån + 290 kr/konsulent (`sv.json:7523,7661`), men **ingenting under `docs/` nämner en
  prismodell alls**, och det finns inga avtalsvillkor för företagskunder. B2B slipper
  konsumenträttens ångerrätt, men det är inte samma sak som att frågan är löst. **Sätt inte ett
  pris förrän tre företag sagt vad de skulle betala för** · beslut

### Det viktigaste rådet i hela spåret

**Ring tre företag innan AG4 påbörjas.** Etapp 0 är byggd för att vara värdefull ändå, så den kan
göras direkt. Men etapp 1 kostar en omskriven Terms, en uppdaterad DPIA, en ny samtyckestyp och en
genomgång av 43 RLS-policyer — det är veckor av arbete som blir fel om det byggs mot en gissning
om vad ett företag vill ha. Frågan att ställa dem är inte *"skulle ni använda det här?"* utan
**"vad gjorde ni senast ni tog emot en praktikant, och vad tog längst tid?"**

### Vad marknadsanalysen säger om chansen

Ingen svensk aktör kombinerar handläggarsystem med en deltagarverktygslåda — men flera äger
arbetsgivarrelationen redan: Teamtailor och Varbi är arbetsgivarens karriärsida, Adway är rent
B2B. Att konkurrera om arbetsgivarens *uppmärksamhet* är en annan marknad än den Jobin vunnit.
...
## Passering 2026-09-08 — 20 poster ur genomgången, åtta agenter, tre premisser som föll

> Översikt: <https://claude.ai/code/artifact/f5c5f787-8169-4dad-beba-7f9a851044bb>.
> Commit `1ca09eea`. Åtta agenter med exklusiva fillistor (ingen fil på två listor,
> inga git-kommandon, inga delade byggen), varje post premissgranskad före ändring.

### Gjort (deployat i `1ca09eea`)

- **ON1/MB1** samtyckesrutan: `{ namn }` på båda `seesList`-anropen; testet förbjuder `{{`
  både i DOM och i `p_consent_text`. Dialogen är `flex-col max-h-[calc(100dvh-2rem)]` med
  innehållet i en rullande yta (`data-ks3="innehall"`) och fel + tre knappar i en fast sidfot
  — inuti `modalRef`, så fokusfällan och DOM-ordningen håller. 12 tester, 4 mutationer fällda.
- **KA1** ett toastsystem: `lib/toast.ts` delegerar till `components/Toast.tsx` (som fick ett
  riktigt `dismissToast` — det var en tom stubb), `<Toaster/>` borta ur `Profile.tsx`,
  `react-hot-toast` avinstallerat. Grinden `toast.grind.test.ts` skalar av kommentarer före
  matchning (den fällde först på sin egen dokumentation). **Kvar:** `ToastContainer` ligger
  `bottom-4 right-4` — det gamla systemet valde top-center för att inte krocka med
  bottennavet; `Toast.tsx` har hårdkodat `bg-white`/`text-stone-900` (mörkt läge).
- **SD2** minnesfallback i `cv-pdf.js`/`upload-image.js` via ny `_utils/rate-limit-fallback.js`
  (tomt RPC-svar räknas som fel — strängare än `ai.js:306`). `api/_utils/rate-limiter.js`
  hade noll importörer och är raderad; `typecheck:api` 0.
- **DE2** reservvärden: `https://www.jobin.se` (307 från apex) med **hash-rutter** —
  jobbevakningsmejlets fyra länkar saknade `#` och pekade på `?tab=alerts`, som aldrig varit
  en rutt (rätt: `/#/job-search/alerts`). Saknad `EMAIL_FROM` med satt `RESEND_API_KEY`
  ger 500 för `send-digest`; `check`/`check-user` körs vidare (portalnotiserna är den enda
  kanal som når fram tills DE1 är löst) och bär `emailConfigError` i svaret.
  `send-invite-email` likadant. Vercel-variablerna kan inte läsas härifrån.
- **DE6** `[functions.health] verify_jwt = false` i `supabase/config.toml`; svaret är
  `{ status, timestamp, checks: { database, storage } }` med bara `ok`/`error` (förut råa
  Postgres-strängar, `version`, `uptime` och en `auth.getSession()` som med service role
  aldrig kunde bli annat än ok). HEAD stöds; db-fel → 503.
- **KA2** `authStore` registrerar `onAuthStateChange` på modulnivå och tömmer React Query
  vid `SIGNED_OUT` och när `session.user.id` byter från ett känt id till ett annat.
  `useAuth` var bara monterad på hubbsidorna; `useAuthInit` nås bara via en död barrel.
  Vakten `registreraAuthLyssnare()` kontrollerar att metoden finns, för tre testfiler mockar
  `supabase.auth` utan den — ta bort vakten när mockarna fått `onAuthStateChange`.
- **ON4** `signUp` returnerar `{ needsConfirmation: true }`; `Register.tsx` visar en
  `role="status"`-panel med fokusflytt. `auth.register` är en **sträng** i sv.json, så
  nycklarna heter `auth.confirmEmail.*`.
- **ON2** hjälten: "Sök jobb i din egen takt – med stöd på vägen", tre bockar utan siffror,
  sekundärknapp → `/guider/`; B2B-raden "Arbetar du med arbetssökande?" med tre `<a href>`
  (R&M, AME, demo). Avslutande CTA-sektionen (`Landing.tsx:~960`) är orörd.
- **BL1** `EmptyState` + `mailto:support@jobin.se` (adressen ur `Terms.tsx:243`) — `EmptyState`
  saknar `href`-action, så länken ligger under den. **Om adressen tar emot post är inte
  verifierat.**
- **AS4** `status: null` + "Välj hur det går på platsen", `role="radiogroup"`, 4 tester.
- **MB3** `sm` → `text-sm` på alla bredder (118 `<Button size="sm">` i 42 filer, 12 dödkod);
  `PDFExportButton` hade egna `sizeClasses` med `text-xs` — rättad. "Säg upp kopplingen" `md`.
- **MB2 — premissen höll delvis, och det som höll var inte felet.** `<main>` hade redan
  80 px + safe-area bottenpadding, och att Spara skärs av vikningen är inget fel (allt under
  skärmkanten ligger under navet på varje rullande sida — att fälla ihop rådgivartipset
  flyttar bara vilket kort som skärs). Den riktiga buggen: fönstret rullar, och
  `scroll-padding-bottom: auto` fick `focus()`/Tab att **inte rulla alls** — tre knappar låg
  kvar under navet efter Tab, på alla sidor. Nu `html { scroll-padding-bottom:
  var(--bottom-nav-h) }` med variabeln satt från navets **uppmätta** höjd (65 px, inte
  hårdkodade 64) via ResizeObserver, 0 när fokusläget döljer navet. Två fynd på vägen:
  `pb-safe` **finns inte i Tailwind 4** (navet har haft noll safe-area) → `pb-[env(…)]`;
  `SamlingarFab` positionerade sig med hårdkodade 68 px → nu ur tokenen. `JobSearch.tsx`
  orörd. Mätskript `e2e/mat-bottennav-hittest.cjs` (tre regler, listar alla fixerade lager).
- **SD3** `delete-account/storageCleanup.ts`: rekursiv, paginerad listning under `<uid>/`
  (CV ligger under `<uid>/cv/…`, `list()` är en nivå), radering före `deleteUser()`, fel
  rapporteras per bucket — aldrig "0 filer" vid fel. Stoppar inte auth-raderingen (profilen
  är redan borta). Testet flyttat till `client/src/test/` — `vitest.config.ts` har
  `include: src/**`, bredvid modulen kördes det aldrig.
- **AS1** `guard_share_proposal_after_decision()` är `BEFORE INSERT OR UPDATE`: INSERT måste
  vara `pending`, varje övergång från `pending` kräver `auth.uid() = OLD.participant_id`
  (service role/NULL stoppas). Ett hål utöver roadmapens: konsulenten kunde radera
  pending-raden och skapa en ny med `accepted` direkt. Prov A–H körda i ett lokalt
  Postgres 18-kluster; med den gamla triggern gick prov C igenom utan rad i
  `consent_history`. Verifierings-SQL ligger sist i migrationsfilen. **Okörd.**
- **AS3** DPIA §1.2/§1.3: `internal_adaptation_notes` på `consultant_work_placements`
  (källa konsulenten, ingen grind). Ställningstagande: 9.2.b/h faller, 9.2.a är hypotesen
  men **inte uppfylld** — samtyckestexten nämner inte hälsa och `share_health_data`
  kontrolleras inte vid skrivning.
- **KG2** — **27 påståenden i 25 guider**, inte 18 (45 artiklar bär procent, 20 är
  CV-exempel, regelsatser eller råd). Enda siffran som behölls: `branscher-brist` 51 %/40 %,
  nu med "Arbetsförmedlingens Arbetsmarknadsutsikterna våren 2026" (verifierat mot PDF:en).
  Skrivet till prod (sv + en) med `scripts/apply-procent-rattelser.cjs` +
  `content/procent-rattelser.json` (torrkörning, backup, driftkontroll, återläsning); prod
  bär **CRLF i `content_en`, LF i svenskan** — skriptet matchar fältets egna radslut.
  `--bara=<slug>` finns för att en redan skriven post matchar 0 gånger nästa körning.
  Grind `guides-pastaenden.test.ts` (ser bara svenskan — `content_en` finns inte i
  snapshoten). Sidofynd rättat samma väg: `lon-och-formaner-guide` sa "minst 4,5 % av lönen
  (lag)" om tjänstepension — ingen lag kräver den; 4,5 % är avtalsnivån i ITP 1/SAF-LO.
- **KG1** kluster 24 + regeln "delad `category_key + subcategory` = kandidatkluster" (55 av
  122 underkategorier delas). De två guiderna **länkar inte** till varandra — premissen höll
  till två tredjedelar. Inlänkar 7 mot 2, ord 557 mot 1 237 → väntar på Search Console.

### Skrivet men EJ kört — väntar på Mikaels ja

Efter varje: `cd client && npm run schema:refresh && npm run grants:refresh` i samma commit.

1. `npx supabase db query --linked -f supabase/migrations/20260902100000_ag5_share_proposals.sql`
   → `select pg_get_triggerdef(oid) from pg_trigger where tgname='trg_esp_guard_after_decision'`
   innehåller `BEFORE INSERT OR UPDATE`; prov C i filen → 42501.
2. `… -f supabase/migrations/20260908100000_as2_placements_relation.sql` → `pg_policies where
   tablename='consultant_placements'` = 1 rad med `EXISTS ( SELECT 1 FROM consultant_participants`
   i **både** qual och with_check. Beslut i samma fil: ingen deltagar-SELECT (raden bär
   `notes` + `salary_range`); historiska placeringar utan relation blir osynliga.
   Kommentarer som blir falska efteråt: `consultantService.ts:685–695`, `SettingsTab.tsx:305–318`.
3. `… -f supabase/migrations/20260908110000_sd1_get_shared_profile_atomic.sql` → `password_hash`
   0 rader i `information_schema.columns`; `has_function_privilege('anon',
   'public.get_shared_profile(text)','EXECUTE')` = true. Samtidighetsprov lokalt: gamla kroppen
   gav `view_count = 2` vid `max_views = 1`, nya gav NULL till den andra. `SharedProfile.tsx`
   oförändrad — svarsformen är identisk. Omnämnanden att städa: `GDPR-ART30-REGISTER.md:376`,
   `security-audit.md:26`.

### Premisser som föll eller ändrades

- **MB2** — "sidan saknar bottenpadding" och "fäll ihop rådgivartipset" höll inte; felet var
  `scroll-padding-bottom` (se ovan).
- **KG2** — 18 var för lågt: 27 i 25.
- **KG1** — de två guiderna länkar inte till varandra.
- **KA1** — 14 anropare, inte 13.
- **SD1** — "verkställ lösenordet eller ta bort kolumnen" var ett falskt val: ingen kod läste
  eller skrev `password_hash`, inget UI erbjöd lösenord. Bara att droppa.
- **DE2** — roadmapens "högljutt fel" rakt av hade stoppat portalnotiserna med. Grinden sitter
  där mejlet skickas, inte före `check`.

### Fynd på vägen, inte åtgärdade

- `unifiedProfileApi.uploadProfileImage` laddar upp till `profile-images` under `profiles/…`
  medan storage-policyn kräver `<uid>/` — kan aldrig lyckas, 0 anropare (dödkod).
- `SamlingarFab` överlappar högerkanten på en fokuserad "Ansök"-knapp (14×45 px) — fokus
  rullar upp till exakt FAB:ens remsa. Frågan är om en fixerad ö i innehållskolumnen ska
  finnas alls.
- `KonsulentSamtyckeFraga` täcker hela vyporten (z=100) och låser rullningen vid varje
  sidladdning på testkontot — första mobilmätningen mätte rutan, inte navet.
- `Layout.tsx` har ett `pb-safe` (no-op i Tailwind 4, harmlöst).
- Tre testfiler mockar `supabase.auth` utan `onAuthStateChange` (vakten i `authStore`).

### Verifiering

Lokalt: `lint:vercel`, `lint:schema`, `lint:grants`, `typecheck:critical`, `typecheck:api` (0),
`typecheck:ceiling`, `lint:ci` (0 fel, 115 av 122 varningar), `lint:design` (52/52),
`lint:links`, `test:coverage` **209 filer / 2 956 tester** (var 2 894), `build`.

**Verifierat i drift (körning `34230599833`):** `Deploy Frontend` ✅, `Deploy Supabase
Migrations` (= edge-funktionerna) ✅, **`Smoke Test` ❌ på steget "API health check"** — steget
kräver `401` från `/functions/v1/health` utan auth som bevis på "deployad och skyddad"
(`deploy.yml:~185–200`), och DE6 gjorde just den funktionen publik. Alla steg före var gröna.
Prod är alltså **levererad**, och röktestet mäter en premiss som inte längre gäller.
**Kräver ja (workflow-ändring):** i `case "$anon"` byt `401)`-raden mot
`200) echo "health: 200 utan auth — publik övervakningsendpoint (DE6). OK" ;;` och låt `401)`
fälla med "verify_jwt har slagits på igen — config.toml [functions.health]"; den skarpa
nyckelkontrollen kan stå kvar. Mätt direkt mot prod efter deployen:
- `curl -o /dev/null -w "%{http_code}" …/functions/v1/health` → **200** (var 401); body
  `{"status":"healthy","timestamp":…,"checks":{"database":"ok","storage":"ok"}}`; HEAD → 200.
- Hjälten (`www.jobin.se`, Playwright): h1 "Sök jobb i din egen takt – med stöd på vägen",
  två länkar till `/for-rusta-och-matcha/`, "Stärk dina deltagare" förekommer inte,
  B2B-raden finns.
- Samtyckesrutan på testkontot, 390×844 och 375×667: inget `{{` i DOM, raden lyder "mål
  som Claude Test skriver om dig", alla tre knapparna inom vyporten (640–804 resp. 463–627),
  innehållet rullar (`scrollHeight` 814/865 mot `clientHeight` 607/430),
  `elementFromPoint` på första knappen träffar knappen.
- `BASE_URL=https://www.jobin.se node e2e/mat-bottennav-hittest.cjs /job-search` (kör med
  `MSYS_NO_PATHCONV=1` i Git Bash, annars blir `/job-search` en Windows-sökväg):
  `scroll-padding-bottom=65px`, "efter fokus/Tab: alla nåbara" på båda vyporterna, **OK**.
  Vid laddning ligger Spara/Ansök fortfarande 83 % under navet (vikningen, inte ett fel).
...
- [x] **ON1** *(✅ 2026-09-08 — `{ namn }` på båda anropen; test förbjuder `{{` i DOM och i beviset)* **Samtyckesrutan om konsulenten visar `{{namn}}` rått och sparar det i
...
- [x] **MB1** *(✅ 2026-09-08 — fast sidfot utanför den rullande ytan (`100dvh`); pixelmätning mot prod efter deploy, se passeringen)* **Samma ruta på mobil: alla tre knapparna under skärmkanten, utan tecken på
  ...
  skärmdump · ~½ dag
- [x] **KA1** *(✅ 2026-09-08 — `lib/toast.ts` är adapter mot `components/Toast.tsx`, `react-hot-toast` avinstallerat, grind `toast.grind.test.ts`; anroparna var **14**, inte 13)* **`notifications.*` ritas bara ut på profilsidan — två toastsystem, ett
  ...
  nåbarhet) · ~1 h
- [x] **AS1** *(✅ 2026-09-08 — triggern grindar `INSERT OR UPDATE`; ett andra hål (radera pending + skapa `accepted`) stängt i samma fil. **Migrationen är fortfarande okörd**)* **AG5-migrationen låter konsulenten godkänna ett delningsförslag åt
...
- [x] **AS2** ✅ **Körd 2026-09-13 (ja 1–4)** — verifierad i prod efter körning (se verif-raden i passets commit). *(Var:)* *(📝 2026-09-08 — migration `20260908100000_as2_placements_relation.sql` skriven och prövad lokalt; **väntar på ja**)* **`consultant_placements` har en enda policy utan relationskoll.** Mätt i
  ...
  ~20 min + migration
- [x] **BL4** ✅ **Körd 2026-09-12 (ja 1–4):** FK-regler mot profiles/auth.users i prod nu 114 CASCADE (konsulentens eget + deltagarägt under deltagaren), 26 SET NULL, 1 NO ACTION kvar med flit (`audit_logs.user_id` — raderingsfunktionerna nullar den först). Snapshots uppdaterade; `typecheck:ceiling` exakt på taket 338 utan nya fel. *(Var:)* *(migration `20260912180000_bl4_fk_set_null.sql` skriven och bevisad i rollback (`e2e/bl4-fk-prov.sql`))*. Mätt: 14 CASCADE + 7 NO ACTION/RESTRICT på konsulent-FK:er. **Värre än raden sa:** NO ACTION-FK:erna (uppföljningar, `assigned_by`, sex `sta_*`) gör att en konsulent med STA-historik inte kan raderas alls — provet föll på `sta_quick_notes_author_id_fkey` — och båda gallringsfunktionerna från i dag hade loggat FAILED varje natt för henne. Migrationen: 20 FK → SET NULL (12 kolumner `DROP NOT NULL`), konsulentens eget cascadar som förr; efter: deltagarens rader kvar med `consultant_id NULL` (KS2 b:s avsikt). Obs efter körning: `schema:refresh` gör `consultant_id` till `string | null` — `PlaceringFormModal`/`PlaceringUppfoljningModal` kan behöva `?? ''`. *(Var:)* **Raderar en konsulent sitt konto försvinner deltagarnas journaler, mål,
...
- [x] **SD1** ✅ **Körd 2026-09-13 (ja 1–4)** — verifierad i prod efter körning (se verif-raden i passets commit). *(Var:)* *(📝 2026-09-08 — migration `20260908110000_sd1_get_shared_profile_atomic.sql` (atomär UPDATE, `DROP COLUMN password_hash` — ingen kod läste eller skrev den); **väntar på ja**)* **`get_shared_profile` kontrollerar aldrig `password_hash`, och
...
- [x] **SD2** *(✅ 2026-09-08 — `_utils/rate-limit-fallback.js` i båda; `api/_utils/rate-limiter.js` raderad; grind `api-rate-limit-fallback.test.ts`)* **CV-PDF och bilduppladdning släpper igenom allt vid rate-limit-fel.**
...
- [x] **KG2** *(✅ 2026-09-08 — **27 påståenden i 25 guider** (inte 18) omskrivna i prod sv+en; grind `guides-pastaenden.test.ts`)* **18 av 240 artiklar bär ospårbara procentpåståenden** ("75 % av alla CV
...
- [x] **ON2** *(✅ 2026-09-08 — hjälten i du-form; B2B-rad med `<a href>` till de prerenderade sidorna)* **Landningssidans hjälte talar till inköparen** ("Stärk dina deltagare mot
...
- [ ] **BL1** *(🟡 2026-09-08 — `EmptyState` + mailto `support@jobin.se`; beslutet (kö eller stängd självregistrering) kvarstår)* *(kvar sedan 21 aug — bekräftad orörd)* `/my-consultant` utan konsulent: kort
...
- [x] **DE2** *(✅ 2026-09-08 — reserv `www.jobin.se` med hash-rutter (länkarna saknade `#`); 500 bara för `send-digest`, `check` bär `emailConfigError`)* **Tre reservvärden i mejlkoden pekar fel:** `onboarding@resend.dev`
...
- [x] **DE6** *(✅ 2026-09-08 — `verify_jwt = false` i `config.toml`, svaret nedskalat; verifiering efter deploy i passeringen)* **`/health` svarar 401 före sin egen kod** — plattformens JWT-grind;
...
- [x] **BL7** ✅ **Klar 2026-09-12 (dokumentationsdelen)** — `privacy.sharing.aiDesc` (sv + en) namnger Perplexity, de fyra funktionerna och att pendlingsplaneraren skickar adressen; Art 30-registret har en Perplexity-rad märkt DPA saknas; `Privacy.tsx` daterad 2026-09-12. **Beslut taget 2026-09-12 (Mikael): a — Perplexity AV för organisationskonton.** De fem Perplexity-anroparna (`ai-career-assistant`, `ai-company-search`, `ai-company-analysis`, `ai-commute-planner`, `ai-industry-radar`) ska för användare med `organization_id` gå på `openai/gpt-oss-120b` utan webbsökning; fria konton behåller `perplexity/sonar`. PUB-avvikelse 2 stryks när det är byggt. ✅ **Byggt 2026-09-12 (PX1).** `valjModell()` i `_shared/aiGate.ts` väljer `perplexity/sonar` bara för konton utan organisation (varken rad i `organization_members` eller koppling till en org-konsulent); alla andra får `openai/gpt-oss-120b` utan sökning, med `UTAN_SOKNING_TILLAGG` i prompten, fail closed åt basmodellen. Alla fem sökfunktionerna går genom den; `ai-company-search` hoppar över allabolag-uppslaget utan sökning; `ai_usage_logs.model` visar nu faktisk modell. Grind PX1 i `ai-sanningsregel.test.ts` fäller varje literal utanför aiGate.ts (mutationsbevisad). **Premissrättelse:** `profiles.organization_id` finns inte — tillhörighet läses ur `organization_members`. **Prod-verifierat 2026-09-12 efter deploy** (`e2e/px1-perplexity-org-prod-rok.cjs`): km-deltagare (org) → 200 och `ai_usage_logs.model = openai/gpt-oss-120b`; claude-playwright-test (ingen org, AI tillfälligt påslaget och avstängt igen) → 200 och `perplexity/sonar`. *(Var:)* **Integritetspolicyns mottagarlista saknar Perplexity.** `privacy.sharing.*`
  ...
  · mätt · beslut + ~30 min
- [x] **SD3** *(✅ 2026-09-08 — `storageCleanup.ts` rekursivt före `deleteUser()`; testet ligger i `client/src/test/` för att vitest inte ser `supabase/`)* **Kontoraderingen städar Vercel Blob men inte bucketen `profile-documents`.**
...
- [x] **AS4** *(✅ 2026-09-08 — `null` + krävt val, 4 tester)* **Uppföljningsmodalen förifyller `status: 'good'`.** En konsulent som bara
...
- [x] **MB2** *(✅ 2026-09-08 — **premissen höll delvis**: felet var `scroll-padding-bottom`, inte padding; se passeringen)* **Första jobbträffens Spara-knapp ligger 54 % under bottennavet vid
...
- [x] **MB3** *(✅ 2026-09-08 — `text-sm` som golv i `Button` och `PDFExportButton`; "Säg upp kopplingen" `md`)* **12 px knapptext på mobil** för "Säg upp kopplingen", "Exportera PDF",
...
- [x] **KG3** ✅ **Klar 2026-09-12** — `scripts/lib/og-bild.cjs` mappar kategori/sidtyp → `/og/<fil>.png` (15 bilder à 25–35 kB, genererade av `scripts/og-bilder.cjs` med Playwright, hubbfärg per kategori, inga gradients); de sju ställena i mallen bytta; grind `src/test/guides-og-bild.test.ts` (varje kategori får egen bild, okänd → fallback). Verifierat i prod: `/guider/aktivitetskrav-forsorjningsstod/` → `/og/dina-rattigheter.png`, `/for-arbetsmarknadsenheter/` → `/og/organisationer.png`. *(Var:)* **Alla 269 prerenderade sidor delar samma `og:image`** — hårdkodat på sju
  ...
  ~2–4 h
- [x] **ON4** *(✅ 2026-09-08 — `needsConfirmation`, `role="status"`, nycklar `auth.confirmEmail.*`)* **"Konto skapat!" renderas som rött fel om e-postbekräftelse slås på.**
...
- [x] **KG1** *(✅ 2026-09-08 — kluster 24 + regeln; de två **länkar inte** till varandra (premissen höll till två tredjedelar))* **Kannibaliseringskartan missar `kompetensutveckling-guide` /
...
- [x] **AS3** *(✅ 2026-09-08 — rader i §1.2/§1.3 + ställningstagande: 9.2.a är hypotesen men inte uppfylld — beslut kvar)* **DPIA:n saknar `internal_adaptation_notes`** — art. 9-nära, skrivet av
...
- [ ] **DE5** 🟡 **Gallringen har utlösare sedan 2026-09-12** (`retention-inactive-accounts` 03:00 UTC lägger 18-månadersvarningen i `email_queue`). **Kvar:** inget anropar `send-inactivity-warning` som tömmer kön — pg_net är av och `vercel.json` har bara jobbevakningens cron. Aktuellt först 2027-08 (äldsta inloggning 2026-02-23). Fix när det behövs: en rad till i `vercel.json` `crons` som anropar edge-funktionen med `CRON_SECRET` (kräver Mikaels ja — vercel.json). *(Var:)* **Inaktivitetsvarningen och gallringen har ingen utlösare.** `cron.job`
...
- [ ] **DE7** **Tokentaket är fail open vid databasfel** (50 000/dag ur env). Ett val som
  ...
  ~15 min
- [x] **KA3** ✅ **Klar 2026-09-12 (roadmap-pass 3).** `ai.js` 2 402 → 1 254 rader: hela `PROMPTS` plus REGELVERKSREGEL/SANNINGSREGEL/agentpromptarna bor i `client/api/_prompts/` (nio domänfiler + `index.js` som sätter ihop objektet i samma nyckelordning och lägger på SA4-loopen — sammansättningsstället flyttade med, understreck så Vercel inte deployar dem som endpoints). Bevis: 20 nycklar × 65 fixturer = 1 300 anrop mot HEAD-versionen, 0 avvikelser. `cloudStorage.ts` 2 677 → 13 rader (barrel `export * from './cloud'`); 17 api-objekt i `services/cloud/` per domän, `_shared.ts` internt; 31 importörer orörda; madge: inga cirklar; grind i `cloudStorage.test.ts` som räknar api-ytan (mutationstestad). Säkerhetsdel och parser ligger kvar i `ai.js` med flit — deras tester pekar dit. Två källtextläsande tester ompekade (`kompetensanalys-arlighet`, `varumarke-arlighet`). *(Var:)* **Två gudfiler:** `ai.js` 2 356 rader (säkerhet rad 62–583, 20 promptar
...
- [x] **KA4** ✅ **Klar 2026-09-12 — och den bar två skarpa buggar.** Fem levande `CVData`-kopior (två var dödkod: `ContinueWhereYouLeft.tsx`, `utils/validation.ts`, orörda och listade som undantag). `pages/Resources.tsx` och `services/cvWordExport.ts` antog `skills: string[]` — prod har objekt, så kompetenschipen och Word-exporten skrev `[object Object]`. `useDashboardData.ts` läste `cv.personal_info`, en kolumn som inte finns. Nu: `types/cv.ts` (kanonisk, med `created_at`/`updated_at` som saknades), `utils/skillText.ts` (`skillNamn`, `normaliseraKompetens`, `skillNamnLista`) i cvOptimizer/jobMatching/pdfExportService, och grinden `src/test/en-cvdata.test.ts` som fäller varje andra `interface CVData`. Taket för strict-typfel sänkt 356 → 352. Tankstrecksfyndet var redan hanterat i `pdfExportService.sanitizeText` — nu vaktat på byteströmmen. *(Var:)* **`CVData` ×7 och kompetensnormaliseringen ×8** (CB5 lagade fem; en åttonde
...
- [x] **KA5** ✅ **Klar 2026-09-12 — premissen höll, men av ett annat skäl.** jsPDF låg inte i entry; entry importerade `vendor-jspdf` statiskt för att Rollup lagt Vites preload-helper (` vite/preload-helper.js`, ingen `node_modules` i id:t så manualChunks släppte det) i den största chunken som behövde den. Fix i `vite.config.ts`: helpern får egen chunk, `vendor-animation`-regeln borttagen så framer-motion delas per modul. **Mätt brotli: statiskt laddat vid sidladdning 460 → 314 kB (−145 kB)**; entry-chunken själv oförändrad (200 kB). **Över nätet mot prod efter deploy (`bf3df9ba`, curl med `Accept-Encoding: br`):** entry + sex statiska chunkar = 385 kB, och varken `vendor-jspdf` eller `vendor-animation` finns längre bland de statiska importerna. Kommentaren i vite.config som påstod motsatsen var fel. Bifynd: `npm run verify:widget-chunks` är död (letar widget-chunkar för det arkiverade systemet, C1) — ta bort skriptet. *(Var:)* **Entry drar in jsPDF (134 kB brotli) och Framer Motion statiskt** — E13/E17
...
- [x] **BL5** ✅ **Klar 2026-09-12 (UI-delen)** — `lib/testkonton.ts` (`arTestkonto`: `@jobin.test`, `@example.com`, `claude-playwright-test@jobin.se`, `jobin.uxtest.*@gmail.com`), superadminpanelens statistik räknar utan dem och visar "Testkonton (borträknade): N"; listan har chip + "Dölj testkonton". AI-anropsstatistiken visas inte i klienten alls — den delen är en SQL-fråga med samma allowlist. *(Var:)* **Användningssiffrorna är testtrafik:** 91 av 104 AI-anrop senaste 30 d från
...
## Passering 2026-09-02 (kväll) — nio agenter, 25 poster, tio premisser som föll

> "Skicka ut sonnet-agenter som jobbar på att beta av roadplan." Nio agenter med exklusiva
> fillistor, inga git-kommandon, inga migrationer. 25 öppna poster togs; **10 av dem hade fel
> eller föråldrad premiss** — sex var redan lösta av tidigare passeringar utan att raden stängts
> (TI6, SA1, JD2, DOK4, SA3-appdelen, flyttdelen av SA5), en rörde dödkod (TI5), en fix landade i
> kod utan anropare (SA6), och två hade fel tal (SA4: 5 av 20 saknade den specifika regeln, inte 12;
> F31: 159 nycklar, inte 286). Fyra poster stängdes av huvudagenten före passet som redan lösta
> (JD3, DR7, SK3, SK6).

### Fynd som vägde tyngre än posterna själva

- **Supabase-felet lästes aldrig ut** i `SettingsTab` och `CommunicationTab` — inte "saknad
  felruta" utan "felet kunde inte ens nå en catch". (KS7)
- **Alla sju namnläsningar i insiktspanelen var trasiga** — varje insikt sa "Deltagare". (KV2)
- **Fem `wellness.consent.*`-strängar renderade den råa nyckeln i drift** — `t(key) || fallback`
  är ett mönster som aldrig faller tillbaka. (F31)
- **Kontrasten i uppföljningskortet var 1,32:1**, inte 1,93 — raden hade mätt mot fel botten. (TI1)
- **`calculateGoalCategories` fabricerade tre nollkategorier** vid tomt underlag. (KK6)
- **Ett race i rapportutkastet**: nyckeln blev klar asynkront, tidiga ändringar försvann tyst. (KA3)

### Lärdomen om raderna

Sex stängda poster stod öppna för att den som löste dem i förbifarten inte letade efter raden.
**En fix i förbifarten ska stänga sin rad** — annars kostar den en agents premissgranskning en
vecka senare. Och två poster (TI5, SA6) var korrekta fixar i kod ingen kör: **nåbarhet före
tillgänglighetsfix**, samma regel som för WCAG-svepet 2026-08-09.

### Kvar efter passet

- **SA5:** disclaimer där yrkesdatan renderas (`OccupationsTab`, `ExploreTab`, `JobCard`), och
  Yrkesbarometern som verklig källa.
- **TI5 / SA6:** `Progress`/`ProgressBars` och `useJobMatching` — koppla in eller radera.
- **KK6:** render-test för hela `AnalyticsTab` (de sex andra flikarna har).
- **DOK4-bifynd:** `landing.socialProof.*`/`trust.partnersTitle` är onåbar locale-data — städa.
- **A2:s not:** `consultant.analytics.keyMetrics.cvQuality` oanvänd nyckel.
- **Migrationshuvuden:** fem filer från 2026-08-31 sa "INTE KÖRD ÄN" fast de var körda och
  verifierade — rättade i samma commit. Regeln: **när en migration körs, ändra huvudet i samma
  commit som snapshoten.**

---

## Genomgång 2026-08-31 — konsulentvyn, åtta linser

> **Underlag:** åtta parallella granskare över `/consultant` (14 080 rader), varav två gjorde
> marknadsresearch i stället för kodgranskning. Översikt:
> https://claude.ai/code/artifact/fcb634fc-de1b-4c03-b46c-14840a6b3dfc
>
> **Premissen för hela genomgången, mätt först:** vyn är i praktiken oanvänd. Prod har
> **2 konsulentkonton, 31 kopplingar — och 0 journalrader, 0 mål, 0 möten, 0 placeringar,
> 0 rader med `last_contact_at`.** Det förklarar varför så mycket ser färdigt ut och ändå inte
> fungerar: felen har aldrig mött en användare. Det gör också att ingen datamigrering behövs för
> något nedan.
>
> **Rusta och matcha-kraven är lästa ordagrant ur förfrågningsunderlaget** (Af-2025/0017 5396,
> version 3, `ffu-rusta-och-matcha.pdf`), inte återgivna ur minnet — beloppen, möteskadensen,
> vitesbeloppet och handledartaket är verifierade rad för rad i dokumentet. Tjänsten är aktiv:
> AF hävde 57 leverantörsavtal i en resultatöversyn 13 april 2026, och ingen efterföljande
> upphandling hittades.
>
> **Inte upprepat här:** KV1–KV7 och BL1 från granskningen 9 augusti står kvar öppna längre ned
> i dokumentet.
...
- [x] **KS2** ✅ **Klar 2026-08-31 — migration körd och verifierad mot prod.** Konsulentens `ALL`-policy på `consultant_journal` och `consultant_goals` kräver nu `EXISTS` mot `consultant_participants`, alltså en **aktiv** relation. **Prövat rollat i transaktion:** en konsulent utan relation nekas med `42501`, en med aktiv relation släpps igenom. Båda proven behövdes — en policy som nekar alla hade gett samma svar i det negativa. Radantalen mättes före (0/0/2), så ändringen var riskfri. *(Beslut Mikael:)* historiska anteckningar **låses kvar oläsbara**, de raderas inte. *(Var:)* **Att säga upp sin konsulent tar inte ifrån henne åtkomsten.** Mätt mot prod:
  ...
  raden bär fortfarande den forna konsulentens id. Läs- och skrivrätten till anteckningarna, inklusive
  kategorin "Oro", består alltså efter att deltagaren själv brutit kopplingen. `docs/GDPR-ART30-REGISTER.md:319`
...
- [x] **KS1** ✅ **Klar 2026-08-31 — åtgärdad tillsammans med AG3, deployad i `c73366f1`.** **"Placeringsgrad" mätte inte placeringar.** Kortet räknar nu från `consultant_placements` i stället för `status === COMPLETED`, och `recordPlacement()` har fått sin första anropare efter att ha legat oanvänd sedan den skrevs. Med noll placeringar visar kortet **`—` och "Inga placeringar än"**, aldrig `0 %` i rött. Se AG3 i spår AG för hela genomförandet, inklusive uppföljningarna vid 3 och 6 månader och de två mutationstesterna. *(Var:)* **"Placeringsgrad" mäter inte placeringar — och kan strukturellt aldrig göra det.**
  ...
  `consultant_placements` med arbetsgivare, titel och startdatum, som `cohorts.ts` räknar korrekt
  från — men `recordPlacement()` (`consultantService.ts:466`) har **noll anropare utanför testfilen**
  och prod har **noll rader**. Kohortens "Placerade" är därför låst vid 0 %, rödmarkerad. Två tal om
  samma sak på två flikar; det ena för högt, det andra omöjligt. Detta är talet som förs in i en
  rapport till uppdragsgivaren. Utan data: `—` och en rad om varför, aldrig 0 % · ~4–6 h
- [x] **KS3** ✅ **Klar 2026-09-01 — efterhandsfrågan byggd, migration körd.** *(Beslut Mikael: fråga i efterhand i portalen.)* **Gjort:** ny RPC `grant_consultant_consent(text, jsonb)` (migration `20260901120000`) och komponenten `KonsulentSamtyckeFraga`, monterad i `App.tsx` **utanför `<Routes>`** så att alla 13 möts av frågan oavsett vilken sida de landar på — den renderar `null` för alla andra.
  **RPC och inte INSERT-policy:** `consultant_consents` har tre policyer och ingen för INSERT. Att öppna INSERT för `authenticated` hade låtit vem som helst skriva en samtyckesrad om vilken koppling som helst — och en samtyckesrad *är* bevisning. Funktionen tar **varken deltagar- eller konsulent-id** som argument; båda härleds ur `auth.uid()` och `profiles.consultant_id`, så den kan strukturellt inte användas för någon annans räkning. Idempotent, och den **avvisar tom samtyckestext**.
  **Texten är mätt, inte formulerad.** Listan över vad konsulenten ser är hämtad ur kolumnerna i vyn `consultant_dashboard_participants`: kontaktuppgifter, `has_cv`, `ats_score`, `cv_updated_at`, `completed_interest_test`, `holland_code`, `saved_jobs_count`, `last_login`, `consultant_notes`. Dagbok och mående saknas i vyn och är grindade var för sig (MV2) — därför står det rakt ut att de inte ingår. Att uppsägningen biter är också verifierat: KS2 kopplade journal och mål till en **aktiv** rad i `consultant_participants`, och `revoke_consultant_link` tar bort den. Före KS2 hade den meningen varit osann.
  **Ingen tvingande spärr.** Målgruppen är arbetssökande, ofta med psykisk ohälsa; en modal som inte går att stänga är inte "lugn vän" (DESIGN.md §1). "Jag vill tänka på det" skjuter frågan till nästa session (`sessionStorage`, inte `localStorage` — det senare hade betytt "aldrig mer"). Esc gör samma sak. Kopplingen består under tiden, vilket var beslutet.
  **Fällor på vägen:** torrkörningen föll på `consultant_consents_granted_via_check` — kolumnen hade tre tillåtna värden; `efterhandsfraga` är tillagt som ett fjärde, eftersom det säger något annat om samtyckets omständigheter. Och komponentens felhantering tappade Supabase-felets text (`err instanceof Error` är falskt för Supabase-fel), så "42501 permission denied" hade visats som "Ett fel uppstod".
  **9 tester, mutationsprövade två gånger:** borttaget kryssrutekrav fäller ett test, tom samtyckestext i RPC-anropet fäller ett annat. Båda mutationerna verifierades ligga på disk före körning — och den första omgången lämnade filen muterad när skriptet kraschade på teckenkodning, vilket är värt att komma ihåg: en mutationskörning utan `finally` kan lämna kvar sin egen mutation.
  *(Kvarstår, framåt:)* `InviteHandler` visar fortfarande samtyckesrutorna bara för STA-inbjudningar, och `handle_first_signin` skriver bara `consultant_consents` när inbjudan bär `metadata.consent_text`. Den nyinbjudne fångas nu av efterhandsfrågan vid första inloggningen i stället — fönstret där konsulenten ser data utan samtycke är därmed minuter, inte permanent. Att flytta frågan till inbjudningssidan kräver att texten finns på **ett** ställe för båda vägarna; gör inte två.
- [x] **KS4** ✅ **Klar 2026-08-31 — samma migration.** `consultant_journal` har nu en SELECT-policy för `participant_id`, som `consultant_goals` och `consultant_notes` redan hade. *(Beslut Mikael:)* **allt är synligt, även kategorin "Oro"** — art. 15 ger ändå rätt till registerutdrag, och en anteckning man vet blir läst tenderar att bli mer saklig. Journalytan säger det rakt ut för konsulenten. *(Var:)* **Deltagaren får se sina mål men aldrig anteckningarna — och mönstret fanns rätt en
  ...
  hade rätt mönster och tappade det vid tabellbytet. En "Oro"-anteckning kan stå om en människa
  permanent utan att hon kan se den, och art. 15 har ingen teknisk väg. Ska något undantas är det ett
  produktbeslut som ska skrivas ut, inte en följd av vilken tabell utvecklaren bytte till · beslut + M
- [x] **KS5** ✅ **Klar 2026-08-31.** **Massåtgärden sa "Slutfört" när sex av tjugo misslyckats.** **Gjort:** nytt state `partialFailure: string[] | null`, skilt från `error` (som fortfarande täcker "alla föll"-grenen). Båda hanterarna plockar ut de faktiskt misslyckade deltagarna ur `Promise.allSettled`-resultatet och returnerar tidigt — `setSuccess(true)` och autostängningen nås aldrig. Ny rendergren med `role="alert"` som **namnger vilka som inte gick igenom**, säger att resten lyckades, och kräver ett aktivt klick på "Stäng". Ton enligt DESIGN.md §2: information, inte varning. Inga färgval rörda (KT3 är separat). **Ny testfil** med 3 tester och fake timers: alla lyckas → autostängning efter 1,5 s; några faller → alert-vy som **inte** stänger sig ens efter 5 s simulerad tid; alla faller → fel syns. **Mutationstestat:** återinfört ovillkorligt `setSuccess(true)` fällde delvis-testet. *(Var:)* **Massåtgärden säger "Slutfört" när sex av tjugo misslyckats.** `BulkActionsDialog.tsx:189-196`
  (tagg) och `:275-282` (status) sätter `setError(...)` och två rader senare `setSuccess(true)`.
  Rendervillkoret `:390` är `{success ? <lyckadesvy> : <formulär+fel>}` — felet skrivs till state och
  visas aldrig, dialogen stänger efter 1,5 s med grön bock. Gå inte till success-vyn när `failed > 0`;
  visa ett delvis-utfall som kräver aktiv stängning och namnger vilka som föll · ~1 h
- [x] **KS7** ✅ **Klar 2026-09-02 (nio-agenterspasset).** Fem flikar åtgärdade, två (Platser, deltagarsidan) var redan klara. **Skarpare än raden:** i `SettingsTab` och `CommunicationTab` lästes Supabase-svarets `error` aldrig ut, så ett trasigt anrop gav `data: undefined` och tolkades som "ingen rad än"/"tom inkorg" — inget kastades, ingen catch nåddes. Nu tre lägen (laddar/fel/klart) med `ErrorState` + "Försök igen" i alla sju, 13 nya tester, varje fix mutationstestad. `ResourcesTab` behåller exempelmallarna vid fel men säger nu att det är exempel. *(Var:)* **Ett fel ser exakt ut som en tom lista — i alla sju flikar.** Mätt: **43 `console.error`
  och noll `setError`** i flikfilerna. Faller frågan mot `consultant_dashboard_participants` (timeout,
  RLS, kvot) får konsulenten samma skärm som den som verkligen inte har några deltagare. Portalens
  stående felklass, här sju filer i rad · `ParticipantsTab.tsx:83-99,405-414` · ~3–4 h
- [x] **KS6** ✅ **Klar 2026-08-31.** **Rapportens tidsperiod bytte rubrik men inte siffror.** **Gjort — väg (a), att ta bort väljaren:** `dateRange`-state, dropdownen och `getDateRangeLabel()` är borta. Dialogen tar nu en `periodLabel`-prop och **visar** den i stället för att låtsas bestämma den. Saknas den — vilket är fallet från Översikt, som inte har någon periodavgränsning — skrivs **"Aktuell status per {datum}"**, alltså vad datan faktiskt är. Det är ärlighetsregeln tillämpad på en rubrik. **Bonusfynd:** `handleGeneratePreview` anropade `generateReportDataUrl()` **utan `await`**, så `setPreviewUrl` fick ett Promise-objekt i stället för en sträng — förhandsgranskningens iframe kunde därför aldrig visa något. Var ett skarpt `TS2345` i full `tsc`, nu borta. **Tester:** 4 st mot den **riktiga** PDF-generatorn (ingen mock), som av-eskaperar byteströmmen före jämförelse — utan det steget är ett `toContain` mot en PDF meningslöst (fällan från `artikelPdf.test.ts`). En av dem är en regressionsvakt mot att en frikopplad väljare återinförs. **Mutationstestat:** hårdkodad "Senaste månaden" fällde 3 av 4. i18n-nycklar städade i båda språken, paritetsgrinden grön. *(Kvar, i andra filer:)* `AnalyticsTab.tsx:1061` behöver skicka `periodLabel` från sin egen `dateRange` (kräver att `dateRangeLabels` lyfts ur `handleExport`); `OverviewTab.tsx:878` ska **inte** skicka någon — reservtexten är rätt där. *(Var:)* **Rapportens tidsperiod byter rubrik men inte siffror.** `ReportGeneratorDialog.tsx:52,84-128`
  har en väljare vecka/månad/kvartal/år som bara skriver en etikett i PDF-huvudet
...
- [x] **KS8** ✅ **Klar 2026-08-31 — samma migration.** `consultant_messages` INSERT kräver nu aktiv relation i **båda riktningarna** (deltagaren svarar genom samma tabell). De två befintliga raderna kontrollerades individuellt mot relationstabellen före ändringen — båda passerar. *(Var:)* **Vem som helst kan meddela vem som helst — spärren finns bara i gränssnittet.** Mätt mot
  ...
  riktningarna · `20260323100000_consultant_features.sql:26-27` · ~2–3 h
- [x] **KS9** ✅ **Klar 2026-08-31.** **Landmina: `'completed'` där resten av koden skriver
  `'COMPLETED'`.** **Gjort:** `consultantService.ts:562` rättad till versaler, med kommentar om varför.
  **Hela filen genomsöktes efter samma felklass** — `consultant_meetings.status` är gemener i både
  `CHECK` och skrivväg (korrekt, rördes inte), `profiles.status` är versaler i båda (korrekt). Nytt
  test asserterar direkt på argumenten till `.eq()`; den gamla mocken returnerade `count: 3` oavsett
  filtervärde och kunde därför aldrig falla. **Mutationstestat:** återinförda gemener fällde exakt
  det nya testet (1 av 60). **Fälla på vägen:** första mutationsförsöket med `sed -i` normaliserade
...
- [x] **KS10** ✅ **Klar 2026-08-31 — policy körd och verifierad mot prod.** Skrivningen går till **`profiles.status`**, med `.select().single()` så en blockerad uppdatering **kastar** i stället för att tyst "lyckas" med noll rader. Policyn `20260831150000_ks10_consultant_status_only_update.sql` är körd. **Kolumnrättigheter visade sig omöjliga:** `authenticated` är samma roll för deltagare, konsulent och admin, så ett `GRANT UPDATE (status)` hade brutit både självredigering och adminvägen. Lösningen är en `SECURITY DEFINER`-kontroll som jämför hela raden före och efter med **jsonb minus `status` och `updated_at`** — vilket gör **framtida kolumner säkra som standard**: en ny kolumn måste vara oförändrad, inte uttryckligen listas. `updated_at` måste undantas eftersom en trigger sätter den innan `WITH CHECK` körs. **Verifierat rollat med ett riktigt CONSULTANT-konto:** status på egen deltagare → lyckas; `role = SUPERADMIN` → `42501`; status på främmande deltagare → 0 rader. `it.fails`-testet är omskrivet till ett vanligt test. *(Var:)* **Massåtgärden "ändra status" skriver till en kolumn som inte finns.**
  ...
  **ingen `status`-kolumn**. Mätt mot `information_schema`: kolumnerna är `id`, `consultant_id`,
  `participant_id`, `assigned_at`, `assigned_by`, `notes`, `priority`, `last_contact_at`,
  `next_meeting_scheduled`, `tags`. Statusen bor på **`profiles.status`**
  (`ACTIVE`/`INACTIVE`/`COMPLETED`/`ON_HOLD`), som vyn `consultant_dashboard_participants` läser.
  Skrivningen kan alltså strukturellt aldrig lyckas. **Enda produktionsanroparen är
  `BulkActionsDialog.tsx:306`** — massåtgärden "ändra status", som därmed aldrig har fungerat.
  **Och testet cementerar buggen:** `consultantService.test.ts:655` heter "updateParticipantStatus
  skriver mot consultant_participants" och asserterar den trasiga formen — mot en mockad klient går
  ett kolumnnamn som inte finns alltid igenom. Exakt samma fälla som `journey_goals` och
  `useJobsokHubSummary` (båda 2026-07-27). **Varför `lint:schema` inte fällde det:** grinden
  kontrollerar tabell- och kolumn*referenser*, inte kolumnnycklar i `.update()` — den kända luckan
  från 9 augusti. **Note:** KS5-fixen gör buggen synlig — där dialogen förut sa "Åtgärd slutförd!"
  säger den nu att alla misslyckades. De två buggarna dolde varandra · skriv mot `profiles`, rätta
  testet, överväg att utöka `lint:schema` till `.update()`-nycklar · ~1 h

### Sedan — skav som märks

- [x] **KA1** ✅ **Klar 2026-08-31.** **Tre knappar utan funktion, alla där man behöver dem.** **Gjort:** "Bjud in deltagare" (`ParticipantsTab.tsx:376`) har `onClick` och monterar `InviteParticipantDialog` med samma mönster som `OverviewTab.tsx:851`. **Tomtillståndet är omskrivet till `<EmptyState>`** (DESIGN.md §7) med **två skilda lägen** — "inga deltagare ännu" med CTA "Bjud in din första deltagare", och "inga sökträffar" med CTA "Rensa filter". Det var den verkliga väggen: förut renderades "Rensa filter" villkorat på att ett filter var satt, så en ny konsulent med noll deltagare hade **ingen klickbar väg alls** från fliken som heter Deltagare. "Nytt mål" (`ParticipantDetailPage.tsx:624`) öppnar nu `GoalCreationDialog` med förvald deltagare. **"Redigera" togs bort i stället för att kopplas** — det finns ingen konsulentsidig redigeringsdialog att återanvända, och att bygga en kräver en skrivväg genom `consultantService`; att lämna en död knapp var inte ett alternativ. Att kunna rätta ett telefonnummer kvarstår som eget arbete. **Mutationstestat:** borttagen `onClick` fällde testet. *(Var:)* **Tre knappar utan funktion.** Mekaniskt räknat: **5 av 174
  knappar saknar `onClick`**, tre i levande kod — "Bjud in deltagare" (`ParticipantsTab.tsx:326`),
  "Redigera" (`ParticipantDetailPage.tsx:498`), "Nytt mål" (`:624`). Den första är värst: en ny
...
- [x] **KT1** ✅ **Klar 2026-08-31.** Ny delad primitiv `components/ui/Dialog.tsx` — portal till body, `role="dialog"`, `aria-modal`, `aria-labelledby`, Esc, fokusfälla och fokusåterställning, plus `inert` på `#root` med **räknare så nästlade dialoger inte låser upp bakgrunden för tidigt**. Effektordningen är medveten: `inert`-effekten deklareras före `useFocusTrap` så dess städning hinner ta bort attributet innan fokus återställs in i subträdet. Sju dialoger migrerade. **`BottomSheet` och `Modal`, som CLAUDE.md:s komponentkatalog listar, finns inte** — katalogen är föråldrad där. **Och TI6 är löst på köpet:** `offsetParent`-shimmen ligger nu i `test/setup.ts`. Utan den ser `useFocusTrap` noll fokuserbara element i jsdom, så fokustester gick grönt även när fokushanteringen var trasig — den har alltså aldrig varit prövad förrän nu. *(Var:)* **Ingen av de åtta dialogerna går att stänga med Esc.** Mätt i alla `*Dialog.tsx`: noll
  ...
  filtrerar bort sådana element som dolda — utan shim ser testet noll fokuserbara element och går
  grönt fast fällan är trasig (se TI6) · ~1 dag
- [~] **KA2** 🟡 **Delvis klar 2026-08-31 — listans sida gjord, tillbaka-länken väntar.** **Gjort:** `searchQuery`, `filterStatus`, `view`, `sortField` och `sortOrder` läses nu ur `searchParams` (`q`/`filter`/`view`/`sort`/`dir`) och skrivs tillbaka genom en gemensam hjälpare med `{ replace: true }`. **Bifynd som rättades på vägen:** statusfiltrets `<select>` anropade `setSearchParams({...})` och **raderade därmed alla andra parametrar** — hjälparen slår nu ihop i stället för att skriva över. *(Kvar:)* `ParticipantDetailPage.tsx` har fortfarande en hårdkodad tillbaka-länk utan querysträng; en kommentar i ParticipantsTab pekar ut kopplingen. *(Var:)* **Sökning och sortering nollställs vid varje återgång.** `searchQuery`, `view`,
  `sortField`, `sortOrder` är ren lokal state (`ParticipantsTab.tsx:67-70`), och detaljsidans
  tillbaka-länk pekar hårdkodat på `/consultant/participants` utan querysträng
...
- [x] **KT3** ✅ **Klar 2026-08-31.** Från **201 amber-förekomster till 55** i de sex filerna, och de kvarvarande är faktiska varningar: hög prioritet, försenad kontakt, ON_HOLD, osparade ändringar, "Oro"-markering. Fokusringar, aktiva flikar, valda rader, avatarer och taggar går nu på hub-tokens. **Granskaren mätte `--c-accent` som fokusring till 1,39–1,95:1** och valde `--c-solid` i stället (4,62:1 ljust, 7,17:1 mörkt) — den fällan står i projektets minne, och den hittades självständigt i stället för att kopieras från `CommunicationTab`, som var enda tidigare användningen. En vitlistevakt håller resultatet: varje amber-rad som inte är en dokumenterad status fäller. Gradientbaslinjen orörd på 52. *(Var:)* **Amber har tagit över som sidans färg.** Vyn sätter `domain="info"` (sky). Räknat per fil,
  ...
  ("obesvarat i sju dagar") får samma färg som "det här är valt" · ~1 dag
- [x] **KT4** ✅ **Klar 2026-08-31.** **Statusikonerna låg på 2,86:1 — under SC 1.4.11.** **Gjort:** `text-amber-600` → `text-amber-700` på samtliga `bg-amber-100`-par i fyra filer (`OverviewTab.tsx:108,173,188,740-742`, `ConsultantStats.tsx:43`, `InsightsPanel.tsx:114,160`, `AnalyticsTab.tsx:100,861`), och `dark:`-varianten som saknades helt på `OverviewTab:173,188` är tillagd enligt filens eget mönster. **Granskaren räknade om själv och bekräftade talen exakt** (2,86 → 4,51; `amber-200` som botten ger 2,56 och är alltså fel väg). **Mörkt läge räknades på den faktiska alfablandningen** av `amber-900/30` och `/40` ovanpå `stone-800`, inte på opak amber-900: 7,64:1 respektive 9,25:1 — båda klarar med marginal och lämnades. **Vakt:** `statusikoner-kontrast.test.ts`, 8 tester som stryker kommentarer före matchning och kräver både att paret inte återinförs och att varje `bg-amber-100` har en mörk granne. **Mutationstestat:** återinfört par fällde 2 av 8. `lint:design` orörd på 52. *(Var:)* **Statusikonerna ligger på 2,86:1 — under SC 1.4.11.** "Behöver kontakt" och "lågt
  engagemang" (`OverviewTab.tsx:173,188`) använder `text-amber-600` på `bg-amber-100`, samma par i
  båda lägena. Egen uträkning: L(#D97706) ≈ 0,280, L(#FEF3C7) ≈ 0,885 → **2,86:1** mot kravet 3:1.
  **Rättelse mot granskarens förslag:** `amber-200` som botten ger **2,56:1** — sämre. Bara textsidan
  hjälper: `amber-700` ger **4,51:1**. Mönstret återkommer i `ConsultantStats.tsx:44`,
  `InsightsPanel.tsx:114,132`, `AnalyticsTab.tsx:100` · ~1–2 h
- [x] **KT2** ✅ **Klar 2026-08-31.** **Deltagarlistan gick inte att använda med skärmläsare.** **Gjort:** radkryssrutorna i både rutnäts- och listvyn har `role="checkbox"`, `aria-checked` och `aria-label` ("Välj {förnamn efternamn}"). Välj-alla fick **tre lägen** — `true`/`"mixed"`/`false` — vilket är det korrekta beteendet när några men inte alla är valda. Vyväxlaren har `aria-label` + `aria-pressed`, sorteringsriktningen `aria-label`. **Alla sju `<th>` har `scope="col"`**, och de fyra sorterbara bär `aria-sort` som speglar aktuell sortering. Inga nya i18n-nycklar behövdes — `t()` med `defaultValue`, enligt att konsulentvyn är medvetet oöversatt. *(Var:)* **Deltagarlistan går inte att använda med skärmläsare.** Kryssrutorna är tomma
  `<button>` som bara innehåller en ikon *när de är valda* — i omarkerat läge finns varken text eller
  roll. Välj-alla, vyväxlaren och sorteringsriktningen saknar etikett. Tabellen är en riktig `<table>`
  men har **8 `<th>` och noll `scope`**. Massutskicket går inte att nå utan mus ·
  `ParticipantsTab.tsx:287-322,445-455,546-578` · ~1–2 h
- [x] **KA3** ✅ **Klar 2026-09-02.** Utkastlager i **`sessionStorage`** (inte `localStorage`, som raden sa): `includeConcern` kan ta med orosanteckningar i utkastet, och `useCVAutoSave.ts` gjorde samma avvägning 2026-05-09 för delade datorer. Nycklat per konsulent+deltagare, debounce 800 ms, flush vid unmount. Vid öppning: "Fortsätt på utkastet från <tid>" / "Börja om", generera-knappen spärrad tills valet gjorts; rensas vid Börja om/Kopiera/Kasta, inte längre vid Stäng. Ett race hittades på vägen: lagringsnyckeln blev klar asynkront och en tidig ändring kunde gå förlorad tyst — löst med nyckeln i state. 7 tester, mutationstestat. *(Var:)* **Rapportutkastet finns bara i minnet.** `ReportDraftDialog.tsx:37-39` håller `draft` i
  lokal state och `:118-122` nollställer vid stängning; enda vägen ut är urklipp. Blir hon avbruten
  mitt i en AI-genererad och handredigerad rapport är den borta. Samma buggklass som CB1 (CV-utkastet)
  — **flusha den väntande debouncen vid unmount**, vid SPA-navigering körs varken `visibilitychange`
  eller `beforeunload` · ~2–3 h
- [x] **KK1 + KV1** ✅ **Klara 2026-08-31 — lösta tillsammans, eftersom de är samma trasiga tillståndshantering.** **Gjort:** ett `error`-state, en `activeParticipantIdRef`, och en `useEffect` som **nollställer** deltagare, mål, journal och tidslinje vid varje byte innan hämtningen startar. `fetchParticipantData` kontrollerar `isStale()` efter **varje** `await` — ett sent svar för en övergiven deltagare kastas i stället för att skriva över den aktuella vyn. Deltagarfrågans fel- och tomfall sätter nu ett uttryckligt felmeddelande i stället för att lämna föregående tillstånd orört (det saknade `else`:t i KV1). Ny rendergren med `role="alert"` mellan `loading` och `!participant` — **tre lägen: laddar / fel / klart**, som DESIGN.md kräver. **Testerna byter deltagare utan att montera om**, via en komponent som exponerar `useNavigate` — direktladdning hade sett rätt ut, samma insikt som `radgivarSidbyte.test.tsx` bygger på. **Mutationstestat två gånger:** borttagen felgren fällde KV1-testet; `isStale()` ersatt med `() => false` fällde KK1-testet, med den gamla deltagarens namn kvar på skärmen. *(Var:)* **Snabbt deltagarbyte kan visa fel persons anteckningar.** `fetchParticipantData`
  (`ParticipantDetailPage.tsx:284-358`) gör tre sekventiella `await` utan avbrytning eller kontroll av
...
- [x] **KK7** ✅ **Klar 2026-08-31 — `ParticipantJournal` inkopplad, textarean borta.** Beslut av Mikael. Den bara textarean sparade **alltid `category: 'GENERAL'`** och gick inte att redigera eller radera; journalytan har kategorier, redigering och radering. **Det gör `ReportDraftDialog`s "Oro"-filter meningsfullt** — filtret fanns i datamodellen men gränssnittet lät aldrig konsulenten sätta kategorin, så skyddet var verkningslöst. **Tre premisser i den gamla komponenten föll vid kontroll mot prod:** den förutsatte kolumnerna `updated_at`, `is_goal`, `goal_deadline` och `is_completed`, som **inte finns** — måldeadline-funktionen hade sett ut att fungera och tyst tappats vid omladdning. Den saknade helt `dark:`-klasser (skriven före mörkt läge). Och den skötte `consultant_id` själv; nu äger föräldern persistensen. **Ett fynd värt att minnas:** en UPDATE eller DELETE som RLS blockerar returnerar `{data: [], error: null}` — **inget fel**. Båda hanterarna behandlar därför tom radmängd som misslyckande, annars hade en nekad radering sett lyckad ut. Ytan säger rakt ut att deltagaren kan läsa allt, inklusive "Oro".
  ...
  ligger där: `ParticipantJournal` har kategorier, redigering och radering; den som körs är en
  textarea som alltid sparar `category: 'GENERAL'`. Och `ReportDraftDialog.tsx:68-71` filtrerar
  medvetet bort kategorin "Oro" ur rapportutkast — **en kategori gränssnittet aldrig låter henne
  sätta.** Skyddet finns i datamodellen och är verkningslöst. **Avgör om `ParticipantJournal` ska
  ersätta textarean innan resten arkiveras**, och kör inga svep över mappen dessförinnan (lärdomen
  9 augusti) · beslut + ~15 min
- [x] **KK5** ✅ **Klar 2026-09-06 — premissen höll delvis, och den halva som redan var löst var löst på rätt nivå.** Mätt mot `pg_policies` i prod och mot `20260831140000_ks_consultant_rls.sql`: **KS2 (stängd 31 augusti) begränsar redan `consultant_journal` och `consultant_goals` till aktiva relationer via `EXISTS` mot `consultant_participants`** — journal- och måldelarna av exporten uteslöt alltså avslutade relationer strukturellt, oavsett vad applikationskoden gjorde. Det som **inte** var täckt, och som verifierades sakna motsvarande spärr: `consultant_meetings`, `consultant_messages` och `consultant_placements`. **Gjort:** `handleExportData` hämtar nu aktiva deltagar-id:n via nya `consultantService.getActiveParticipantIds` och filtrerar möten, meddelanden och placeringar mot dem; journal och mål filtreras också explicit som djupförsvar, vilket kostar noll eftersom RLS redan gör jobbet. Journalkategorin **"Oro" utelämnas som standard**, med en kryssruta för att ta med den — samma mönster som `ReportDraftDialog` redan följer. **Åtkomstmodellen är orörd** — bara vad som paketeras i filen har ändrats. Att återkallelse inte tar bort journalåtkomsten kvarstår som eget beslut.
- [x] **KK3** ✅ **Klar 2026-09-12.** Sista filen, `CommunicationTab`, går nu genom `consultantService` (`markMessagesAsRead` ny, `sendMessage`, `sendBulkMessage`, `cancelMeeting`, `createMeeting`); fel visas som toast i stället för att loggas tyst. Läsningarna (SELECT) ligger kvar i fliken med flit. Bifynd `handleCancelMeeting` (native `confirm()`) rättat samma dag: `useConfirmDialog()` med Avboka/Behåll, avbokningsknappen fick `aria-label`. Warningtaket sänkt 122 → 117 (eslint gav 114–117 i olika körningar samma kväll; taket är den högsta mätningen, sänk vidare när den står stilla). *(Var:)* 🟡 **Omscopad 2026-09-12 — premissen är nästan betald.** Mätt: `ParticipantDetailPage`, `GoalCreationDialog`, `MeetingSchedulerDialog`, `PlacementDialog`, `ParticipantsTab`, `OverviewTab` har **0** `supabase.from(`; kvar är bara `CommunicationTab` (tre insert: två `consultant_messages`, ett `consultant_meetings`) — åtgärdas i pass 9. *(Var:)* **Skrivvägar dupliceras utanför det enda testade lagret — och har redan glidit isär.**
  `consultantService.ts` (59 tester) anropas bara av `BulkActionsDialog`. 13 av 14 filer som skriver
  mot `consultant_*` gör egna `supabase.from()`-anrop i komponenten.
  `ParticipantDetailPage.handleCompleteGoal`/`handleEditGoal` (`:392-410`) gör exakt vad
  `consultantService.completeGoal`/`updateGoal` (`:340-362`) gör — men fick aldrig D11:s auth-guards ·
  ~4–6 h
- [x] **KK4** ✅ **Klar 2026-09-06 — premissen höll, men raden hade tre fel i detaljerna.** (1) Det var **fyra** dubblettfrågor, inte fem: `OverviewTab`, `AnalyticsTab`, `CommunicationTab` och `ParticipantsTab`. (2) `SettingsTab:295` som raden pekade ut frågar aldrig `consultant_dashboard_participants` — hänvisningen hörde till KK5:s exportkod. (3) `ParticipantDetailPage` har en **annan frågeform** (`.eq('participant_id', id).single()` — en rad, inte listan) och är medvetet lämnad orörd: att tvinga in den i list-cachen hade gjort direktnavigering dyrare utan vinst. **Gjort:** ny modul `consultantParticipantsQuery.ts` med en nyckel, en `useConsultantParticipants()` för den hook-drivna fliken och en `fetchCachedConsultantParticipants(queryClient)` för de tre imperativa, plus `useInvalidateConsultantParticipants()` där en mutation faktiskt ändrar vyns fält. **En nyckel, en form, en ägare** — lärdomen från `['application-stats']` är respekterad. **Bifynd som var en riktig bugg:** `CommunicationTab`s realtidsprenumeration hämtade om **hela** deltagarlistan vid varje meddelandehändelse, trots att ett meddelande aldrig ändrar den. **Verifierat:** 172 tester i 14 filer gröna; ny `consultantParticipantsQuery.test.tsx` bevisar att två samtidiga konsumenter ger **en** nätverkshämtning. **Mutationstestat:** nyckeln gjord unik per anrop (`Math.random()`), mutationen bekräftad applicerad, alla 5 tester röda, återställd ⇒ gröna.

- [x] **KK6** ✅ **Klar 2026-09-02.** `computeMonthlyProgress`/`calculateTrends`/`calculateGoalCategories` utbrutna till `pages/consultant/analytics.ts`, 18 nya tester (40 i katalogen). Bifynd: `calculateGoalCategories` fabricerade tre nollkategorier ("CV-förbättring: 0 mål") vid tomt underlag — nu `[]` och en invit i UI. Injicerbar `now`, ingen division med noll. Render-test för hela `AnalyticsTab` saknas fortfarande (övriga sex flikar fick sådana i KS7). *(Var:)* **Ingen av de sju flikarna har en testfil.** 4 av 160 testfiler rör vyn. Tre mutationer
  som ingen test skulle fånga: tömma `setParticipants(data || [])` (`ParticipantsTab.tsx:95`), stryka
  `{ onConflict: 'consultant_id' }` ur upserten (`SettingsTab.tsx:266-268`), och ta bort
  `.eq('id', goalId)` ur "slutför mål" (`ParticipantDetailPage.tsx:395`) — den sista skulle markera
  **samtliga konsulentens mål för alla deltagare** som klara, eftersom RLS bara filtrerar på
  konsulent. Bryt ut `computeMonthlyProgress`, `calculateTrends` och `calculateGoalCategories` ur
  `AnalyticsTab.tsx:468-620` till en `analytics.ts` — samma grepp som gav `cohorts.ts` sina 11 tester ·
  ~3–4 h

### Framåt — vad marknaden kräver

> Kraven är verifierade ordagrant i förfrågningsunderlaget. **Den goda nyheten:** datamodellen är
> redan halvbyggd mot rätt affärsmodell — `consultant_placements` bär `followup_3m` och `followup_6m`,
> exakt R&M:s två utbetalningspunkter, och `consultant_meetings` har `duration_minutes` och ett
> `meeting_type` som skiljer `physical` från `video`. Det som saknas är skrivvägar och en nivåkolumn.

- [ ] **RM5** 🔴 **beslut först — är kunden en konsulent eller en organisation?** Sökning på `team_id`,
  `org_id`, `manager`, `handover` i konsulentmodulen ger **noll träffar**. Datamodellen är
  `profiles.consultant_id`: en nyckel, en relation. Ingen "ta bort deltagare", ingen överlämning,
  ingen chefsvy. Relationen kan bara avslutas **från deltagarens sida**
  (`RevokeConsultantLinkSection.tsx` finns bara där) — en konsulent som slutar eller blir
  föräldraledig lämnar sin caseload inlåst tills varje deltagare själv säger upp kopplingen. Avtalet
  har dessutom ett tak på **50 samtidiga deltagare per heltidshandledare** (FFU §4.5.2, proportionerligt
  vid deltid) som inte går att visa för någon. **Det här beslutet styr datamodellen och bör tas före
  allt bygge nedan** · beslut, sedan ~2 veckor
- [ ] **RM1** **Resultatklockan.** Tidslinje per deltagare: nivå A/B/C, placeringsdatum,
  tvåmånadersfönstret för tillträde, och utbetalningspunkterna vid 3 och 6 månader med status
  väntar/verifierad/utbetald. Belagt i FFU §5.1–5.1.2: grundersättning **55/70/90 kr per dag**,
  resultatersättning **20 300/27 700/38 000 kr** (halva efter 3 mån, resten efter 6, tillträde senast
  2 mån efter avslutad placering), tilläggsersättning **3 630/4 620/5 940 kr** om resultatet nås inom
  första halvåret. Nivån är låst hela placeringen. **Bygg KS1 först** — klockan ska stå på riktig
  placeringsdata. Visa `—` tills placeringen är registrerad, aldrig ett gissat belopp · ~1 vecka
- [ ] **RM2** **Närvaro och avvikelse.** FFU §4.4 kräver rutiner för avvikelserapportering, och §6.12.2
  sätter vitet vid **50 000 kr per upptäckt tillfälle** för systematiska brister — avtalets högsta
  enskilda, över lokaler, personal och kvalitetsledning. Portalen har ingen närvarofunktion alls.
  Samma funktion är kommunens viktigaste: deltagandet kan påverka rätten till ekonomiskt bistånd
  (SoL 4 kap. 4 §), och biståndshandläggaren behöver datum, orsak och bedömd giltighet som underlag.
  **Bygg giltig/ogiltig frånvaro som eget fält** — det är skillnaden som gör den användbar i båda
  marknaderna · ~1 vecka
- [x] **RM3** ✅ **Klar 2026-09-12** — `services/moteskadens.ts` (14 dagar individuellt, 28 fysiskt, "snart" inom 3 dagar, bara `status='completed'` räknas, lokal tid) + chip i deltagarlistan (kort- och tabellvy) med tre lägen; 14 + 3 tester, mutationskontrollerade (14 → 15 dagar rött). *(Var:)* **Möteskadensen.** FFU §4.1.1: individuellt möte minst var **14:e kalenderdag**, längd
  **30/45/60 min** efter nivå, endast av handledare. Första mötet fysiskt, därefter fysiskt minst var
  fjärde vecka. Byggstenarna finns i `consultant_meetings`; kadensbevakningen saknas. En kolumn i
  listan: dagar sedan senaste möte, veckor sedan senaste fysiska, flagga **innan** gränsen passeras ·
  ~3–4 dagar
- [x] **RM4** ✅ **Klar 2026-09-12 (roadmap-pass 3), byggd på KM-datan.** `services/aktivitetslogg.ts` (ren logik, 17 tester): närvarotid = `present`/`external`, krav 1 h/v planmånad 1–6 och 2 h/v 7–12 dag-exakt från `activity_plans.start_date`, vecka mån–sön lokal tid, bara avslutade veckor bedöms. Fysiskt = `activity_type = 'workplace'` eller ifylld plats — en härledning, inte en flagga; vill AF ha exakt 50 %-underlag behövs kolumnen `is_physical` (migration, väntar på ja). Andel = null (visas `—`) utan närvaropass, aldrig 0 %. `AvtalskravKort.tsx` i Rapporter under IVO-kortet: månadsväljare, tre lägen, tabell per aktiv plan, amber under 50 %/missade veckor, textrad om att kravet gäller R&M-avtalet och inte kommunens aktivitetskrav. Ingen migration. *(Var:)* **Aktivitetsloggen.** FFU §4.1.1: månad 1–6 minst **1 tim/vecka**, månad 7–12 minst
  **2 tim/vecka**. Minst hälften av aktiviteterna fysiska, och **den periodiska rapporten ska visa att
  minst 50 % var fysiska per deltagare**. Obs: 50 %-kravet gäller *aktiviteterna*, inte de individuella
  mötena — därför skild post från RM3 · ~3–4 dagar
- [ ] **RM6** **Periodisk rapport i avtalets format.** FFU §5.1.1: grundersättning betalas **inte ut**
  utan godkänd periodisk rapport och gemensam planering via AF:s systemstöd; ny placeringsperiod kräver
  uppdaterad planering inom 5 arbetsdagar. Dagens rapportgenerator producerar fri text.
  **Notera begränsningen:** det finns **inget öppet leverantörs-API mot Arbetsförmedlingen** — allt
  registreras manuellt i "Mina sidor för fristående aktörer" (MSFA). Portalen kan alltså inte lova
  automatisk synk; funktionen är ett internt underlag som konsulenten för över · ~1 vecka
- [ ] **KM1** 🔴 **beslut — kommunen är en annan kund än leverantören, och skillnaden är juridisk.**
  En R&M-leverantör är ett privat företag med en uppdragsgivare och resultatbaserad ersättning. En
  kommunal arbetsmarknadsenhet är myndighetsutövning: deltagandet kan påverka ekonomiskt bistånd,
  verksamheten omfattas av OSL (inklusive möjlig inre sekretess mot den egna socialtjänsten), och
  uppföljningen går till nämnden **och** till nationell statistik (SKR:s årliga insamling, numera
  publicerad i Kolada; INUK för DUA-unga). Tre saker portalen saknar: rollbaserad åtkomst med
  sekretessgräns inom samma organisation, avvikelseunderlag som duger som beslutsunderlag (RM2), och
  en årssammanställning. **Marknaden är inte tom** — *GW Arbetsmarknad* (YongLogic) säljs redan till
  kommuner med närvaromodul, och socialtjänsten ligger ofta i Treserva. **Portalens tydligaste
...
- [x] **ÖV1** ✅ 2026-09-12 (VIEWED_PARTICIPANT_DATA + participant_id + deltagarens policy; kortet i Min konsulent) **En läslogg deltagaren själv kan se.** Enda loggade konsulenthändelsen är
  ...
  heller deltagaren** kan se vem som tittat. Logga `VIEWED_PARTICIPANT_DATA` (befintlig INSERT-policy
  räcker), lägg `participant_id` som egen kolumn i stället för gömd i `old_value`, och ge deltagaren
  en policy för rader om sig själv. Gör KS4 hel · ~4–6 h, deltagarvy 1–2 dagar
- [x] **KA4** ✅ **Verifierat klar 2026-09-12** — `BulkActionsDialog.tsx` har grenen `contact` som anropar `consultantService.logContact()` per vald deltagare (rad ~358, ~403); raden stod kvar som öppen efter KV3. *(Var:)* **Massåtgärder saknar det en grupp faktiskt behöver: logga kontakt.**
  `BulkActionsDialog.tsx:318-333` har fyra grenar — meddelande, tagg, status, export — och ingen
  skriver till `consultant_journal` eller sätter `last_contact_at`. Efter en workshop med åtta
  deltagare måste samma rad skrivas åtta gånger. **Bygg tillsammans med KV3**, som är skrivvägen till
  `last_contact_at` som aldrig kopplades in · ~2–3 h
- [x] **KA5** ✅ **Klar 2026-08-31.** **Varken möte eller mål gick att skapa från deltagarens egen sida.** **Gjort:** `GoalCreationDialog` och `MeetingSchedulerDialog` monteras nu i `ParticipantDetailPage` med `preselectedParticipant={participant}` — propen fanns redan i båda, byggd för precis det här. "Nytt mål" öppnar den första; en ny "Boka möte"-knapp i huvudet öppnar den andra. Efter sparat mål hämtas bara målen om, genom en egen `refetchGoals()` med samma inaktualitetsvakt som KK1 — inte hela sidan. Konsulenten slipper byta flik och söka fram den person hon just tittade på. *(Var:)* **Varken möte eller mål går att skapa från deltagarens egen sida.**
...
| Hypotes | Hur den föll |
|---|---|
| "Mörkna chippet till `amber-200` så klarar statusikonen kontrastkravet" | Granskarens egen fix. Egen uträkning: `amber-600` på `amber-200` ger **2,56:1** — sämre än de 2,86:1 som var problemet. Bara textsidan hjälper (`amber-700` → 4,51:1). Fyndet står, åtgärden utbytt i KT4 |
| "Minst 50 % av de individuella mötena ska vara fysiska" | Två krav sammanblandade. FFU §4.1.1 säger om *individuella* möten: första fysiskt, därefter minst var fjärde vecka. 50 %-kravet gäller *aktiviteterna*, och det är där skrivningen om periodiska rapporten står. RM3 och RM4 är därför skilda poster |
| "Konsulentvyn har en kollision på delad React Query-nyckel" | Rimligt — mönstret fällde `['application-stats']`. Men `queryKey`, `setQueryData` och `invalidateQueries` ger **noll träffar** i hela modulen; allt hämtas med `useState` + `useEffect`. Kollisionen kan inte uppstå. Priset står i KK4 i stället |
| "Vyn bröt mot layoutomläggningen — kvarglömda hjältar eller gradienter" | Falskt. `Consultant.tsx:29-35` använder `PageLayout` med `SidRail`, `.sidbredd` och `domain="info"` korrekt. Layout under press håller också: tabellen har `overflow-x-auto`, långa namn bryter utan att spräcka korten. Färgvalet *inuti* ramen är däremot fel — KT3 |
| "Rusta och matcha 2 kan ha ersatts av en ny upphandling" | Kontrollerat: AF publicerade resultatöversyn 13 april 2026 (57 avtal hävda) och uppdaterade betyg i juli 2026 under samma tjänstenamn. Ingen efterföljare hittades. FFU version 3 (2025-03-27) gäller |

### Vad som är bra

- **Kodladdningen är föredömlig.** Hela vyn väger **51,8 kB brotli**, en chunk per flik, ingen över
  9 kB. Gjort rätt en gång (E8, maj) och har hållit.
- **Ärlighetsregeln har fäste där någon arbetat.** ATS-poängen visar `—` i stället för 0. "Riskerar att
  fastna" kräver två signaler. Insatsjämförelsen döljer sig under tre deltagare per grupp.
  `cohorts.ts` har 11 tester med kolumnuppsättningen ur `information_schema` och en negativ kontroll
  som bekräftar att den gamla koden gav `QNaN NaN`.
- **Den döda AI-panelen är verkligen borta** (AR3), med en kommentar på platsen om varför.

### Vad som återstår — bara Mikael kan svara

1. **Den visuella granskningen är läst i kod, inte sedd på skärm.** `/consultant` kräver rollen
   CONSULTANT och jag har ingen sådan inloggning. Kontrastvärdena är räknade ur klassnamnen och
   stämmer, men densiteten och rytmen med trettio riktiga rader har ingen sett. Bör göras innan KT3
   byggs om.
2. **Konsulent eller organisation?** Styr RM5 och därmed datamodellen.
3. **De 13 kopplingarna utan samtycke** (KS3) — efterhandsfråga i portalen, eller avkoppling?

### Dokumentationsrevision 2026-08-31 — rättat och kvarstående

Två granskare gick igenom dokumentationen: en mot koden, en mot skarp drift på jobin.se.
**Trädet var rent när genomgången började**, så de belagda faktafelen är rättade i filerna;
allt som kräver ett beslut står kvar nedan.

#### Rättat i filerna

1. **`CLAUDE.md:245` sa att det inte finns någon streaming-väg — det gör det.** Raden löd
   ordagrant *"Det finns ingen streaming-väg … skriv inte kod som antar dem"*, vilket kunde få
   nästa läsare att bygga ett duplicerat lager. Sant är att den **gamla** vägen är borta
   (`client/api/ai-stream.js` och `useAIStream` finns inte). Men `ai.js:1999` har en egen
   SSE-gren — `if (stream && fn === 'ai-team-chat')` sätter `Content-Type: text/event-stream` —
   och `callAIStream()` i `services/aiApi.ts:319` anropas av `components/ai-team/AgentChat.tsx`
   i drift. **Verifierat av mig med grep i båda ändarna.** Rättat, med förklaring av vad som
   faktiskt gäller: streaming finns för **en** funktion, genom `/api/ai`, och man får aldrig gå
   förbi `callAI`/`callAIStream` med ett eget `fetch` (då körs varken PII-saneringen eller
   art. 9-grinden — vaktat av `AgentChat.pii.test.tsx`).
2. **`CLAUDE.md:467` sa "Streaming via `useAIStream`-hooken (anropar `/api/ai-stream`)".**
   Samma fil motsade alltså sig själv, och båda påståendena var fel. Rättat.
3. **Antalet funktioner i `ai.js` var fel i fyra dokument samtidigt.** Räknat i `PROMPTS`-objektet
   2026-08-31: **20**. `CLAUDE.md` och `AI_MODEL_LOCKING.md` sa 18, `services-overview.md` och
   `AI_ARCHITECTURE_OVERVIEW.md` sa 16, och historiskt har 24 stått där. Fyra saknades ur varje
   tabell — `cv-import`, `cv-import-erfarenhet`, `intervju-sammanfattning`, `vecko-reflektion` —
   och alla fyra har levande anropare. Rättat i `CLAUDE.md` och `README.md`; **kvarstår i
   `AI_MODEL_LOCKING.md`, `services-overview.md` och `AI_ARCHITECTURE_OVERVIEW.md`.**
4. **Typecheck-taket stod som 406 i `CLAUDE.md`; konstanten är 362.** Mätt: `npm run
   typecheck:ceiling` ger *"361 strict-typfel, under taket 362"* — dagens arbete betalade av ett.
   Samma fil hade dessutom 469, 466 och 470 på andra rader. Rättat till 362 på båda ställena.
   *De två andra taken stämde exakt* — 122 warnings (aktuellt 118) och 52 gradienter.
5. **Migrationsfiler: 133 → 142.** Räknat.
6. **`e2e/`: "10 spec + 10 verktygsskript" → 10 spec + 23 `.cjs`.** Räknat. Arkivsiffran 82 stämde.
7. **Testsviten: "2 384 tester i 146 filer" → ~2 550 i ~165.** Ersatt med en uppmaning att mäta om,
   eftersom talet driver ett par procent i veckan.
8. **`README.md` beskrev ett PR-flöde med feature-grenar** — i direkt konflikt med den enda
   gällande släppproceduren (*"Allt går direkt på `main`. Inga feature-grenar."*). Det är det
   farligaste dokumentationsfelet i genomgången, eftersom det leder någon fel i just det moment
   som ÄR produktionsdeployen. Ersatt med rätt procedur och en hänvisning till `CLAUDE.md § Släpp`.
   Samma fil sa också att dev-servern kör på **5173** (den kör på 3000, och `CLAUDE.md` varnar
   uttryckligen för just den fällan) och listade `ai-stream.js` i filträdet. Båda rättade.

#### Kvarstår — kräver beslut eller hör ihop med annat arbete

- [x] **DOK1** ✅ **Klar 2026-09-02.** KS2-migrationen bekräftad körd mot `pg_policies` (fem policyer). Raden säger nu exakt vad som gäller: åtkomsten upphör vid återkallelse (EXISTS mot aktiv relation), men **ingen gallring sker** — ingen kod rör tabellerna (grep i `supabase/functions`, `vercel.json`). Beslutet arkivera/radera/lås kvar är fortfarande öppet. *(Var:)* **`docs/GDPR-ART30-REGISTER.md:319` påstår ett skydd som inte finns.** Raden säger
  att konsulentens anteckningar skyddas av "RLS kopplad till `consultant_participants`… vilket bör
  utlösa gallring". Verifierat mot prod: `consultant_journal` har **en enda policy**,
...
- [x] **DOK2** ✅ **Klar 2026-08-31.** `/ai-policy` anger nu **OpenRouter**, inte "OpenAI, Inc.", och **Perplexity redovisas** — med de fem funktionerna, att Sonar gör en webbsökning på användarens fritext, och att reseplaneraren skickar hemadressen. DPF-påståendet är borta; sidan säger att överföringsgrunden inte är fastställd, vilket är sant enligt A5. *(Var:)*  **`/ai-policy` i drift anger fel personuppgiftsbiträde.** Verifierat på skarp sajt
  ...
  **JD2**, som nu är bekräftad i drift och inte bara läst i koden. Integritetspolicyn listar
  OpenRouter korrekt men **nämner inte heller Perplexity**, trots att `CLAUDE.md` självt
  dokumenterar det som underbiträde i fem edge-funktioner · se JD2
- [x] **DOK3** ✅ **Stängd 2026-09-12:** "Jobin AB" (ett bolag som inte finns) ersatt med Glänne & Söner, enskild firma, i `privacy.controller.*` (sv + en), Art 30, DPIA, PUB-utkast. Org.nr = personnummer och lämnas på begäran, adress lämnas på begäran (enskild firma = bostadsadress; publicera inte utan beslut). *(Var:)* **Integritetspolicyn har ifyllda platshållare i publicerad drift.** Citerat:
  *"Jobin AB, Org.nr: **559XXX-XXXX**, Adress: **[Adress], [Postnummer] [Stad]**."* En publicerad
  juridisk sida saknar alltså giltig personuppgiftsansvarig. Känt sedan tidigare i
  `docs/COMPLIANCE-USER-ACTIONS.md:101`, men det är värt att veta att det står så **live** ·
  kräver uppgifterna från dig
- [x] **DOK4** ✅ **Stängd 2026-09-02 — var redan omskriven i `42407b9f` (2026-08-31).** Ingen "många användare"-formulering kvar, verifierat live. Bifynd: `landing.trust.partnersTitle` och `landing.socialProof.*` (påhittat citat, "Växer varje dag") ligger kvar i locale-filerna men refereras inte längre från `Landing.tsx` — onåbar locale-data, ingen live-risk, kandidat för städning. *(Var:)* **Startsidan bär två ospårbara påståenden.** *"Många av våra användare har varit
  långtidsarbetslösa och har … tagit sig tillbaka till arbete"* och *"Många användare har upptäckt
  nya karriärvägar"*. Mot 7 aktiva konton av 92 (mätt 9 augusti) är "många" inte belagt. **Den
  gamla "5 000+ användare"-buggen är däremot borta** — inga uppblåsta tal, inga påhittade omdömen
  och inga partnerlogotyper hittades den här gången · ~15 min
- [x] **DOK5** ✅ **Klar 2026-09-06 — `docs/PRISMODELL.md` skriven.** Premissen höll: full textsökning i `docs/` efter pris/licens/avgift/kostnad gav noll dokument som beskriver modellen, bara granskningsrapporter som citerar den som fynd. Dokumentet beskriver **det som gäller i drift**, med varje tal källhänvisat till fil och rad (`sv.json:7626,7643,7660`, `Landing.tsx:735-739,759,787,813`), och räknar ut RM5-konsekvensen: 50/100/200/500/1000 deltagare ⇒ 3 280 / 3 570 / 4 150 / 5 890 / 8 790 kr/mån. **Två fynd tyngre än punkten:** (1) **Inget `plan`, `tier` eller `organization` finns i prod-schemat** (kontrollerat mot `information_schema` 2026-09-06) — ingenting grindar tekniskt någon prisnivå, och det finns ingen Stripe- eller faktureringsinfrastruktur alls. Prissidan säljer en produktstruktur databasen inte känner till. (2) *"Mest populär"* är hårdkodad i `Landing.tsx:735-739`, inte härledd ur data. Sju öppna frågor listade i §5: moms, bindningstid, teknisk definition av "aktiv konsulent", avtalsmall.

- [x] **DOK9** ✅ **Klar 2026-09-11 (KM12:3)** — SSO, API-integrationer och anpassad branding strukna ur `landing.pricing.organization.features` i både sv och en; ersatta med schemamallar/plan/närvaro som faktiskt finns. *(Var:)* **Prissidan lovar tre funktioner som inte finns i koden.** Föll ut ur DOK5 och är verifierat oberoende 2026-09-06. `landing.pricing.organization.features` räknar upp åtta punkter. Tre av dem har **noll** motsvarande kod: `SSO-inloggning (tillval)` och `Anpassad branding (tillval)` ger noll träffar på `SSO|SAML|OIDC|single sign` respektive `white.label|egen logotyp|varumärkesanpass` i `client/src`, `client/api` och `supabase/functions`; **`API-integrationer` har inget kund-API alls** — de fyra endpointsen i `client/api/` är portalens egna, och det finns ingen dokumentation, ingen nyckelhantering och ingen OpenAPI-yta. **`API-integrationer` är den värsta av de tre**, eftersom de två andra åtminstone är märkta "(tillval)" medan den står som ingående utan förbehåll. De fem övriga håller — `Gruppadministration` backas av `BulkActionsDialog` och kohortvyerna, kontrollerat. Det här är samma felklass som granskningen 2026-08-09 kallade portalens dyraste vana, men riktad mot en **betalande köpare** i stället för mot en deltagare. Åtgärd: stryk eller märk om de tre, eller bygg dem. Kräver ditt beslut om vilket · S
...
**`DESIGN.md` §3 är inte längre osann.** Minnesanteckningen från 17 augusti sa att §3 beskrev ett
system som inte finns. Granskaren läste den i sin helhet: den dokumenterar nu korrekt `SidRail`,
`HubPage`, `OversiktPanel`, avsaknaden av hjältar och full bredd via `.sidbredd`, är versionsmärkt
v3.2, och stämmer mot `navigation.ts`. Anteckningen är inaktuell — det enda felet som återstår i
§3 är `/print-resources` (DOK7).

#### Vad som stämde

Teknikstacken med versioner, antalet edge-funktioner (24), alla nio CI-grindar (körda gröna),
hubbtabellen mot `navigation.ts`, de tio agenterna, STA-modulens avstängning, modellallowlistens
två modeller och de fem Perplexity-undantagen, samt `CLAUDE.md`s egen notering om att
konsulentvyns tabeller är tomma i prod — verifierat: 0 rader i `consultant_journal`,
`consultant_goals`, `consultant_meetings` och `consultant_placements`, 31 i
`consultant_participants`.


---

### Marknadsuppdatering 2026-08-31 — fyra kartläggningar, ~65 aktörer

Marknadskarta: https://claude.ai/code/artifact/9a7528ae-40dc-44bf-a8c7-f4109f611bd1
Bygger vidare på **spår O** (25 augusti). Bilden har ändrats på tre punkter.

**1. Tre svenska portaler finns inte längre.** `Blocket Jobb` lades ner december 2024 och köptes
inte upp. `Metrojobb` är en tom platshållarsida. `Monster Sverige` gick i konkurs juli 2025 —
`monster.se` omdirigerar nu till en LiveCareer-CV-byggare utan jobbsökning. Dessutom är **Lernia
ur arbetsmarknadsspelet**: AMU nedlagt, Rusta och matcha sålt till HelloLilly mars 2026, bolaget
sålt av staten till AURELIUS. `Jobbsafari` lever men ägs av finska Duunitori, som också äger
`Jobbland`. **En konkurrentbild från 2024 är oanvändbar i dag.**

**2. Spår O:s bedömning av Cowrite bekräftas oberoende — och skärps.** Kartläggningen 25 augusti
skrev "Cowrite är gratis för 2,3 miljoner fackmedlemmar, det är hela deras försprång". Den nya
kartläggningen kom till samma slutsats från ett annat håll, och lade till vilka: **Unionen, Vision,
Akademikerförbundet SSR, Kommunal, Vårdförbundet och Akavia** — samma produkt, sex varumärken.
Cowrite gör CV från annonslänk, personligt brev, LinkedIn-sammanfattning, CV-feedback och
**intervjuträning med AI-avatar**. Samma produkt dyker dessutom upp som tilläggsintegration i
**Workbuster**, som flera R&M-leverantörer kör sin deltagarportal på — den täcker alltså både
fack- och leverantörskanalen. *Förbehåll: jag hämtade `cowrite.com` själv, och den nämner inga
fackförbund och tar $11/PDF eller $19/mån. Granskarens svenska priser (99/179/399 kr) kommer från
en annan yta och är inte bekräftade av mig.*

**3. Ingen svensk tjänst tar betalt av den arbetssökande.** Av 22 kartlagda — portaler,
bemanningsföretag, ATS-plattformar, fack, omställningsorganisationer — tar **ingen** betalt för sin
kärnfunktion mot kandidaten. Undantagen är LinkedIn Premium (frivilligt tillval) och Cowrite för
den som inte är fackansluten. **En betalvägg mot deltagaren är alltså inte en möjlig affärsmodell
här**, och Jobins nuvarande upplägg — gratis för deltagaren, betalt av organisationen — ligger i
linje med hela marknaden.

#### Nya poster ur marknadsanalysen

- [ ] **MK1** **Ingen matchningsleverans mot verkliga arbetsgivare.** Portalen har sökning och
  bevakning men **ingen som aktivt kopplar en deltagare till en ledig tjänst.** Trygghetsstiftelsen
  har en egen rekryteringsservice med 700+ placeringar sedan 2006; TRR matchar automatiserat mot
...
- [ ] **MK2** **Lönestatistiken är tunn jämfört med konkurrenternas.** Vision har en egen databas
  ...
  gör samma sak påtagligt bättre, med data vi inte har · utred källa
- [x] **MK3** ✅ **Klar 2026-09-12.** Källan är **Yrkesbarometern** (Arbetsförmedlingen via JobTech, CC0, `data.arbetsformedlingen.se/prognoser/yrkesbarometer.json`: 4 818 rader, 219 yrken, riket + 21 län, omgång 2026-1, ingen CORS). Edge-funktionen `af-prognos` proxar med dygnscache och IP-rate-limit, ingen modell; `services/afPrognosApi.ts` tolkar strikt (okänt värde → null, aldrig påhittat); sektionen "Utsikter för ett yrke" i Arbetsmarknads-fliken visar AF:s bedömning ordagrant med omgång, datum och licens, "AF saknar prognos" när yrket inte finns. 16 tester, fem mutationer fällda, `career-arlighet` grön. Verifieras i prod efter deploy. *(Var:)* **Arbetsmarknadsprognos saknas — men datan ligger öppen.** AF:s Yrkeskompassen och
  "Var finns jobben?" ger bristyrken och regionala prognoser. Jobin har inget sådant sedan
  branschradarn raderades (den dubblerade en flik och byggde på fejkad AI). **Yrkesbarometern ligger
  som öppna data på `data.jobtechdev.se`** — en "vilka yrken efterfrågas där du bor" byggd på riktig
  prognosdata vore en belagd funktion, inte en påhittad. Hör ihop med spår O:s befintliga punkt om
  JobTech-stacken · M
- [ ] **MK4** **Flerspråkigt CV för nyanlända — AF:s Jobskills gör något vi inte gör.** Deltagaren
  fyller i på sitt modersmål (arabiska, persiska, somaliska, tigrinja) och profilen översätts
  automatiskt; matchningen är namn-, köns- och ålderslös. Det riktar sig till exakt den grupp
  "Ny i Sverige"-sidan finns för · utred
- [ ] **MK5** **Inget webbläsartillägg som fyller i externa ansökningsformulär.** Huntr, Simplify
  och Careerflow fyller automatiskt över 100+ ATS-system (Greenhouse, Lever, Workday, iCIMS, Taleo)
  — den mest handgripliga tidsbesparingen i den internationella kartläggningen. **Skilj detta från
  autonom massansökan**, som Simplify uttryckligen inte gör: den kategorin (LazyApply 2,4/5 på
  Trustpilot, Sonara) har dokumenterade kvalitetsproblem, och att Jobin saknar den är en fördel att
  kommunicera, inte en lucka · beslut + L

#### Vad marknaden bekräftar att vi ska behålla

- **Den kontinuerliga AI-coachdialogen är unik.** Konkurrenternas AI-moment är engångshändelser:
  TRR:s *Horisont* är en femminuters frågeserie, Cowrite genererar ett dokument, TNG:s *Tengai*
  håller en intervju. Ingen har en löpande dialog med specialiserade agenter.
- **Spontanansökan är unik.** Ingen av de 22 svenska tjänsterna letar upp arbetsgivare åt
  användaren — alla är reaktiva och visar annonser som redan finns.
- **Att portalen är öppen för alla är en strukturell skillnad**, inte en formulering. TRR, TSL och
  TSN kräver uppsägning från anslutet företag; facken kräver medlemskap; Rusta och matcha kräver
  anvisning. Hos TSL måste arbetsgivaren eller facket dessutom **ansöka åt individen** vid
  arbetsbrist — man kan inte söka själv.
- **Att intervjusimulatorn är rent övningsbaserad är en regulatorisk fördel.** Kategorin
  realtidssoufflé under skarpa intervjuer (Final Round AI, Verve) är omtvistad, arbetsgivare bygger
  detekteringsverktyg mot den, och EU:s AI-akt klassar rekryterings-AI som högrisk. Bygg den inte.
- **Google la ner Interview Warmup i april 2026** utan strukturerad ersättare. Den gratisluckan
  står öppen nu.
...
- ~~**KB-A — de påhittade bylinerna ligger kvar i databasen.**~~
  ✅ **Klart 2026-08-22 (beslut Mikael).** Samtliga 163 artiklar bär nu
  **Mikael Glännström, Arbetskonsulent** — en verklig, ansvarig person med ett
  namn läsaren kan kontrollera och vända sig till. Migration:
  `supabase/migrations/20260822_artikelforfattare.sql`, körd mot prod
  (163 rader, 1 unikt namn, 1 unik titel). Säkerhetskopia av de gamla värdena:
  `supabase/backups/2026-08-22-artikelforfattare.json` (37 namn, varav 36
  påhittade).
  **Fältet läses igen** — `Article.tsx` hårdkodar inte namnet, så en framtida
  artikel av någon annan kan krediteras rätt utan kodändring.
  **Guidesidorna följer med:** JSON-LD gick från `author: Organization "Jobin"`
  till `author: Person` med `jobTitle`, med organisationen kvar som
  `publisher` — portalen och den publika sidan visar samma avsändare.
  `apply-new-articles.cjs` sätter samma byline på nya artiklar i stället för
  "Jobin-redaktionen".
  **Vakt:** `src/test/artikelbyline.test.ts` läser innehållsögonblicksbilden
  och fäller om något av de gamla namnen, en myndighets- eller klinikertitel,
  eller en tom författare dyker upp igen. Mutationsprövad.
...
> Översikt att arbeta mot: <https://claude.ai/code/artifact/227bae53-753b-4c66-bdcb-f8767a35260e>
> Samma ID:n används där, här och i commit-meddelanden.

Helhetsgenomgång, inte en sidgenomgång. Nio deltagarsidor har gåtts igenom en
och en sedan 18 augusti; linserna här valdes ur det som blev kvar:
**verktygssidorna som inte granskats** (Kompetensanalys, Intresseguide, Karriär,
Utbildning, AI-team, Personligt varumärke), **konsulentvyn**, **Min vardag**,
**juridiken i drift**, **CV-byggaren**, **tillgängligheten i det nya skalet**
(omlagt 17 augusti) och en **blindfläckslins**. Bortvalt: SEO (spår K är mätt i
tre omgångar), prestanda (mätt 9 augusti), i18n (väntar på beslut, inte på en
granskning).

**Mätt på rent träd, commit `7afc606e`:** 833 TS/TSX-filer, 266 020 rader i
`client/src` · 2 086 tester i 133 filer, gröna på 41,5 s · alla åtta grindar
gröna (typfel 406/406, warnings 120/122, gradienter 52/52, schema rent över 735
filer) · CI 6 av 8 (`Security Scan` och `Lighthouse CI` röda som väntat) · prod
200 på 135 ms, sitemap 180 URL:er · 169 filer / 39 261 rader onåbara från
`main.tsx` · entry 204 kB brotli över nätet.

**Vad genomgången bekräftade som redan åtgärdat** (verifierat i koden, inte på
planens ord): `AICoachAssistant` är borta (AR3), `QNaN NaN` i kohorttabellen är
löst (AR1), `/international`s checklista har `role="checkbox"`/`aria-checked`,
`Layout.tsx` har `inert` + fokusfälla + Escape (TG1), och **UX18 är löst** —
`Settings.tsx:700-775` har återkallningskort för både wellness- och
hälsosamtycket, som dessutom stoppar delningen med konsulenten.

**Överlapp med öppna punkter:** juridiklinsen landade till stor del på SK1–SK5
och DR2–DR4, som redan står öppna sedan 17 augusti. De upprepas inte nedan; det
som står här är **nytt eller nyare än de raderna**.

### Nu — riktiga fel

- [x] **JD1** ✅ **Klar 2026-08-21.** **Hemadressen gick till Perplexity utan att AI-brytaren gällde.** **Gjort:** `checkAiEnabled` + `checkDailyTokenCap` inkopplade i `ai-commute-planner` och `ai-industry-radar`, före OpenRouter-anropet, med service-role-klienten (A19-mönstret). Fälten saneras nu också i promptbyggarna — `homeAddress`/`workAddress` respektive yrke, region och intressen gick tidigare osanerade in, och `userInterests` är en array från klienten utan tak på vare sig antal eller längd. **Grinden härleder sin egen lista** (`ai-sanningsregel.test.ts`): varje edge-funktion som nämner `perplexity/sonar` måste bära båda kontrollerna, och grinda **före** anropet. En sjätte anropare kan alltså inte glömmas bort — en handskriven lista var precis varför de här två blev kvar (samma lärdom som A20). **Mutationstestat:** bortkopplad grind fällde 2 tester, grind flyttad efter OpenRouter-anropet fällde 1. 47 tester i filen. *(Var:)* `ai-commute-planner/index.ts:50` skickar `HEMADRESS: ${homeAddress}` rakt in i en `perplexity/sonar`-prompt. **Nytt sedan SK5:** grinden finns nu — `_shared/aiGate.ts` byggdes 19–20 augusti och är inkopplad i tre av de fem Perplexity-anroparna (`ai-career-assistant`, `ai-company-analysis`, `ai-company-search`). `ai-commute-planner` och `ai-industry-radar` blev kvar. En deltagare som stängt av AI får alltså ändå sin bostadsadress skickad till ett underbiträde som varken integritetspolicyn, Art. 30-registret eller DPIA:n nämner. Mätt: `grep -rln aiGate supabase/functions` ger tre filer, `grep -rln perplexity` ger fem · två importer, mönstret finns · ~1 h
- [x] **MV1** ✅ **Klar 2026-08-21.** **Samtyckesregistret skrevs aldrig till.** **Gjort:** nytt servicelager `services/consentApi.ts` som anropar `grant_consent`/`withdraw_consent` — RPC:erna som funnits sedan 28 mars och haft noll anropare. Alla fyra skrivvägar går nu genom det: `Settings.tsx` (alla fyra typer, ge och återkalla), `AiConsentGate`, `HealthConsentGate`, `WellnessConsentGate`. Ingen skriver längre en `*_consent_at`-kolumn direkt. **Bonus:** ett misslyckat sparande loggades tidigare bara till konsolen medan växlaren såg ut att ha fungerat — Settings visar nu ett fel och ändrar inget. **Vakt:** `consent-loggas.test.ts` sveper hela `src/` och fäller varje direkt kolumnskrivning inuti ett `update`/`upsert`/`insert`/`updateProfile`-anrop, plus att de fyra vägarna faktiskt anropar `consentApi`, plus att `consentApi` täcker migrationens alla sex typer (härlett ur SQL-filens CASE-sats, inte handskrivet). **Detektorn fick skärpas efter första körningen:** den letade efter `kolumn:` var som helst och gav sju falska utslag på `authStore.ts`s interface-deklarationer och en docstring — en grind som fäller läsningar hade blivit avstängd i stället för åtgärdad. **Mutationstestat:** återinförd direktskrivning fällde 2 tester. **Sidoeffekt:** typskulden sjönk 406 → 403, taket sänkt i samma commit. *(Var:)* `grant_consent` och `withdraw_consent` skapades 2026-03-28 (`20260328100000_health_data_consent.sql:34-91`) just för att uppfylla art. 7.1 bevisbörda. Mätt: **noll anropare i hela kodbasen**, och `consent_history` har noll skrivare. `Settings.tsx:185-228` gör en ren `UPDATE profiles` i stället. Det här är rotorsaken till A30 ("noll rader") — inte en glömd körning utan en aldrig inkopplad väg. Bestrider en deltagare att samtycke gavs eller drogs tillbaka finns ingenting att visa · ~3–4 h
- [x] **MV2** ✅ **KLAR 2026-08-21 — körd och verifierad mot prod** (godkänd av Mikael). Migration `20260821120000_mv2_diary_wellness_consent.sql`. **Räckvidden mättes före körning, inte antogs:** 1 dagboksrad, 1 skribent, **0 rader från konton utan samtycke** — ingen hade gått förbi UI-grinden, och ingen låstes ute. **Verifierat i fem rollade transaktioner:** (1) INSERT som konto utan samtycke → `42501`; (2) INSERT som konto **med** samtycke → raden skapades — den positiva kontrollen är nödvändig, en grind som nekar alla hade också gett `42501` i första provet och sett rätt ut; (3) SELECT utan samtycke → 1 rad läst (art. 15); (4) DELETE utan samtycke → 1 rad raderad (art. 17); (5) UPDATE utan samtycke → `42501`. Efter körning: 1 dagboksrad, 2 profiler med samtycke, 4 policyer — oförändrat. **Policyuppsättningen lästes hel, inte bara de nya raderna:** exakt fyra policyer, alla PERMISSIVE, ingen med `cmd = 'ALL'` — alltså ingen dubblett som kunde upphäva grinden (mönstret som träffat fem gånger). Att UPDATE grindas är poängen: utan det kunde någon som återkallat sitt samtycke fortsätta lämna nya hälsouppgifter, bara genom en annan verbform. SELECT och DELETE lämnades öppna med flit. Grindar INSERT **och** UPDATE på `diary_entries` med `check_wellness_consent`; SELECT och DELETE lämnas medvetet öppna eftersom art. 15 och 17 gäller oberoende av samtyckets status. **Räckvidden kontrollerad före skrivning:** hela `/diary` ligger innanför `<WellnessConsentGate>` (`Diary.tsx:172-194`), så ingen som kan skriva i dag låses ute — grinden stänger bara vägar som går förbi gränssnittet. **PREMISSEN HÖLL INTE för `exercise_answers` — den ingår därför inte.** Övningarna har 38 kategorier, varav tre är hälsonära (Självkännedom, Välmående, Rehabilitering); de övriga 35 är Jobbsökning, Nätverkande, Arbetsrätt och liknande. Tabellen har **ingen kategorikolumn** (verifierat mot schema-snapshoten), så RLS kan omöjligt skilja dem åt, och sidan har ingen UI-grind. En blanket-grind hade krävt hälsosamtycke för att svara på en jobbsökningsövning och låst ute alla utom de få med samtycke. Se **MV2b**. *(Var:)* `mood_logs` och `interest_results` fick `check_wellness_consent`/`check_health_consent` 2026-03-28. Dagbokstexten — samma innehåll som `WellnessConsentGate` säger sig skydda — har bara `WITH CHECK (user_id = auth.uid())` (`20260317_diary_tables.sql:40`). Detsamma för `exercise_answers` (`20240303_create_exercise_answers.sql:29`), som bär fritextsvar i kategorierna Självkännedom, Välmående och Rehabilitering. **Det är inte dubblettpolicy-mönstret — grinden byggdes aldrig.** Skyddet är alltså enbart klientsidans komponent, och MV3 visar att den redan går att gå förbi · ~1 h + migration
- [x] **MV3** ✅ **Klar 2026-08-21.** **Fokusläget skrev mående förbi samtyckesgrinden och svalde felet.** **Gjort:** `Wellness.tsx` renderar nu fokuslägets wizard innanför `<WellnessConsentGate>` — grinden låg bara på `<Route path="/">`, alltså på den ena av två vägar. Och `FocusWellnessWizard` stänger inte längre wizarden när sparningen nekats: den stannar kvar och säger det, i lugn ton med `role="alert"`, enligt DESIGN.md §2. Det som skrivits finns kvar så man kan försöka igen. *(Var:)* `Wellness.tsx:25-35` returnerar `FocusWellnessWizard` **före** `WellnessConsentGate` — grinden ligger bara på `<Route path="/">`. Wizarden skriver till `mood_logs`, RLS blockerar utan samtycke, och `FocusWellnessWizard.tsx:78` gör `catch (err) { console.error(err) }` följt av `onExit()`. Ett fokusläge byggt för den som lätt tappar tråden ger alltså en falsk kvittens på att måendet loggats · ~2 h
- [x] **SA1** ✅ **Stängd 2026-09-02 — var redan lagad.** Alla åtta länkar omkontrollerade med curl + `<title>`: 200 med rätt titel på var och en, "Hjälpmedelsinstitutet" borta. `LANKARNA_KONTROLLERADES` → 2026-09-02, nytt test (https + inga dubbletter). *(Var:)* **Tre av fem myndighetslänkar i Anpassning → Resurser är döda.** Mätt med `curl -L` 2026-08-21: `av.se/…/arbetsanpassning/` **404**, `arbetsformedlingen.se/…/stod-vid-funktionsnedsattning` **404**, `arbetsformedlingen.se/for-arbetsgivare/…/anpassningsstod` **404**. Den fjärde svarar 200 men heter i portalen "Hjälpmedelsinstitutet" — en organisation som gick upp i Myndigheten för delaktighet 2014. Användaren har just valt sin anpassning och klickar för att faktiskt ansöka om stöd; tre av fem dörrar är stängda · `client/src/pages/career/AdaptationTab.tsx:161-194` · ~20 min
- [x] **CB1** ✅ **Klar 2026-08-21.** **CV-byggarens utkastlager var dekorativt.** **Gjort, tre kopplingar:** (1) unmount-cleanupen i `useCVAutoSave` flushar nu den väntande debouncen i stället för att bara rensa timern — vid SPA-navigering körs varken `visibilitychange` eller `beforeunload`, bara unmount, så allt i 800 ms-fönstret kastades tyst; (2) `CVBuilder` rensar inte längre sessionStorage-utkastet vid mount (localStorage-saneringen från maj är kvar — den är en verklig PII-läcka på delade datorer); (3) `restoreDraft()` anropas faktiskt i `loadCV()`, **före** `cv-last-saved` skrivs om — den gamla koden skrev tidsstämpeln en rad för tidigt, vilket ensamt hade gjort återställningen omöjlig. Ett återställt utkast säger ifrån med en toast; ett tyst överskrivet CV vore lika förvirrande som ett tappat. **Vakt:** `useCVUtkast.test.ts`, 18 tester — tio beteendetester av `restoreDraft` (åldersgräns, jämförelse mot servern, PII-rensning, trasig JSON) och åtta källkodsvakter för kopplingarna. **Två fällor på vägen, båda värda att minnas:** `CVPage.test.tsx` mockade en `useCVDraft` som returnerade `draft`/`saveDraft`/`hasDraft` — fält som aldrig funnits; mocken kunde ljuga fritt så länge ingen anropade hooken. Och vaktens första version **överlevde en mutation** som ersatte `restoreDraft()` med `null`, eftersom den matchade min egen förklarande kommentar i stället för koden. Vakten strippar nu kommentarer före matchning och kräver en tilldelning, inte bara att namnet förekommer. *(Var:)* Mätt: `restoreDraft()` (`useCVAutoSave.ts:280`) har **noll anropare** i produktionskod. `CVBuilder.tsx:450-454` rensar `cv-draft` ovillkorligt vid varje mount, och den enda kod som *läser* nyckeln (`CVBuilder.tsx:524-535`) gör det bara för att rensa den — datat används aldrig. Unmount-cleanupen (`useCVAutoSave.ts:139-146`) gör bara `clearTimeout()` utan att flusha den väntande 800 ms-debouncen, till skillnad från `visibilitychange`-hanteraren som gör det rätt. Skriver en deltagare något och klickar vidare i portalen inom debounce-fönstret försvinner det tyst — i portalens mest använda verktyg · ~3–4 h
- [x] **CB2** ✅ **Klar 2026-08-21.** **Utkastet rensades efter tjugofem sekunder medan kommentaren lovade fem minuter.** Hela blocket är borta — `restoreDraft()` har sin egen uttryckliga gräns på sju dygn. Vaktat: talformen är förbjuden i `CVBuilder.tsx`, och den historiska noteringen står i ord så regeln kan förbli sträng (samma lösning som KO1 landade på). *(Var:)* `CVBuilder.tsx:529` räknar `5 * 5 * 1000` = 25 000 ms medan raden ovanför säger "Om draft är äldre än 5 minuter". Faktor 12 fel. Fixas i samma svep som CB1, men noteras separat eftersom det är ett räknefel och inte ett designval · ~5 min

### Sedan — skav som märks

- [x] **SA3** ✅ **Appdelen stängd 2026-09-02 — 85 %-siffran och "Nätverkar du aktivt?" finns inte längre i `BrandAuditTab`/`auditFragor.ts`/locale.** Guiderna i prod-databasen (sex artiklar, nio påståenden) är **fortfarande orörda** — det är innehållspipelinen, inte appen. *(Var:)* **Nio ospårbara statistikpåståenden om jobbmarknaden i sex publicerade guider — och de motsäger varandra.** Mätt mot `articles.snapshot.json` och **reproducerat live**: `https://www.jobin.se/guider/natverksbyggande-guide/` säger i brödtexten "Upp till 70% av alla jobb tillsätts via kontakter" och sedan i listan "**85% av tillsatta tjänster** sker via nätverk" — i samma artikel, under rubriken "Statistiken talar". Övriga: `dolda-jobbmarknaden` (2), `informationsintervju-guide`, `kreativ-jobbsokning`, `loneforhandling-komplett` (60 %), `natverka-for-jobb`. Samma 85 %-siffra står också i appen på `BrandAuditTab.tsx:111`, kopplad till frågan "Nätverkar du aktivt?". Alla sex guider är publicerade och indexerbara. Detta är portalens mest spridda innehåll, och siffrorna styr hur någon prioriterar sin tid · ~1 h
- [x] **SA2** ✅ **Klar 2026-09-02, i samma drag som SA4.** Den delade konstanten bär svensk text + engelsk "TRUTH RULE" i en sträng och sätts in efter språkvalet. Grinden har en engelsk underlagsvariant och ett block som kräver `/never invent/` i den engelska grenen; borttagen engelsk halva fällde exakt de testen. *(Var:)* **Sanningsregeln finns bara i den svenska AI-grenen.** `adaptation-recommendations` och `adaptation-conversation` är de enda två promptarna i `client/api/ai.js` med en engelsk variant. Den svenska bär hela spärren ("Hitta ALDRIG på siffror som är regler — belopp, procentsatser, dagantal eller åldersgränser… personen fattar beslut om sin försörjning utifrån det du skriver"). Den engelska säger bara "Give concrete, warm, practical advice in English" (`ai.js:858`, `:871`). Den som läser portalen på engelska är ofta utlandsfödd — alltså precis den som minst kan bedöma om ett bidragsbelopp är påhittat · ~15 min
- [x] **SA4** ✅ **Klar 2026-09-02.** Ommätt: **18 av 20** promptar hade redan *någon* sanningsregel i egen ordalydelse — bara **5 av 20** bar den specifika om belopp/procent/dagantal. Nu en delad tvåspråkig konstant `SANNINGSREGEL` (`ai.js:188`) som fogas in i en loop direkt efter `PROMPTS`-objektet, så alla 20 och varje framtida funktion får den. Mutation: borttagen loop fällde 6 test (de två grenar som saknade eget skydd), inte alla 20 — ett mätt resultat. `typecheck:api` 0, eslint 0, 55 tester. *(Var:)* **12 av 20 promptar i `ai.js` saknar sanningsregel.** Mätt genom att balansera klamrar över `PROMPTS`-objektet: 8 har den, 12 inte — bland dem `cv-writing`, `kompetensgap`, `chatbot`, `ai-team-chat` och `vecko-reflektion`. Regeln har införts prompt för prompt när ett fynd tvingat fram den. Ett fjärde tillfälle att införa den styckvis är ett skäl att göra den till en delad konstant i stället · ~2 h
...
- [x] **KV5** ✅ **Klar 2026-09-02, båda halvorna.** Overview-kortet heter nu "Snitt ATS-poäng" och räknar snittet **bara över dem som har en poäng** (`filter(ats_score != null)` före `reduce`); saknas poäng helt visas `—` och "Ingen ATS-poäng ännu", aldrig `0 %`. Analytics-kortet heter "Andel med CV". Nyckeln `consultant.analytics.keyMetrics.cvQuality` ligger kvar oanvänd i locale-filerna. *(Var:)* **"CV-kvalitet" mäter två olika saker under samma ord.** `OverviewTab.tsx:606` visar `stats.averageProgress` (snittet av `ats_score`, där `null` räknas som 0); `AnalyticsTab.tsx:786` visar `analytics.cvCompletionRate` (andelen med `has_cv`). Båda hämtar strängen `'CV-kvalitet'` ur `sv.json`. En deltagare med komplett CV men saknad ATS-poäng ger nära 0 % på en flik och nära 100 % på nästa, i samma session · ~1 h
...
- [x] **KV2** ✅ **Klar 2026-09-02.** Embed rättad till `first_name, last_name` (kolumnbristen bevisad mot `information_schema`; exakt PostgREST-felkod inte mätt live). **Större än raden:** alla sju `.name`-läsningar i `consultantInsights.ts` var trasiga på samma sätt — varje insikt och risk visade "Deltagare" i stället för namnet. Ny `formatParticipantName()`. Mål-källan isolerad i try/catch: ett fel där fäller inte längre de redan beräknade insikterna, och tomt + fel visar "kunde inte hämtas", aldrig "Alla deltagare ser bra ut!". 13 tester, mutationstestat. *(Var:)* **`InsightsPanel` frågar efter en kolumn vyn inte har.** `consultantInsights.ts:157` gör `.select('*, participant:consultant_dashboard_participants!inner(name, user_id)')`. Vyn har `first_name`/`last_name`, **inte** `name` (kontrollerat mot `schema-snapshot.json`). `if (goalsError) throw goalsError` fäller hela panelen, inklusive de insikter som redan räknats fram. Panelen monteras i `AnalyticsTab.tsx:742`. **Omätt mot prod** — kör frågan med samma embed och läs felkoden innan fixen väljs · ~1–2 h
...
- [ ] **MV2b** **Övningarnas tre hälsokategorier har ingen grind — och kan inte få en i RLS som tabellen ser ut.** Föll ur MV2:s premissgranskning 2026-08-21. `exercise_answers` bär fritextsvar i 38 kategorier; tre av dem (Självkännedom, Välmående, Rehabilitering) är art. 9-nära, de övriga 35 är Jobbsökning, Nätverkande, Arbetsrätt och liknande. Tabellen har **ingen kategorikolumn** — bara `exercise_id`, `answers`, `is_completed` och tidsstämplar — så en RLS-policy kan omöjligt skilja en rehabiliteringsövning från en om nätverkande. Sidan har heller ingen `WellnessConsentGate`. Två vägar, och valet är ett produktbeslut: **(a)** grinda de tre kategorierna i UI:t (billigast, men skyddet ligger då bara i klienten — samma svaghet som MV3 visade går att gå förbi), eller **(b)** bära kategorin i tabellen så RLS kan grinda på riktigt (dyrare, kräver migration + backfill av befintliga rader) · beslut + S/M


- [x] **JD2** ✅ **Stängd 2026-09-02 — var redan rättad i DOK2 (2026-08-31).** `grep OpenAI AiPolicy.tsx` → 0; sidan namnger OpenRouter och Perplexity med alla fem funktionerna, och DPF nämns bara för att säga att det inte gäller. Det som saknades var vakten: `AiPolicy.test.tsx` (4 tester, mutationstestad). *(Var:)* **`/ai-policy` påstår fel biträde och ett skydd som inte finns.** `AiPolicy.tsx:107` säger "OpenAI, Inc." och `:115` säger "USA (EU-US DPF)". Faktisk leverantör är OpenRouter, och överföringsgrunden är A5 — uttryckligen olöst. Saneringen 9 augusti tog bort DPF-påståendet ur `sv.json`/`en.json` men missade den här filen, eftersom strängarna ligger som ren JSX och inte som i18n-nycklar. Perplexity nämns ingenstans. Det är portalens enda dedikerade AI-policysida · ~30 min
- [x] **JD3** ✅ **Stängd 2026-09-02 — byggd som A36 den 1 september:** `npm run lint:grants` finns, ingår i `npm run verify`, och mäter 67 definer-funktioner (3 öppna för anon, alla motiverade; 29 för authenticated mot taket 29). Raden stod kvar för att ingen letade efter den. *(Var:)* **`lint:grants` finns inte.** A36 föreslog en grind som läser `has_function_privilege` per definer-funktion och `relrowsecurity` per tabell. Mätt: inget sådant skript i `client/package.json`. Dubblettpolicy- och grant-mönstret har nu träffat fem gånger; `lint:schema` föddes ur exakt samma insikt 27 juli · ~3 h
- [x] **CB3** ✅ **Klar 2026-09-02.** `handleSave` är `async` (`mutateAsync`) och returnerar utfall; `goNext` byter steg bara vid `true`. Vid fel: `role="alert"` med "Försök igen", wizarden stannar. Ingen debounce fanns att flusha — fokusläget sparar explicit per steg, så CB1:s lager behövdes inte. 3 tester, mutationstestat. *(Var:)* **Fokuslägets CV-wizard avancerar utan att vänta på sparningen.** `FocusCVBuilder.tsx:111-114`: `handleSave()` är en fire-and-forget `mutate`, och `setCurrentStep(prev => prev + 1)` körs på nästa rad. Misslyckas sparningen visas bara en toast medan deltagaren redan är på nästa steg. Fokusläget är den väg portalen själv pekar ut för användare med minst marginal, och har svagast dataskydd av hela CV-ytan · ~2–3 h
- [x] **CB4** ✅ **Klar 2026-09-02.** Båda hanterarna sparar föregående state och rullar tillbaka vid misslyckat `updateCV`, med toast (`cv.jobAdapt.saveRollback`). 4 tester, mutationstestat. *(Var:)* **`JobAdaptPage` rullar aldrig tillbaka en misslyckad optimistisk uppdatering.** `handleAddSkill` (`:35-58`) och `handleUpdateSummary` (`:60-72`) sätter state direkt, anropar `cvApi.updateCV`, och visar vid fel bara en toast — ändringen ligger kvar i UI:t som om den sparats. Nästa laddning är den borta · ~1 h
- [x] **CB5** ✅ **Klar 2026-09-02.** Ommätt i prod: **81 av 81** kompetensposter i 17 CV:n är objekt, 0 strängar. `skillName()` exporterad ur `cvOptimizer.ts` och använd i `CVOptimizer.tsx`/`JobAdaptPanel.tsx`; `ai.js` fick en egen `skillText()` för `profile-summary` och `cv-writing` (radnumren 1009/1082 var förlegade). `unifiedProfileApi.ts` typar `skills: Skill[]`. *(Var:)* **Kompetensformen hanteras defensivt på ett ställe och naivt på fyra.** `cvOptimizer.ts:229` har `skillText()` som tål både sträng och objekt — den lagade platsen. `CVOptimizer.tsx:41`, `JobAdaptPanel.tsx:57` och `ai.js:1009,1082` gör `.map(s => s.name)` rakt av, vilket ger ordet `undefined` i AI-prompten för gamla poster i strängform. Och `unifiedProfileApi.ts:27` typar fortfarande `skills: string[]` trots att prod har objekt — samma typlögn som orsakade produktionskraschen i UX14 · ~1 h
- [x] **TI4** ✅ **Klar 2026-09-02.** `sr-only` `aria-live="polite"`-region i både `SidRail` (desktop) och `SidoflikRad` (mobil) annonserar flikens etikett vid byte, härledd under rendering (ingen `useEffect`, ingen ny lint-varning), inget vid första rendering. De fem sidorna behövde inte ändras. Nyckel `sidRail.sidoflikAnnonsering` i båda språken. 7 tester, mutationstestat. *(Var:)* **Sidoflikarna i skenan annonserar inte att innehållet bytts.** De fem sidorna med `sidoflikar` (LinkedIn, Dagbok, Externa resurser, Profil, Resurser) byter state utan ruttbyte, så `RouteAnnouncer` fångar det inte. Ingen `aria-live` säger vad som hände · ~2 h
- [x] **TI5** ✅ **Klar 2026-09-02 — men effekt noll i drift.** `Progress`/`ProgressBars` har `role="progressbar"` + `aria-valuenow/min/max` + etikett. **Premissrättelse:** båda nås bara via barrel-filen `components/ui/index.ts` och monteras av ingen levande sida — varje stapel i portalen är en handrullad `<div>`. `dead-code.cjs` luras av samma barrel-blindhet som lärdomen 2026-08-04. Kandidat för radering eller för att faktiskt användas; eget beslut. *(Var:)* **`Progress` saknar `role="progressbar"` och `aria-valuenow`.** Rena `<div>`-staplar med bredd i `style` — osynliga i tillgänglighetsträdet · ~30 min
- [x] **TI6** ✅ **Stängd 2026-09-02 — var redan löst i `42407b9f` (2026-08-31).** Shimmen ligger enbart i `test/setup.ts:77-79`; noll dubbletter i hela `client/src` (de sex filer som nämner `offsetParent` har bara kommentarer). Mutationsprov: utan shimmen föll 4 tester i Dialog/CVOnboarding — den bär last och sitter rätt. *(Var:)* **`offsetParent`-shimmen är duplicerad i tre testfiler i stället för att ligga i `setup.ts`.** Utan den ser `useFocusTrap` noll fokuserbara element i jsdom, så ett fokustest kan gå grönt fast fokushanteringen är trasig. Tre filer bär samma ~15 rader med samma kommentar. 12 av de 17 komponenter som använder `useFocusTrap` saknar testfil helt — luckan är i skyddsnätet, inte i produktionskoden · ~1 h
- [x] **KV6** ✅ **Klar 2026-09-02 (S-varianten).** "Loggade in" visades på **ett** ställe i de sju flikarna (`OverviewTab`s aktivitetsflöde) — nu "Profilen ändrades senast" med förklarande `title`. `ParticipantsTab`/deltagarsidan renderade aldrig `last_login`, bara den sanna `last_contact_at`. Vyn är orörd. *(Var:)* **"Loggade in" är `profiles.updated_at`.** Vyn gör `p.updated_at AS last_login` (`20260710120000_consultant_participant_tags.sql:44`), och etiketten visas på fyra ställen. Konsulenten läser "loggade in 16:48" om något som kan ha varit en bakgrundsskrivning · byt etikett (S) eller exponera `auth.users.last_sign_in_at` (~2–4 h)
- [x] **KV7** ✅ **Klar 2026-09-02 (S-varianten).** Verifierat: ingen edge-funktion, ingen cron i `vercel.json` (enda cronen är jobbevakningen) och ingen workflow läser `consultant_settings.notifications`. Raden är märkt "Kommande — skickas inte ännu", samma mönster som teamhanteringen. Sparningen är kvar. *(Var:)* **Notisinställningarna sparas men levereras aldrig.** `SettingsTab.tsx:190-263` skriver `consultant_settings.notifications`, bl.a. "larma när en deltagare inte loggat in på 7 dagar". Ingen cron, ingen edge-funktion, inget i `vercel.json` prenumererar. Samma sida märker redan teamhanteringen ärligt som kommande — gör likadant här, eller bygg leveransen · S (märk) / L (bygg)
- [x] **SA5** ✅ **Halvt stängd 2026-09-02.** Flyttguiden (`flyttdata.ts`/`RelocationTab`) var redan klar sedan 2026-08-21 (`UPPGIFTERNA_ANGAVS`, källa i UI). **Yrkesdatan (178 yrken, 356 tal) är fortfarande obelagd** — dokumenterat med `YRKESDATA_UTAN_KALLA_KONSTATERAT` i `interestGuideData.ts`; Arbetsförmedlingens Yrkesbarometer verifierad som verklig ersättningskälla men inte kopplad. **Kvar:** disclaimer där talen renderas och summeras (`OccupationsTab.tsx:416,466,477`, `ExploreTab.tsx:102-120`, `JobCard.tsx`), och att koppla Yrkesbarometern yrke för yrke. *(Var:)* **Yrkesdatabasen och flyttguidens regiontabell saknar källa och datum.** `interestGuideData.ts` ger ~180 yrken en "Växande/Stabil/Krympande"-prognos och ett lönespann, hårdkodat, filtrerbart och hopräknat till "X växande jobb" — som om det vore Yrkeskompassen. `RelocationTab.tsx:18-31` har snitthyra, snittlön och bostadskötid för tolv städer, presenterat som beslutsunderlag för en flytt. Ingendera bär källa eller årtal. Det är samma format som lönegolvet i `/international`, som var fel i tre år utan att någon kunde se det · källrad + datum: ~30 min; saklig granskning: större
- [x] **SA6** ✅ **Golvet borttaget 2026-09-02 — men koden har noll levande anropare.** `matchJobsToInterests`/`useJobMatching` nås bara via den döda barrel-filen `hooks/index.ts`. Fixen är rätt (svag träff ger 20 %, inte 30; 3 tester) men rör ingen produktionsyta. Koppla in eller radera är ett eget beslut. *(Var:)* **Matchningsgolvet döljer svaga träffar.** `interestJobMatching.ts:213-216` gör `Math.max(30, …)` så fort en typ matchar. En annons som nätt och jämnt nämner ett ord visas aldrig under "30 % matchning" · ta bort golvet, eller byt till låg/medel/hög · ~15 min
- [x] **BL2** ✅ **Klar 2026-09-02.** `register-flow.test.tsx` asserterar `role: 'USER'`. Skarp mutation gjord av huvudagenten: `Register.tsx:71` → `'CONSULTANT'` fällde testet, återställt. *(Var:)* **Registreringstestet asserterar inte rollen.** Mutationsstickprov: `Register.tsx:71` ändrad från `role: 'USER'` till `'CONSULTANT'` → 29 tester gröna. `expect.objectContaining` i `register-flow.test.tsx:172-177` utelämnar fältet. Ingen levande lucka — `handle_new_user()` läser aldrig klientens metadata — men inget skydd mot att det ändras · ~15 min
- [x] **BL3** ✅ **Klar 2026-08-21.** **Testsviten timeoutade under belastning.** Åtgärdad när den faktiskt blockerade `npm run verify`: under `test:coverage` föll `register-flow.test.tsx` på vitests default 5 000 ms, medan samma fil isolerat kör sina åtta tester grönt — två av dem tar 1,7–1,8 s var, och marginalen äts upp av parallella filer plus v8-instrumentering. `testTimeout` och `hookTimeout` satta till 20 s i `vitest.config.ts`, samma tal som `nav-smoke` redan satte lokalt. Inget test blir svagare: ett som verkligen hänger sig failar fortfarande, bara senare. **Verifierat:** två `npm run verify` i rad, båda exit 0, 135 testfiler. *(Var:)* Två körningar på samma rena träd: den första gav 4 timeouts på 20 s (`nav-smoke`, `aiHandlerResponse`, `aiServerConsentGate`), den andra 2 086/2 086 gröna på 41,5 s. Isolerat kör `nav-smoke` sina 67 tester på 7,1 s. Det är resursberoende, inte en logikbugg — men CI kör på delade runners, och en grind som failar slumpvis lär man sig att köra om i stället för att läsa · höj `testTimeout` för de tunga integrationsfilerna, eller begränsa parallelliteten · ~30 min

### Prövat och avfärdat

| Hypotes | Hur den föll |
|---|---|
| "Framer Motion respekterar inte reduced-motion någonstans (0 av 25)" | Tre filer använder redan `<MotionConfig reducedMotion="user">`. Rättat till 22 av 25 i TI2 — och det gör fixen billigare, inte dyrare |
| "CV-utkastet läses aldrig, så `restoreDraft` borde kopplas in" | `CVBuilder.tsx:524` läser faktiskt nyckeln — men bara för att rensa den. Slutsatsen står, mekanismen var en annan |
| A32 — Vercel-preview-CORS som kritisk | Auth går via `Authorization: Bearer`, inte cookie, så `Allow-Credentials` skyddar inget sessionstoken. Planens egen nedgradering till härdning står sig |
| UX18 — wellness-samtycket går inte att återkalla | Löst. `WellnessConsentGate.tsx:239-250` har "Nej tack", och `Settings.tsx:700-775` har återkallningskort som även stoppar delningen med konsulenten |
| "Konsulentvyn visar fortfarande fyra påhittade deltagare" | `AICoachAssistant` är raderad (AR3, 17 augusti), med en kommentar på platsen om varför |
| "12 av 20 promptar saknar sanningsregel" — första mätningen sa 33 av 41 | Den regexen fångade nycklar utanför `PROMPTS` och lät blocken överlappa. Ommätt genom att balansera klamrar över objektet: 20 toppnivåfunktioner, 8 med regeln |

### Dokumentationsfel som rättats i samma pass

Trädet var rent, så rättelserna ligger i filen. Två saker var osanna:

1. `CLAUDE.md:30` beskrev konsulentvyn som att ha en **GDPR-logg**. Koden säger
   motsatsen: åtkomstloggsknappen togs bort 2026-06-11 eftersom `audit_logs` har
   admin-only SELECT-RLS (`SettingsTab.tsx:656-657`). Det som finns är
   dataexport, inte en läslogg — och konsulentens egen läsning av en deltagares
   CV loggas ingenstans.
2. Grindlistan hette "CI-grindarna (åtta st, alla körbara lokalt)" men `npm run
   verify` kör nio kommandon; `lint:vercel` och `lint:links` saknades ur listan.
   `lint:links` är inte dekorativ — den fällde ett riktigt fynd i C27.
...
- [x] **SK1** ✅ **Roterat 2026-09-12** — båda Playwright-kontonas lösenord (`claude-playwright-test@jobin.se`, `claude-playwright-consultant@jobin.test`) bytta via Supabase admin-API; de nya ligger bara i den gitignorerade `.env.test.local` (`TEST_LEGACY_*`). Det läckta lösenordet är därmed ogiltigt. **Kvar för Mikael:** GitHub-secreten `TEST_USER_PASSWORD` (CI:s E2E Authenticated). **Premiss rättad 2026-09-12:** det finns ingen sådan secret att byta — repot har bara `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `VERCEL_TOKEN`, `VITE_SUPABASE_URL` och `VITE_SUPABASE_ANON_KEY` (mätt via `/actions/secrets`, production-miljön är tom). `TEST_USER_*`/`TEST_CONSULTANT_*` har aldrig funnits, vilket är varför 75 av 96 autentiserade e2e-tester självskippar. Att *skapa* dem är 4 fält i Settings → Secrets, men enligt D28 blir CI röd samma minut eftersom 24 specar är skrivna mot navigationen från före 2026-04-29. Ordning: D28 först (kod), sedan secrets (Mikael), med `claude-playwright-*`-kontona (AI av — inga modellanrop från CI). **✅ Klart 2026-09-12:** fyra repository secrets skapade av Mikael 12:40; omkörning av CI-run 34692213104 gav **65 passed, 2 skipped** i `E2E Authenticated` — första gången jobbet kört sina tester på riktigt sedan D1 (2026-07-10). D30 är därmed också löst (jobbet är inte längre grönt-genom-att-skippa). Kommentaren överst i `ci.yml`:s e2e-jobb säger fortfarande att secrets saknas — rättas vid nästa workflow-ändring som ändå kräver Mikaels ja pekar på det gamla och måste uppdateras — eller pekas om till km-kontona. *(Var:)* Testkontots lösenord ligger i klartext i det **publika** repots historik (9 commits) och är **fortfarande giltigt mot prod** — inloggning gav HTTP 200. Commit `4d060a16` tog bort det framåt men roterade det aldrig · rotera nu, ~15 min
- [x] **SK2** ✅ **Körd 2026-09-13 (ja 1–4)** — verifierad i prod efter körning (se verif-raden i passets commit). *(Var:)* 🟡 *(migration skriven 2026-08-17 — **väntar på Mikaels ja**)* `interest_results` har en permissiv `ALL`-policy med `with_check = NULL` bredvid den samtyckesguardade INSERT-policyn. Permissiva policyer OR:as → hälsosamtyckets grind är dekorativ. **Femte gången** mönstret träffar. **Premissrättelse:** granskningen föreslog att ersätta `ALL` med separata SELECT/UPDATE/DELETE — de **finns redan**, med identiska villkor. `ALL`-policyn är helt redundant utom att den kringgår samtyckesgrinden, så den ska bara bort; en ersättning hade lagt tillbaka en andra policy på samma operationer, vilket är själva felet. Körklar: `supabase/migrations/20260817120000_sk2_interest_results_policy.sql`, med tre verifieringssteg och förväntade svar
- [x] **SK3** ✅ **Stängd 2026-09-02 — löst av A36 den 1 september** (36 → 3 anon-nåbara definer-funktioner, alla tre motiverade, och grinden `lint:grants` håller det). Migrationen från 2026-08-17 är därmed överspelad. *(Var:)* 35 av 64 `SECURITY DEFINER`-funktioner är anropbara av `anon`. **Mätt och klassificerat:** 18 saknar `auth.uid()`-kontroll, varav 8 är triggerfunktioner (inte nåbara via PostgREST) och 4 är anon-anropbara *med avsikt* (`check_login_rate_limit` före inloggning, `get_invitation_by_token`, `get_shared_profile`, `check_rate_limit`) — de lämnas orörda. Migrationen stänger **14**: de två raderande (`cleanup_rate_limits` tömmer alla användares rate-limit-fönster → underminerar brute-force-skyddet för hela portalen), fyra informationsläckor och de åtta triggerfunktionerna. **Konsumentspårat, inte antaget:** de fyra icke-trigger-funktionerna har noll anropare i `client/src`, `client/api` och `supabase/functions`; de två samtyckesgrindarna används enbart i RLS INSERT-policyer på `interest_results` och `mood_logs` och behåller därför `authenticated` — utan det grantet hade migrationen låst ute alla från att spara måendeloggar. Alla 14 signaturer validerade mot `to_regprocedure`. Körklar: `supabase/migrations/20260817121000_sk3_revoke_definer_from_public.sql`, med fem verifieringssteg, förväntade svar (35 → **21**) och rollback
...
- [x] **DR2** ✅ **Stängd 2026-09-12 (A6):** pg_cron aktiverat, 13 jobb i `cron.job` (retention + raderingsbegäranden), plus Vercel-cronen för jobbevakningen (`0 6 * * *`). Gallringen har körts: den förfallna raderingsbegäran verkställdes 14:10 UTC. *(Var:)* Ingen schemaläggare på någon av tre nivåer: `cron.job` finns inte, `crons` saknas i båda `vercel.json`, `schedule:` saknas i alla workflows. `email_queue` har 0 rader någonsin. **GDPR-gallringen (art. 5.1.e) har aldrig körts** · välj *en* nivå · S + M
...
- [x] ~~**AR2**~~ ✅ *(klart 2026-08-22, se KB8 och KB-A)* — samtliga 163 artiklar bär nu en verklig, ansvarig byline (Mikael Glännström, Arbetskonsulent) i både databas, portal och guidesidornas JSON-LD. De 36 påhittade namnen är borta ur datan, med säkerhetskopia. Ursprunglig text: **123 av 163 artiklar bär påhittade namngivna experter som byline** — bl.a. "Katarina Holm, Handläggare Arbetsförmedlingen" (5 artiklar, däribland `nystartsjobb-guide` och `rattigheter-stod`) och "Anna Lindberg, Psykolog" (5), plus fyra olika "Arbetsrättsjurist". 30 säger "Jobin-redaktionen", 10 är tomma. **Omfattningsrättelse:** syns bara i portalen (`Article.tsx:274-278`) — guidemallen sätter korrekt `author: Organization "Jobin"` i JSON-LD, så publika sidor och Google är rena · M
...
- [x] **DR4** ✅ *(klar 2026-08-23)* **Lighthouse CI är grön. Rotorsaken var `assert`, inte `collect` — och två diagnoser före den var fel.**

  **Fel diagnos 1** (stod i raden och i `CLAUDE.md`): `--collect.staticDistDir` som CLI-flagga skulle få lhci att ignorera `.lighthouserc.js`s `url:`-lista och bara auditera `index.html`. Motbevisat genom att köra exakt CI:s flaggsats lokalt — healthchecken skriver `✅ Configuration file found`, och körningen startar på listans *första* URL, inte på `guider/a-kassa.../index.html` som autodiscovery i bokstavsordning hade gett.

  **Fel diagnos 2** (min egen, 22 aug): Chrome kraschade på GitHub-runnerns 64 MB `/dev/shm`. Rimlig hypotes, men inte orsaken. `chromeFlags` står kvar eftersom de hör hemma på en CI-runner.

  **Vad som faktiskt avgjorde det:** steget skriver numera sin utdata som `::error::`-rader i stället för bara till loggen. Jobbloggen kräver admin-behörighet för att hämta via API:t; **annotationer går att läsa publikt** via `/check-runs`. Den ändringen ensam gjorde ett fem månader gammalt osynligt fel läsbart på en enda körning — och utdatan var `lhci assert` som räknade upp underkända audits: `bootup-time`, `unused-javascript`, `total-byte-weight`, `valid-source-maps`, `csp-xss` och hela pwa-familjen. `collect` hade lyckats hela tiden.

  **Felet:** `preset: 'lighthouse:recommended'` asserterar **varje** audit på error-nivå. De fyra `categories:*`-overriderna rörde bara kategoripoängen. Presetet borttaget; kvar står performance ≥ 0,5, accessibility/best-practices/seo ≥ 0,9 och LCP ≤ 2500 ms som **varningar** — vilket var D6:s ursprungliga avsikt. En jobbportal är ingen PWA och behöver inte vara installerbar.

  **Utfall:** CI är **8 av 8 grön** sedan `e86c5b37` — första gången sedan 2 april. Höj trösklarna till `error` först när det stått grönt några körningar och variansen är känd.

  **Lärdomen är inte om Lighthouse:** ett CI-fel vars logg kräver behörighet du inte har är ett osynligt fel. Skriv diagnostik som `::error::`.

- [x] **SK4** ✅ **Klar 2026-09-06 — men premissen föll först, och omfattningen var en tjugondel av den angivna.** Raden sa "nio edge-AI-funktioner har noll samtyckeskontroll (0 av 24)". Mätt: det finns **20** funktioner, inte 24 (A27 avpublicerade fyra, och `cv-analysis` som raden och CLAUDE.md listade finns inte). **Sex** anropar en modell — de fem `ai-*` plus `learning-analyze-gap` — och **fem av sex hade redan grinden**, byggd i augusti (A23/JD1). Uppskattningen "~1 dag" gällde alltså arbete som till 83 % redan var gjort. **Gjort:** `learning-analyze-gap` har nu `checkAiEnabled` + `checkDailyTokenCap` ur `_shared/aiGate.ts`, fail closed, placerade efter autentiseringen och före modellanropet — samma mönster som `ai-career-assistant`. Funktionens förbrukning loggas dessutom mot `ai_usage_logs`, annars hade den aldrig räknats mot sitt eget tak. **Fyndet under fyndet:** fixen gick att lägga in utan att ett enda test blev rött. JD1-vakten i `ai-sanningsregel.test.ts` härledde sin lista ur `/perplexity\/sonar/`, och `learning-analyze-gap` kör `openai/gpt-oss-120b`. Vakten är breddad till kriteriet som faktiskt betyder något — *anropar funktionen en modell?* — och mutationstestad: grinden borttagen ⇒ två tester röda, återställd ⇒ 58 gröna. Ordningskontrollen jämför nu mot **den punkt där modellen nås**, inte mot fetch-raden; `learning-analyze-gap` har sin fetch i en hjälpfunktion som definieras på rad 75 men anropas på rad 343, och en rak positionsjämförelse hade blivit falskt röd. **Kvarstår:** `learning-*` är fortfarande callerlösa och omfattas av C4:s beslut att lämna dem orörda.
- [ ] **SK5** 🔴 **beslut — omscopad 2026-09-06, halva raden var redan avbetald.** Raden antydde att AI-brytaren saknades här. Den kopplades in **2026-08-21 (JD1)**, och DOK2 uppdaterade `/ai-policy` 2026-08-31 så att den uttryckligen nämner att pendlingsplaneraren skickar hemadressen till Perplexity och att överföringsgrunden **inte är fastställd** (`AiPolicy.tsx:131`). Kvar är alltså **bara beslutet**, inte något byggande. Mätt läge: `homeAddress` går genom `sanitizeForPrompt` (längdtak 200 tecken, styrtecken bort) — en **injektionsspärr, inte en PII-maskering** — och rakt in i prompten till `perplexity/sonar`, som gör en webbsökning på indatan. Adressen kommer från ett fritextfält användaren skriver i (`CommutePlannerPanel.tsx:51`), inte från en sparad profilkolumn; `savedHomeAddress`-propen är död (enda anroparen `JobSearch.tsx:1021` skickar den aldrig). Funktionen går **inte** genom `prepareAiRequest` — den vägen gäller bara `/api/ai`. **Tre alternativ, med sin kostnad:** (a) skicka bara postort/kommun — förlorar gångavstånd till station och parkeringsbedömning; (b) fråga efter restid mellan två orter i stället för adresser — samma precisionsförlust plus en UX-ändring; (c) behåll men kräv ett uttryckligt samtycke för just adressen — ingen funktionsförlust, men löser inte att adressen lämnar EU/EES. Väljs (c) kvarstår att redovisa Perplexity i Art. 30 och DPIA:n · **ditt val**
- [x] **SK6** ✅ **Stängd 2026-09-02 — löst av A30 den 1 september** (triggern `log_consent_column_change` + återfyllning). Mätt i prod 2026-09-02: `health_data` granted 1, `wellness_data` granted 2, `ai_processing` 19/1, `privacy` 23/1, `terms` 23/1. *(Var:)* `consent_history` har rader för `privacy` (18), `terms` (18), `ai_processing` (16), `marketing` (1) — **noll** för hälsa/wellness. `check_health_consent()` läser i stället en muterbar kolumn utan versionering. Art. 7.1-bevisbördan är obesvarad · ~halvdag
- [x] **DR6** ✅ *(klar 2026-08-22)* **`Security Scan` i CI var röd i minst åtta körningar i rad** på tre högallvarliga fynd i `extract-zip` → `@puppeteer/browsers` → `puppeteer-core` (GHSA-jmr9-qjv8-65gv, ovaliderad symlänk-traversering vid uppackning).

  **Beslut Mikael 2026-08-22 ("fixa allt"): väg 1 — uppgradera.** `extract-zip@2.0.1` är både senaste och den flaggade versionen, så det fanns ingen `override` att lappa med; enda vägen bort var `puppeteer-core` 24 → 25, en major på CV-PDF-renderaren.

  **Gjort och verifierat:**
  - `puppeteer-core` `^24.43.0` → `^25.8.0`. `npm audit --omit=dev --audit-level=high` i `client/` ger **0 sårbarheter** (var: 3 high). Repo-roten likaså efter `npm audit fix`, som lyfte `ws` 8.20.0 → 8.21.3 och `lighthouse` 13.3.0 → 13.4.1.
  - **Röktestat mot en riktig Chromium**, inte mot ändringsloggen: `node e2e/cv-pdf-puppeteer-rok.cjs` kör exakt de anrop `cv-pdf.js` gör — `launch`, `newPage`, `setViewport`, `goto(networkidle0)`, `waitForSelector('.cv-preview')`, `page.pdf({format, printBackground, preferCSSPageSize})`, `close` — mot `client/dist` serverad statiskt. Resultat: **111 995 byte, en sida, rätt innehåll — identiskt med före uppgraderingen.** `@sparticuz/chromium` går inte att köra lokalt (Lambda-binär), så testet använder samma dev-gren som funktionen själv.
  - **Följdkrav som var lätt att missa:** `puppeteer-core@25` kräver **Node ≥ 22.12**. `client/package.json` har därför fått `"engines": { "node": "22.x"}` — det är den raden som avgör Vercels runtime, och utan den var runtimen ett antagande. `node-version` i `ci.yml` (8 ställen) och `deploy.yml` (1) lyft 20 → 22; GitHub varnade redan för att Node 20 är avvecklat. Sänk inte tillbaka någon av dem utan att först sänka `puppeteer-core`.

  **⚠️ Första försöket tog ner CV-exporten i prod i cirka 45 minuter.** `puppeteer-core@25` är ESM-only (`"type": "module"`), och `client/api/cv-pdf.js` gjorde `require('puppeteer-core')` på modulnivå. Funktionen kraschade vid **laddning** — `FUNCTION_INVOCATION_FAILED`, även på den rena valideringsvägen som svarar 400 långt före all puppeteer-användning. Att `/api/ai` och `/api/upload-image` samtidigt svarade 401 var det som pekade ut modulladdningen. Återställd till v24, sedan gjord om ordentligt.

  **Fixen:** `await import('puppeteer-core')` inne i handlern. Dynamisk import fungerar från CJS mot både CJS och ESM. Verifierat i prod med **både v24 och v25 — samma PDF, 37 950 byte**.

  **Två slutsatser jag drog och som var fel:**
  1. *"Vercel kör Node < 22.12, därför faller `require(esm)`."* Nej. Funktionen fick rapportera `process.version` på en autentiserad felväg: **v24.18.1**, där `require(esm)` fungerar. Kvarstående förklaring är Vercels bundling av en ESM-only modul som `require`:as från CJS.
  2. *"`engines` i `client/package.json` avgör Vercels runtime."* Nej. Pinnen sa `22.x` och funktionen körde ändå 24. Runtimen sätts i Vercels projektinställningar. Fältet står nu som `">=22.12"` — ett ärligt golv för npm, inte en kontroll över Vercel.

  **Regeln som följer:** ett röktest i fel runtime bevisar ingenting om rätt runtime. `e2e/cv-pdf-puppeteer-rok.cjs` kör mot din lokala Node och Chrome och gick grönt hela tiden; `e2e/cv-pdf-prod-rok.cjs` kör mot skarp drift och var det som hittade felet. Kör det andra efter varje ändring som rör funktionen.

  Bedömningen att brådskan var låg stod fast hela tiden: `cv-pdf.js` skickar alltid en explicit `executablePath`, så `@puppeteer/browsers`' nedladdare — den enda anroparen av `extract-zip` — kördes aldrig. Det var ett argument för att inte hasta. Jag hastade ändå.
...
- [x] **KO2** ✅ **Klar 2026-09-02.** Subtiteln på Logga in/Skapa konto säger vart användaren var på väg när `returnTo` finns ("Logga in för att fortsätta till CV"), ur en liten sökväg→verktygsnamn-karta; okänt men säkert mål ger en neutral rad, osäker `returnTo` ger oförändrat beteende. 8 nya tester, båda språken. *(Var:)* Login/Register säger inte varför man hamnade där. `returnTo` fungerar tekniskt (K11), men sidan säger "Välkommen tillbaka" till någon som just klickade "Bygg ditt CV" på en guide · `Login.tsx:89-107`, `Register.tsx:111-129` · ~2 h
...
- [x] **BF2** ✅ **Klar 2026-09-06 — `docs/INCIDENT-RESPONSE.md` omskriven mot den verkliga organisationen.** Premissen höll, och var värre än raden sa: rollerna hade aldrig funnits. `git log --format='%an' | sort -u` ger **en** författare, och `GDPR-ART30-REGISTER.md:489-490` och `DPIA-PORTAL.md:22` bekräftar att DPO- och personuppgiftsansvarigfälten fortfarande är ofyllda platshållare. En incidentplan som namnger roller som inte finns är sämre än ingen, eftersom den skapar en illusion av beredskap. **Gjort:** varje `[utse]` och varje fiktivt team ersatt med det som gäller — en person, ingen DPO, ingen kontrakterad jurist eller pentestfirma. Nytt avsnitt om **vem som upptäcker**: Sentry startar bara efter samtycke till analytics-cookies (`sentry.ts:57`), och det finns ingen kontinuerlig driftövervakning — bara ett röktest vid deploy (`deploy.yml:186-217`, inget `schedule:` i någon workflow). IMY:s 72-timmarsfrist är mappad mot var underlaget faktiskt ligger. Leverantörstabell med regioner ur `HOSTING-REGIONS.md`. Konkreta förstaåtgärder per incidenttyp. Den gamla tabellen med årlig pentest, kvartalsvis backupåterläsning och halvårsvis tabletop är **borttagen i stället för bevarad** — det finns inget spår av att någon av dem genomförts, och ett oinfriat löfte i en incidentplan är en risk i sig. **Bifynd:** raden här kallade A2 för "boka DPO"; A2 gäller en **AI-jurist**, inte ett dataskyddsombud.
- [x] **KO3 = G14** 🟡 **Delvis klar 2026-09-06 — två av tre orsaker var redan avbetalda, den tredje var en riktig bugg.** **(a) "Varje CTA leder till en route som kräver konto" håller inte längre.** K11:s fix sitter kvar och är verifierad i koden: `App.tsx:162-204` skickar en gäst till `medReturnTo('/login', …)` i stället för att tyst dumpa henne på B2B-sidan, `lib/returnTo.ts` är en riktig allowlist (nekar absoluta och protokollrelativa URL:er, backslash-varianten, `..`, kontrolltecken, dubbelkodning), och `Register.tsx:84,140` skickar användaren dit hon var på väg **efter** att kontot skapats. **(b) Kryssrutorna: två av tre blockerar, och båda är legala krav** (villkor + integritetspolicy). Den tredje, AI-samtycket, var redan `optional().default(false)` i schemat och ingick aldrig i submit-knappens `disabled` — men var otydligt märkt, och har nu en synlig "(Valfritt)"-etikett. **(c) Det som faktiskt var trasigt:** `strongPasswordSchema` har **sju** regler, men bara **fem** visades i UI:t. En gäst kunde få alla fem synliga bockar gröna medan submit förblev blockerad, utan att förstå varför förrän ett Zod-fel dök upp vid blur — de två dolda var "inte samma tecken tre gånger i rad" och "inget vanligt lösenord". Alla sju visas nu live, redan innan man börjat skriva. **Verifierat:** `Register.test.tsx` 14/14 (fyra nya), `register-flow.test.tsx` 8/8, i18n-grindarna gröna. **Mutationstestat:** `disabled`-villkoret muterat så att AI-samtycket åter blockerade ⇒ exakt ett test föll, återställt ⇒ 14/14. **Kvar (ditt beslut):** teckenklasskravet, se KO3b.
...
- **D29/DR4** ✅ Löst 2026-08-23. Orsaken var `assert`-presetet (`lighthouse:recommended` asserterar varje audit på error-nivå), inte `collect` och inte `staticDistDir`. **CI är 8 av 8 grön** — första gången sedan 2 april
...
- [x] **DS10** ✅ **Stängd 2026-09-12 — funktionen finns inte längre i `supabase/functions/`.** A27 (2026-09-01) flyttade den till `archive/2026-09-01-avpublicerade-edge/cv-analysis` tillsammans med tre andra callerlösa AI-funktioner. Fällan (`generateFallbackAnalysis()` som hittar på ett omdöme) ligger kvar i arkivet och ska INTE tas tillbaka utan att den grenen tas bort. *(Var:)* **`cv-analysis`-edgefunktionen är en laddad fälla — läs innan du bygger
...
| **A1** | OpenRouter-nyckeln fortfarande inte roterad (öppen sedan 28 maj) | ✅ **Roterad 2026-09-12** — ny nyckel i Vercel + Supabase, verifierad med riktiga anrop mot prod via båda vägarna (`/api/ai` 200 med brev, `ai-career-assistant` 200 med lönedata). Gammal nyckel raderad hos OpenRouter av Mikael 2026-09-12 ~14:00; båda AI-vägarna verifierade 200 efteråt | S |
...
| ID | Punkt | Storlek |
|----|-------|---------|
| **A22** ✅ *(klar 2026-09-01 — körd och verifierad mot prod)* | **36 av 65 definer-funktioner var anropbara av `anon`** (talet 35 av 53 var 9 augusti — både totalen och den öppna mängden hade vuxit). Migration `20260901100000_a22_definer_grants.sql`. **Utfall mätt med `has_function_privilege`: 36 → 3.** Kvar för anon, var och en med verifierat anropsställe: `check_rate_limit`, `get_invitation_by_token` (A10), `get_shared_profile` (A7). **Klassificerad per funktion, inte som svep — tre mätningar tvingade fram det:** (1) **triggerfunktioner behöver ingen EXECUTE** — Postgres kontrollerar rättigheten när triggern skapas, inte när den fyrar; bevisat i rollad transaktion (efter REVOKE på `log_profile_changes()` gick `UPDATE profiles` igenom som `authenticated` och triggern körde), vilket stängde 9 funktioner helt; (2) fem funktioner står i `pg_policies.qual/with_check` och behåller `authenticated`; (3) **`check_rate_limit` MÅSTE behålla anon** — `api/_utils/rate-limiter.js:19` och `_shared/rateLimit.ts:54` bygger sin klient med anon-nyckeln och **faller tillbaka på en in-memory-limiter vid fel utan att larma**, så ett svep hade tyst degraderat den distribuerade rate-limiten till per-instans-minne för `/api/ai`, cv-pdf, job-alerts, upload-image, bolagsverket och alla sju proxy-guards. **Röktestat över HTTP med skarp anon-nyckel:** `check_rate_limit` → 200 `allowed: true`, `get_shared_profile` → 200 `null`, `export_user_data` → **401 `42501 permission denied`** | — |
...
| **A24** ✅ *(stängd 2026-09-01 — premissen har fallit)* | Sökt igenom hela `sv.json`/`en.json` och sidorna: **inget** "ingen data lämnar EES"-påstående och **ingen** "OpenAI" som biträde finns kvar i levande kod. Kvarvarande EU/EES-omnämnanden är sanna (`hostingDesc`: Supabase lagrar inom EU/EES) eller handlar om något annat (Migrationsverket, legitimation). Enda "OpenAI"-träffen är en kodkommentar i `hooks/useSupabase.ts`, som är onåbar från `main.tsx`. Arbetspass 1 den 9 augusti gjorde jobbet; raden var bara aldrig stängd | — |
| **A25** | `CRON_SECRET` aldrig satt → 503 för alla (Nu-listan) | S (Mikael) |
| **A26** ✅ *(stängd — premissen föll 2026-09-01)* | `interest_results` har i dag **fem** policyer, ingen `ALL`: INSERT och UPDATE bär båda ett `WITH CHECK` (hälsosamtyckets grind), SELECT/DELETE är läs-/raderingsvägar utan check, vilket är rätt. Åtgärdades sannolikt i `20260819_intresseguide_art9_rls.sql`. Mätt med `pg_policies` | — |
| **A27** ✅ *(klar 2026-09-01 — omscopad, premissen var fel form)* | Premissen sa "nio edge-AI-funktioner utan samtyckeskontroll". Mätt: **fem** AI-edge-funktioner saknade grind, och **alla fem var callerlösa** — det var alltså inte en grindfråga utan en dödkodsfråga. Fyra av dem låg **ACTIVE i prod** med `verify_jwt: true` (`ai-assistant` v29, `ai-cover-letter` v36, `ai-cv-writing` v24, `cv-analysis` v27), alltså anropbara av vilket som helst av de 93 kontona förbi AI-brytaren, PII-saneringen, art. 9-grinden och tokentaket. Att grinda kod ingen kör hade upprepat WCAG-svepets fel från 9 augusti. **Åtgärd (beslut Mikael): avpublicerade** med `functions delete`, verifierat mot `functions list`. **Katalogerna är flyttade till `archive/2026-09-01-avpublicerade-edge/` — och det är den varaktiga delen:** `deploy.yml:71` kör `supabase functions deploy` **utan argument** (ingen `--exclude`-flagga finns, `--prune` används inte), så nästa push hade återskapat exakt det som togs bort. Ny regressionsvakt i `ai-sanningsregel.test.ts` kräver att de fyra inte ligger i `supabase/functions/`, plus ett test som håller sanningsregeln vid liv i arkivkopian. **Mutationstestat:** återlagd katalog fäller vakten (1 av 51). `learning-analyze-gap` rördes inte — den hålls i vänteläge av C4 | — |
...
| **A30** ✅ *(klar 2026-09-01 — körd och verifierad mot prod)* | **Premissen var fel, och under den låg en skarp bugg.** Planen sa "registret har noll rader". Fel: `consent_history` hade 53 rader och `grant_consent` HAR grenar för `health_data`/`wellness_data`. **Det verkliga felet:** `authenticated` hade **ingen EXECUTE** på `grant_consent` eller `withdraw_consent` (`proacl = {postgres,service_role}`), så varje `supabase.rpc('grant_consent')` från webbläsaren fick `42501`. A17 revokade dem 4 augusti när de hade noll anropare — korrekt då. Sjutton dagar senare byggdes `consentApi.ts` (MV1) som portalens **enda** väg till samtycken, och den anropar precis dem. Ingen gav tillbaka rättigheten. **Följd i drift: från 2026-08-21 kunde ingen användare ge eller återkalla ett samtycke.** Belägg: nyaste raden i `consent_history` är 2026-07-23, alltså fyra veckor innan "den enda vägen in" byggdes. `consentApi` fail:ar closed och kastar, så felet var inte tyst — men grindarna gick inte att använda. **Åtgärdat i migration `20260901110000`:** (1) EXECUTE återställd till `authenticated` på båda; (2) historikraden härleds nu ur **kolumnövergången** på `profiles` via triggern `log_consent_changes`, så ingen skrivväg kan gå förbi registret — `grant_consent`/`withdraw_consent` slutade därför logga själva (annars dubbelloggning); (3) de 3 saknade raderna backfillade, **märkta i `user_agent` som härledda ur `profiles`, inte observerade** — en backfill som ser ut som en loggad händelse vore samma påhitt som planen finns för att utrota. IP och user agent lämnas NULL: `profiles.consent_ip` sattes vid registreringen och säger inget om ett samtycke två månader senare. **Verifierat i rollade transaktioner:** grant ger exakt 1 rad (ingen dubbellogg), withdraw ger 1, en **förbigången direkt `UPDATE profiles`** ger också 1, och INSERT-grenen (ny registrering) ger 1 rad per satt kolumn. Mätt efter körning: 56 rader, 0 konton utan historik, båda RPC:erna nåbara för `authenticated`. **Grinden utökad i samma commit:** `lint:grants` regel 3 kräver att varje `.rpc()` i `client/src` pekar på en funktion `authenticated` kan köra — den hittar buggen självständigt från kodsidan, och hade fällt den 21 augusti | — |
...
| **A32** ✅ *(klar 2026-09-01 — verifierad före och efter mot prod)* | **Reproducerat i drift:** `curl -X POST https://www.jobin.se/api/ai -H 'Origin: https://deltagarportalen-abc123-evilteam.vercel.app'` fick tillbaka **exakt den origin** i `Access-Control-Allow-Origin`, med credentials-rubriken satt. En helt främmande origin (`elak.example`) föll korrekt tillbaka på produktionsdomänen — det var **namnlikheten** som öppnade. Mönstret `deltagarportal(en)?-<hash>-<vem-som-helst>.vercel.app` fanns i **tre** filer (`ai.js`, `job-alerts.js`, `cv-pdf.js`), och vem som helst kan skapa ett Vercel-projekt med det namnet. **Åtgärd:** mönstermatchningen är borttagen; varje deploy tillåter i stället **sin egen** URL ur Vercels systemvariabler (`VERCEL_URL`, `VERCEL_BRANCH_URL`, `VERCEL_PROJECT_PRODUCTION_URL`), som plattformen sätter per deployment och som ett annat projekt inte kan sätta. Previews fortsätter fungera, gissningsytan är noll. **Credentials-rubriken är också borta ur alla fyra filerna:** portalen autentiserar med `Authorization: Bearer`, inte cookies, och `credentials:` finns inte i någon fetch i `client/src` — rubriken gav ingen funktion, bara den egenskap som gör en reflekterad origin farlig. **12 tester** i `cors-preview.test.ts`, mutationsprövade: återinförd regex fäller ett test, återinförd credentials-rubrik ett annat. Vakten matchar **kodformen** `'Access-Control-Allow-Credentials':`, inte ordet — annars hade den matchat sin egen kommentar, vilket kostade tid i just det här passet | — |
...
| **A34** ✅ *(klar 2026-09-01 — omscopad, premissen stämde bara delvis)* | Planen sa "utan egen samtyckeskategori". Läst i koden: en integritetsnotis **fanns redan** i språkmenyns fot. Tre saker var däremot fel. **(1) Notisen var oläsbar.** Uppmätt kontrast: `text-stone-400` på vitt = **2,52:1**, `dark:text-stone-500` på stone-900 = **3,65:1** — båda under AA:s 4,5:1, och i 11 px. En upplysning om att text lämnar portalen får inte vara det minst läsbara i menyn. Nu `stone-600`/`stone-300` (7,63:1 respektive 11,74:1) i 12 px. **(2) Texten sa "sidans innehåll"** — den som står på Dagboken läser inte nödvändigtvis det som "det jag själv har skrivit". Nu står det uttryckligen, med dagboksinlägget som exempel. **(3) Google stod i integritetspolicyn bara som "opt-in Calendar-integration"** — översättningsfunktionen nämndes inte alls. Nu beskriven i ett eget stycke, medvetet **utanför** biträdeslistan: den rutan säger "vi har skriftliga avtal med alla biträden", och något art. 28-avtal för översättningswidgeten finns inte. Att lägga in den där hade gjort listans egen mening osann. Stycket säger också att portalen fungerar fullt ut på svenska och engelska utan funktionen. *(Kvarstår, dokumentation:)* Google som mottagare hör hemma i Art. 30-registret och DPIA:n — se A4 | — |
...
| **A37** *(nytt 2026-09-01 — mätt, medvetet inte åtgärdat)* | **Edge-funktionernas CORS släpper in `localhost` i produktion, och `DEV_ORIGINS`-mekanismen är därmed död kod.** `_shared/cors.ts` har en `ALLOWED_ORIGINS` som **innehåller** `http://localhost:5173/5174/3000` och `127.0.0.1:5173`, plus en separat `DEV_ORIGINS` som `isOriginAllowed` lägger till **bara** när `ENVIRONMENT=development`. Eftersom samma origins redan står i baslistan gör det villkoret ingenting. **Verifierat mot prod 2026-09-01:** preflight mot `af-jobsearch`, `bolagsverket` och `ai-company-search` med `Origin: http://localhost:5173` ger alla **200 med `Access-Control-Allow-Origin: http://localhost:5173`**. **Inte åtgärdat, med avsikt:** projektet har **en** Supabase-instans, och lokal utveckling på `:3000` anropar prod-edge. Att ta bort localhost hade brutit Mikaels dagliga arbete på ett fynd vars angreppsscenario kräver att offret själv kör angriparens kod lokalt — fel håll att felsäkra åt, samma resonemang som D23-hooken. **Rätt fix är en egen dev-instans**, vilket är ett beslut, inte en kodändring · beslut |
...
- [x] **F31** ✅ **Klar 2026-09-02.** Talet 286 var före nåbarhetsfilter och utan hänsyn till CLDR-pluralformer (`key_one`/`key_other` löses utan grundnyckel). Efter båda korrigeringarna: **159 unika nycklar i 65 filer**, alla insatta via ett position-medvetet skript (`scripts/i18n-infoga-nycklar.cjs`) som inte serialiserar om filen. **Fem `wellness.consent.*`-strängar var en skarp bugg:** `t(key) || fallback` triggar aldrig, så den råa nyckeln renderades i drift. Ny grind `i18n/nycklar-finns.test.ts` (0 saknade). Fälla på vägen: `selectParticipant` bar namnet i `defaultValue`; när nyckeln fick en statisk text försvann per-rad-namnet ur aria-label — rättat med interpolation. *(Var:)* **286 i18n-nycklar i 72 filer finns i varken `sv.json` eller `en.json`.** Koden

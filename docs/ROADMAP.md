...
- [ ] **KS7** **Ett fel ser exakt ut som en tom lista — i alla sju flikar.** Mätt: **43 `console.error`
...
- [ ] **KA3** **Rapportutkastet finns bara i minnet.** `ReportDraftDialog.tsx:37-39` håller `draft` i
...
- [ ] **KK6** **Ingen av de sju flikarna har en testfil.** 4 av 160 testfiler rör vyn. Tre mutationer
...
- [ ] **DOK1** **`docs/GDPR-ART30-REGISTER.md:319` påstår ett skydd som inte finns.** Raden säger
...
- [ ] **DOK4** **Startsidan bär två ospårbara påståenden.** *"Många av våra användare har varit
...
- [ ] **TI1** **Mörkt läge saknar `dark:`-varianter där varningen behöver synas mest.** `ApplicationsPipeline.tsx:502-526` har sex `amber`-klasser utan `dark:`-motsvarighet, medan `Card`-primitiven sätter sin egen mörka botten. Egen beräkning: `text-amber-900` på `bg-stone-900` ger **1,93:1** där AA kräver 4,5:1; `text-amber-700` ger 3,48:1. Just det kortet är "N ansökningar behöver uppföljning". Mörkt läge är inte kosmetik för den här målgruppen — bländningskänslighet är en av de fysiska utmaningar portalen finns till för · ~2–3 h
- [ ] **TI2** **22 av 25 framer-motion-filer respekterar inte `prefers-reduced-motion`.** **Premissrättelse:** granskaren rapporterade "0 av 25" — fel. Tre filer (`OccupationsTab.tsx:169`, `ResultsTab.tsx:179`, `CognitiveTab.tsx:243`) använder redan `<MotionConfig reducedMotion="user">`. Mönstret finns alltså i kodbasen och behöver inte uppfinnas — det behöver lyftas till roten. CSS-regeln i `accessibility.css:100-116` neutraliserar bara CSS-animationer, inte Framer Motions JS-drivna transforms · en `MotionConfig` högst upp i trädet · ~1 h
- [ ] **TI3** **`DropdownMenu` har noll aria-attribut.** Mätt: `grep "aria-"` i `ui/DropdownMenu.tsx` ger inga träffar. Sex användningsställen ärver bristen. En skärmläsaranvändare hör "knapp" — inget om att en meny finns eller om den är öppen. `NotificationBell.tsx:277-289` har mönstret rätt, så fixen är att flytta det till primitiven · ~1 h
...
- [ ] **JD3** **`lint:grants` finns inte.** A36 föreslog en grind som läser `has_function_privilege` per definer-funktion och `relrowsecurity` per tabell. Mätt: inget sådant skript i `client/package.json`. Dubblettpolicy- och grant-mönstret har nu träffat fem gånger; `lint:schema` föddes ur exakt samma insikt 27 juli · ~3 h
- [ ] **CB3** **Fokuslägets CV-wizard avancerar utan att vänta på sparningen.** `FocusCVBuilder.tsx:111-114`: `handleSave()` är en fire-and-forget `mutate`, och `setCurrentStep(prev => prev + 1)` körs på nästa rad. Misslyckas sparningen visas bara en toast medan deltagaren redan är på nästa steg. Fokusläget är den väg portalen själv pekar ut för användare med minst marginal, och har svagast dataskydd av hela CV-ytan · ~2–3 h
- [ ] **CB4** **`JobAdaptPage` rullar aldrig tillbaka en misslyckad optimistisk uppdatering.** `handleAddSkill` (`:35-58`) och `handleUpdateSummary` (`:60-72`) sätter state direkt, anropar `cvApi.updateCV`, och visar vid fel bara en toast — ändringen ligger kvar i UI:t som om den sparats. Nästa laddning är den borta · ~1 h
- [ ] **CB5** **Kompetensformen hanteras defensivt på ett ställe och naivt på fyra.** `cvOptimizer.ts:229` har `skillText()` som tål både sträng och objekt — den lagade platsen. `CVOptimizer.tsx:41`, `JobAdaptPanel.tsx:57` och `ai.js:1009,1082` gör `.map(s => s.name)` rakt av, vilket ger ordet `undefined` i AI-prompten för gamla poster i strängform. Och `unifiedProfileApi.ts:27` typar fortfarande `skills: string[]` trots att prod har objekt — samma typlögn som orsakade produktionskraschen i UX14 · ~1 h
- [ ] **TI4** **Sidoflikarna i skenan annonserar inte att innehållet bytts.** De fem sidorna med `sidoflikar` (LinkedIn, Dagbok, Externa resurser, Profil, Resurser) byter state utan ruttbyte, så `RouteAnnouncer` fångar det inte. Ingen `aria-live` säger vad som hände · ~2 h
- [ ] **TI5** **`Progress` saknar `role="progressbar"` och `aria-valuenow`.** Rena `<div>`-staplar med bredd i `style` — osynliga i tillgänglighetsträdet · ~30 min
- [ ] **TI6** **`offsetParent`-shimmen är duplicerad i tre testfiler i stället för att ligga i `setup.ts`.** Utan den ser `useFocusTrap` noll fokuserbara element i jsdom, så ett fokustest kan gå grönt fast fokushanteringen är trasig. Tre filer bär samma ~15 rader med samma kommentar. 12 av de 17 komponenter som använder `useFocusTrap` saknar testfil helt — luckan är i skyddsnätet, inte i produktionskoden · ~1 h
- [ ] **KV6** **"Loggade in" är `profiles.updated_at`.** Vyn gör `p.updated_at AS last_login` (`20260710120000_consultant_participant_tags.sql:44`), och etiketten visas på fyra ställen. Konsulenten läser "loggade in 16:48" om något som kan ha varit en bakgrundsskrivning · byt etikett (S) eller exponera `auth.users.last_sign_in_at` (~2–4 h)
- [ ] **KV7** **Notisinställningarna sparas men levereras aldrig.** `SettingsTab.tsx:190-263` skriver `consultant_settings.notifications`, bl.a. "larma när en deltagare inte loggat in på 7 dagar". Ingen cron, ingen edge-funktion, inget i `vercel.json` prenumererar. Samma sida märker redan teamhanteringen ärligt som kommande — gör likadant här, eller bygg leveransen · S (märk) / L (bygg)
- [ ] **SA5** **Yrkesdatabasen och flyttguidens regiontabell saknar källa och datum.** `interestGuideData.ts` ger ~180 yrken en "Växande/Stabil/Krympande"-prognos och ett lönespann, hårdkodat, filtrerbart och hopräknat till "X växande jobb" — som om det vore Yrkeskompassen. `RelocationTab.tsx:18-31` har snitthyra, snittlön och bostadskötid för tolv städer, presenterat som beslutsunderlag för en flytt. Ingendera bär källa eller årtal. Det är samma format som lönegolvet i `/international`, som var fel i tre år utan att någon kunde se det · källrad + datum: ~30 min; saklig granskning: större
- [ ] **SA6** **Matchningsgolvet döljer svaga träffar.** `interestJobMatching.ts:213-216` gör `Math.max(30, …)` så fort en typ matchar. En annons som nätt och jämnt nämner ett ord visas aldrig under "30 % matchning" · ta bort golvet, eller byt till låg/medel/hög · ~15 min
- [ ] **BL2** **Registreringstestet asserterar inte rollen.** Mutationsstickprov: `Register.tsx:71` ändrad från `role: 'USER'` till `'CONSULTANT'` → 29 tester gröna. `expect.objectContaining` i `register-flow.test.tsx:172-177` utelämnar fältet. Ingen levande lucka — `handle_new_user()` läser aldrig klientens metadata — men inget skydd mot att det ändras · ~15 min

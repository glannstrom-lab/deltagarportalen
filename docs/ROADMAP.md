...
- [ ] **KS3** **Den enda levande inbjudningsvägen har ingen samtyckesruta.** `InviteHandler.tsx:110`
  beräknar `consentOk = !isStaInvite || (…)` — rutorna visas bara för STA-inbjudningar, och **STA är
  avstängd sedan 3 augusti**. Triggern gör samma sak från andra hållet: `profiles.consultant_id` sätts
  ovillkorligt medan `consultant_consents` bara skrivs för två namngivna program
  (`20260524_sta_link_status_truth.sql:72-99`). **Mätt i prod: 18 av 31 kopplingar har samtycke, alla
...
| **A30** 🔵 *(premissen omskriven 2026-09-01 — mindre men skarpare)* | Planen sa att registret har noll rader. **Fel:** `consent_history` har 53 rader (terms 18, privacy 18, ai_processing 16, marketing 1), och `grant_consent` **har** grenar för `health_data` och `wellness_data`. Det som faktiskt saknas: **två konton bär art. 9-samtycken direkt på `profiles`** (`wellness_consent_at`, `health_consent_at`) **med noll rader i `consent_history`** — tidsstämpeln finns, revisionsspåret inte. Ett av de två är testkontot `5b0904ac-…`. Skrivvägen som satte dem gick förbi `grant_consent`; `consentApi.ts:33` varnar redan för exakt det. **Föreslagen åtgärd:** en trigger som härleder historikraden ur kolumnövergången (NULL → värde = `granted`, värde → NULL = `withdrawn`), så att ingen skrivväg kan gå förbi registret — samma grepp som `REGELVERKSREGEL` på sammansättningsstället. Kräver att `grant_consent`/`withdraw_consent` slutar logga själva, annars dubbelloggas varje samtycke | S |

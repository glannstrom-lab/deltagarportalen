...
- [x] **SK1** ✅ **Roterat 2026-09-12** — båda Playwright-kontonas lösenord (`claude-playwright-test@jobin.se`, `claude-playwright-consultant@jobin.test`) bytta via Supabase admin-API; de nya ligger bara i den gitignorerade `.env.test.local` (`TEST_LEGACY_*`). Det läckta lösenordet är därmed ogiltigt. **Kvar för Mikael:** GitHub-secreten `TEST_USER_PASSWORD` (CI:s E2E Authenticated) pekar på det gamla och måste uppdateras — eller pekas om till km-kontona. *(Var:)* Testkontots lösenord ligger i klartext i det **publika** repots historik (9 commits) och är **fortfarande giltigt mot prod** — inloggning gav HTTP 200. Commit `4d060a16` tog bort det framåt men roterade det aldrig · rotera nu, ~15 min

...
- [x] **KG3** ✅ **Klar 2026-09-12** — `scripts/lib/og-bild.cjs` mappar kategori/sidtyp → `/og/<fil>.png` (15 bilder à 25–35 kB, genererade av `scripts/og-bilder.cjs` med Playwright, hubbfärg per kategori, inga gradients); de sju ställena i mallen bytta; grind `src/test/guides-og-bild.test.ts` (varje kategori får egen bild, okänd → fallback). *(Var:)* **Alla 269 prerenderade sidor delar samma `og:image`** — hårdkodat på sju

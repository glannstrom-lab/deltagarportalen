...
| B9 | **AI-brevet hittar på meriter** | ⬜ Nytt fynd 2026-07-23 (UX-test, "Jonas"). `personligt-brev` skrev in overifierad CRM-/avtalsförhandlingserfarenhet som användaren aldrig angav — regression mot ärlighetsarbetet. Skärp prompten (hitta ALDRIG på erfarenheter — bara det underlaget stödjer). Även: AI:ns egen "Med vänliga hälsningar" + mallens signatur = dubblerad avslutning. **B9-prio: hög** |
...
| UX2 | **"Skapa ansökan" i jobbannons kraschar** | ⬜ Nytt 2026-07-23, KRITISKT. `TypeError: be.update is not a function` i JobSearch createApplication — exakt den latenta buggen E11-utredningen förutsåg (workflowApi/`cloudStorage.savedJobsApi` saknar `.update()`). Nu prod-verifierad på primära konverteringsvägen. **Ingår i E12-konsolideringen** — men bör snabbfixas separat (lägg till `.update()` eller styr om till applicationsApi) |
| UX3 | Intervjusimulatorn tappar all data utan varning | ⬜ Nytt 2026-07-23. "Avsluta"/sidladdning nollställer React-state utan spara/bekräfta/historik (`InterviewSimulator.tsx:361-376`). Lägg bekräftelsedialog + spara session (Supabase) — schemat `IntervjuSimulatorResultSchema` finns redan (jfr G11) |
| UX4 | Råa i18n-nycklar + otextad text i prod | ⬜ Nytt 2026-07-23. ICF-blocket i Intresseguidens resultat renderar `health.consent.*`-nycklar rått (trasigt hälsodata-samtycke!); "Failed to fetch" vid nätfel i kompetensgap; hub-namnen i sidopanelen översätts ej till engelska (känt). Störst: ICF-blocket |
| UX5 | Snabb-CV falsk framgång + Big Five platshållare | ⬜ Nytt 2026-07-23. Snabb-CV-wizarden visar "Ditt CV är skapat!" utan att spara (Dina CV = 0); Big Five ger identiska 63 % på alla 5 drag oavsett svar (beräknings-/platshållarbugg). Undergräver förtroende |
...
| UX7 | Schema-fel `share_health_data_with_consultant` | ⬜ Nytt 2026-07-23. 400/42703 varje gång Integritet-sidan öppnas — kolumn saknas i `participant_data_sharing` (migrationslucka). Dela-hälsodata-togglarna fungerar troligen inte. Även: Wellness samlar hälsodata utan samma samtyckesgrind som Dagbok har |
...
| E12 | **Konsolidera saved_jobs-vägarna** | ⬜ Ny 2026-07-23 (E11-utredningen). Standardisera på `applicationsApi` (rikaste schemat, Kanban-UI:t); pensionera cloudStorage.savedJobsApi, jobsApi.savedJobsApi och job_applications-stacken (applicationService/workflowApi — som har en latent `.update()`-bugg och redan tyst degraderade fält); migrera ~7 konsumenter; därefter beslut om job_applications-tabellen (M–L) |

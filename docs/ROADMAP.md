...
- [ ] **DR4** Lighthouse CI — **omscopad 2026-08-22. Premissen höll inte.**

  Raden och `CLAUDE.md` påstod att `--collect.staticDistDir` som CLI-flagga fick lhci att ignorera `.lighthouserc.js`s `url:`-lista och bara auditera `index.html`. **Uppmätt genom att köra exakt CI:s flaggsats lokalt: det stämmer inte.** Healthchecken skriver `✅ Configuration file found`, och körningen startar på listans *första* URL — inte på `guider/a-kassa.../index.html`, som autodiscovery i bokstavsordning hade gett. Rc-filen läses och gäller.

  **Orsaken är fortfarande okänd, och det ska stå så.** Jobbet faller med enda spåret "No files were found with the provided path: .lighthouseci", alltså att `collect` dog innan något skrevs. Det gick inte att reproducera lokalt: på Windows faller körningen på ett `EPERM` när chrome-launcher städar sin temp-katalog — ett plattformsfel som inte finns på Ubuntu. Rapporten som ändå producerades hade `runtimeError: false` och inga `runWarnings`, så själva mätningen fungerar. `@lhci/cli` saknar try/catch i sin collect-loop: vilken som helst nollskild exitkod från lighthouse-barnprocessen, **även ett städningsfel efter en lyckad mätning**, ger exakt det här symptomet. Det förklarar varför det tagit fem månader utan att någon kommit närmare.

  **Gjort 2026-08-22** (Mikaels ja: "fixa allt"): all konfiguration flyttad från tio CLI-flaggor till `.lighthouserc.js`, så det finns *en* sanning i stället för två som skuggar varandra. `chromeFlags` med `--no-sandbox --disable-dev-shm-usage --disable-gpu` tillagda — GitHub-runnerns `/dev/shm` är 64 MB och Chrome kraschar under minnestryck utan flaggan; det är den mest sannolika Ubuntu-orsaken men **en hypotes, inte en mätning**. Steget kör dessutom `lhci healthcheck` först och skriver ut innehållet i `.lighthouseci/` innan det faller, så nästa körning lämnar något att läsa.

  **Kvarstår:** verifiera mot en riktig körning. Blir jobbet inte grönt är nästa steg att läsa den nya diagnostiken — inte att gissa igen.
...
- [x] **DR6** ✅ *(klar 2026-08-22)* **`Security Scan` i CI var röd i minst åtta körningar i rad** på tre högallvarliga fynd i `extract-zip` → `@puppeteer/browsers` → `puppeteer-core` (GHSA-jmr9-qjv8-65gv, ovaliderad symlänk-traversering vid uppackning).
  ...
  **Gjort och verifierat:**
  ...
  Den gamla rutans bedömning att brådskan var låg stod fast: `cv-pdf.js` skickar alltid en explicit `executablePath`, så `@puppeteer/browsers`' nedladdare — den enda anroparen av `extract-zip` — kördes aldrig. Det var ett argument för att inte hasta, inte för att låta grinden stå röd.
...
- **D29/DR4** Lighthouse failar fortfarande. Den "fastställda rotorsaken" (`--collect.staticDistDir` skulle kringgå `.lighthouserc.js`) **mättes bort 2026-08-22** — rc-filen läses och gäller. Konfigurationen är konsoliderad och steget diagnostiserar sig självt nu; orsaken är öppen tills en riktig körning säger något

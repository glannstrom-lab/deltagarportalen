...
- [x] **PG5** ✅ byggt 2026-09-12 (F1): "Jag kan inte komma" på kommande pass (orsak + rad till konsulenten), notis till konsulenten via definer-trigger, "Anmäld frånvaro" i konsulentens PassRad; migration `20260913002000_f1_franvaroanmalan.sql` (tre nullable kolumner, guarden utvidgad, notis-trigger) bevisad i rollback (`e2e/f1-franvaro-prov.sql`) — **väntar på Mikaels ja**; tills dess ger knappen ärligt felet "gick inte att spara". Ångra ger ingen ny notis. *(Var:)* Aktivitetskravet utan väg för deltagaren att anmäla frånvaro: "Frånvaro" på passet utan orsak, utan knapp; sjukanmälan bara via Min konsulent. Oanmäld frånvaro kan påverka försörjningsstödet. Se förslag F1 · M
...
- [ ] **PG19** 📝 2026-09-12 — UI byggt (`AiBrytare` i Din organisation, bara chef/admin, bekräftelse med sann beskrivning ur `checkOrgAiEnabled`, `orgApi.setOrgAiEnabled` kastar vid 0 rader); migration `20260913001000_pg19_org_ai_brytare_chef.sql` (UPDATE-policy + guard-trigger som bara släpper `ai_enabled`) bevisad i rollback (`e2e/pg19-ai-brytare-prov.sql`) — **väntar på Mikaels ja**; förrän dess ger knappen ärligt fel. *(Var:)* Chefen har ingen UI för organisationens AI-brytare (kräver SQL) — bekräftar uppdragets "självservice för AI-brytaren" · S

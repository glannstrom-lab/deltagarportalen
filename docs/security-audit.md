...
- ✅ **BL6 (2026-09-12): backend rapporterar fel till Sentry.** `supabase/functions/_shared/sentry.ts` och `client/api/_utils/sentry.js` wrappar alla 18 edge-funktioner och `cv-pdf`/`job-alerts`/`upload-image` (`ai.js` och `send-invite-email` väntar på sina ägare): oväntade fel → 500 JSON + sanerad händelse; svar ≥ 500 rapporteras också. Saneringen maskerar e-post, personnummer, Bearer/JWT/API-nycklar; aldrig body eller headers; mutationstestad i `src/test/backendSentry.test.ts`. **No-op tills `SENTRY_DSN` sätts** i Vercel-env och Supabase-secrets — och `VITE_SENTRY_DSN` saknas också i Vercel, så inte heller klienten rapporterar i prod i dag.

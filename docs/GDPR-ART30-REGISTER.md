...
| Säkerhetsåtgärder | **Rättat 2026-09-02 (DOK1, efter KS2).** Migrationen `supabase/migrations/20260831140000_ks_consultant_rls.sql` är **körd och verifierad mot prod** (roadmapens KS2, klar 2026-08-31; kontrollerad på nytt här 2026-09-02 med `SELECT policyname, cmd, qual FROM pg_policies WHERE tablename IN ('consultant_journal','consultant_goals','consultant_messages')` — alla fem policyer står som migrationen skrev dem). `consultant_journal` och `consultant_goals` har nu en `ALL`-policy som kräver `EXISTS` mot en **aktiv** rad i `consultant_participants`, så en konsulent vars koppling brutits av `revoke_consultant_link()` (eller av deltagaren) tappar läs- och skrivrätten omedelbart — inklusive fritextkategorin "Oro". `consultant_journal` har dessutom fått en egen SELECT-policy för deltagaren (art. 15 hade tidigare ingen teknisk väg för just den tabellen), och `consultant_messages` kräver nu samma aktiva-relation-kontroll vid INSERT. **Det raden inte får påstå: att detta "utlöser gallring".** Åtkomsten upphör — datan gör det inte. Ingen kod raderar eller arkiverar journalrader eller mål när relationen bryts (kontrollerat 2026-09-02: `grep -rl consultant_journal supabase/functions` gav noll träffar, `vercel.json` har en enda cron och den gäller jobbevakningar, och ingen databasfunktion heter något i stil med gallring/retention/purge/cleanup för dessa tabeller). Effekten är alternativ **(c)** i migrationens egen kommentar: raderna låses kvar, oläsbara för alla parter, tills vidare. Öppet produktbeslut (ej avgjort): arkivera dem åt en ny konsulent (a), radera dem (b), eller låta (c) stå som permanent policy — se migrationsfilens huvud för avvägningen |

 ...
 *
 *   BASE_URL=https://www.jobin.se node e2e/svep-kunskapsbank.cjs [bredd] [--mork]
 *
...
;(async () => {
  const env = laddaEnv()
  const utDir = path.join(__dirname, 'screenshots', `kunskapsbank-${bredd}${mork ? '-mork' : ''}`)
  ...
  async function besok(namn, rutt, vantetid = 3500) {
    ...
    const m = await page.evaluate(() => {
      ...
      return {
        ...
        rubriker: [...document.querySelectorAll('main h1, main h2, main h3')].map(h => h.tagName + ':' + h.textContent.trim().slice(0, 45)).slice(0, 25),
        skena: skena ? Math.round(skena.getBoundingClientRect().width) : null,
        hscroll: window.innerWidth < document.documentElement.scrollWidth,
        text: main ? main.innerText.slice(0, 1200) : null,
        lankar: [...document.querySelectorAll('main a')].map(a => a.getAttribute('href')).filter(Boolean).slice(0, 30),
      }
    })
    logg[namn] = m
  ...
  await besok('01-landning', '/knowledge-base')
  await besok('02-kategori-tomt', '/knowledge-base?category=easy-swedish')
  await besok('03-kategori-jobbsokning', '/knowledge-base?category=job-search')
  await besok('04-sok-lonefohandling', '/knowledge-base?q=' + encodeURIComponent('Löneförhandling'))
  await besok('05-sok-utan-traffar', '/knowledge-base?q=zzzqqqxyz')

  // Första artikellänken från kategorivyn
  const artikelLank = (logg['03-kategori-jobbsokning'].lankar || []).find((h) => h.includes('article'))
  if (artikelLank) {
    const rutt = artikelLank.replace(/^#/, '')
    await besok('06-artikel', rutt.startsWith('/') ? rutt : '/' + rutt, 4000)
  } else {
    console.log('ingen artikellänk hittad i kategorivyn')
    logg.ingenArtikellank = true
  }

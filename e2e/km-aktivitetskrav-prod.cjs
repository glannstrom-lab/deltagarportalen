...
;(async () => {
  ...
  try {
    ...
    // Knappen heter "Ta bort <namn>" (aria-label); bekräftelsen i portalens dialog heter "Ta bort".
    const taBort = k.getByRole('list', { name: /Kollegor i/ }).getByRole('button', { name: /^Ta bort / }).first()
    await taBort.evaluate((el) => el.scrollIntoView({ block: 'center' })); await taBort.click(); await k.waitForTimeout(600)
    const ja = k.getByRole('dialog').getByRole('button', { name: /^Ta bort$/ }); if (await ja.isVisible({ timeout: 3000 }).catch(() => false)) await ja.click()

    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className={cn(
            'fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-r from-violet-600 to-purple-600 rounded-full shadow-lg shadow-violet-200 flex items-center justify-center text-white hover:scale-110 transition-transform',
            className
          )}
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-96 h-[600px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4">
          {/* Header */}
            ...
            {messages.map((message) => (
              ...
              >
                <div className={cn(
                  ...
                  message.role === 'assistant' ? 'bg-violet-100' : 'bg-slate-100'
                )}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {message.suggestions.map((suggestion) => (
                        <button
                          ...
                          className="px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs text-slate-600 hover:border-violet-300 hover:text-violet-600 transition-colors"
                        >
                          {suggestion}
                        </button>
                      ))}
          ...
          <div className="p-4 border-t border-slate-100">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm"
              />
              <button
                onClick={handleSend}

      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-stone-800">Mest efterfrågade yrken</h3>
          </div>
            ...
            {popularOccupations.map((occupation, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-stone-800">{occupation.term}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-sm text-stone-700">{occupation.count} jobb</span>
                  {occupation.trend === 'up' && (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-stone-800">Trendande kompetenser</h3>
          </div>
            ...
            {trendingSkills.map((skill) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-stone-800">{skill.skill}</span>
                  <div className="flex items-center gap-2">
      ...
      {marketStats?.by_region && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-stone-800">Jobb per region</h3>
          </div>
            ...
            {marketStats.by_region.map((region) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium text-stone-800">{region.region}</span>
                  <span className="text-sm text-stone-700">{region.job_count} jobb</span>
                </div>
...
function StatCard({ icon: Icon, label, value, subtext, color }: StatCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-stone-200">
      ...
      <p className="text-xs text-stone-600 mt-1">{subtext}</p>
    </div>
  );
}

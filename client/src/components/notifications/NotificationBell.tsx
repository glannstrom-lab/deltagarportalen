    ...
    <div
      className={cn(
        ...
        notification.read
          ...
          : 'bg-brand-50/50 dark:bg-brand-900/20 hover:bg-brand-100/50 dark:hover:bg-brand-900/30'
      )}
      onClick={handleClick}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          {!notification.read && (
            <span className="flex-shrink-0 w-2 h-2 mt-1.5 bg-brand-700 rounded-full" aria-label="Oläst" />
          )}
        </div>
      ...
      >
        {!notification.read && (
          ...
          >
            <Check className="w-3 h-3 text-brand-900" />
          </button>
        )}
        <button
    ...
    <div className={cn('relative', className)}>
      ...
      <button
        ...
        className={cn(
          ...
          variant === 'compact'
            ? 'w-10 h-10 rounded-xl bg-emerald-100/80 hover:bg-emerald-200/90 dark:bg-emerald-900/40 dark:hover:bg-emerald-800/50 hover:'
      ...
      {unreadCount > 0 && (
        <span className="absolute -top-0.5 -right-0.5 flex h-5 w-5 pointer-events-none">
          ...
          <span className="relative inline-flex rounded-full h-5 w-5 bg-emerald-500 items-center justify-center text-[10px] font-bold text-white">
          ...
          >
            ...
            {notifications.length > 0 && (
              <div className="px-4 py-2 border-t border-stone-100 dark:border-stone-700">
                <Link
                  to="/settings"
                  className="block text-center text-xs text-stone-500 dark:text-stone-600 hover:text-brand-900 dark:hover:text-brand-400"
                  onClick={handleClose}
                >

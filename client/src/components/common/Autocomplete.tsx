    ...
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          disabled={disabled}
          className="w-full pl-10 pr-10 py-2.5 border border-stone-200 rounded-xl 
                     focus:outline-none focus:ring-2 focus:ring-[#4f46e5] focus:border-transparent
                     disabled:bg-stone-100 disabled:text-stone-700
                     transition-all"
          autoComplete="off"
        />
      ...
      {isOpen && (
        <div
          className="absolute z-50 w-full mt-1 bg-white rounded-xl shadow-lg 
                     border border-stone-200 max-h-80 overflow-auto"
        >
          {Object.entries(groupedSuggestions).map(([category, items]) => (
            <div key={category}>
              {showCategories && category !== '' && (
                <div className="px-3 py-1.5 text-xs font-semibold text-stone-700 bg-stone-50 uppercase tracking-wider">
                  {category}
                </div>
              )}
                  ...
                  >
                    ...
                    {suggestion.description && (
                      <div className={`text-xs mt-0.5 truncate
                        ${isHighlighted ? 'text-white/80' : 'text-stone-700'}`}>
                        {suggestion.description}
                      </div>
                    )}
                  </button>
          ...
          {suggestions.length === 0 && inputValue.length >= minLength && !isLoading && (
            <div className="px-3 py-4 text-sm text-stone-700 text-center">

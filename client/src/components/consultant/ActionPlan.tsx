...
const statusStyles: Record<GoalStatus, { color: string; icon: typeof CheckCircle2 }> = {
  NOT_STARTED: { color: 'text-stone-700 dark:text-stone-300', icon: Clock },
...
export function ActionPlan({
  ...
  return (
    <div className={cn('bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-700', className)}>
      ...
      <div className="p-6 border-b border-stone-100 dark:border-stone-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-stone-800 dark:text-stone-100">{t('consultant.actionPlan.title')}</h3>
              <p className="text-sm text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.subtitle', { name: participantName })}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="bg-stone-50 dark:bg-stone-800 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.stats.total')}</p>
          </div>
      ...
      {/* Templates */}
      {showTemplates && (
        <div className="p-4 bg-stone-50 dark:bg-stone-800 border-b border-stone-100 dark:border-stone-700">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-medium text-stone-700 dark:text-stone-200 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-amber-500 dark:text-amber-400" />
              {t('consultant.actionPlan.selectTemplate')}
            </h4>
            <button
              onClick={() => setShowTemplates(false)}
              className="text-sm text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-stone-100"
            >
              {t('common.close')}
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {goalTemplates.map((template, i) => (
              <button
                ...
                className="text-left p-4 bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-sm transition-all"
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-stone-600 dark:text-stone-400">{template.timeBound}</span>
                </div>
                <p className="font-medium text-stone-800 dark:text-stone-100">{template.title}</p>
                <p className="text-sm text-stone-700 dark:text-stone-300 mt-1">{template.specific}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Add/Edit form */}
      {(isAdding || editingId) && (
        <div className="p-6 bg-stone-50 dark:bg-stone-800 border-b border-stone-100 dark:border-stone-700">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-stone-200 dark:border-stone-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-200 mb-1">
            ...
            <div className="flex gap-4">
              <div>
                ...
                <select
                  ...
                  className="px-3 py-2 border border-stone-200 dark:border-stone-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100"
                >
      ...
      <div className="max-h-[600px] overflow-y-auto">
        {goals.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.noGoals')}</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h4 className={cn(
                          ...
                          goal.status === 'COMPLETED' && 'line-through text-stone-700 dark:text-stone-400'
                        )}>
                          {goal.title}
                        </h4>
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        <div className="flex-1 h-2 bg-stone-100 dark:bg-stone-700 rounded-full max-w-[150px]">
                          <div
                            className={cn(
                        ...
                        <span className="text-xs text-stone-700 dark:text-stone-300">{goal.progress}%</span>
                      ...
                      {/* Expandable details */}
                      {isExpanded && (
                        <div id={`goal-${goal.id}-details`} role="region" aria-label={t('consultant.actionPlan.smartCriteriaFor', { title: goal.title })} className="mt-3 p-3 bg-stone-50 dark:bg-stone-800 rounded-lg space-y-2 animate-in fade-in">
                          <div>
                            <span className="text-xs font-medium text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.details.measurable')}:</span>
                            <p className="text-sm text-stone-700 dark:text-stone-300">{goal.measurable}</p>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <span className="text-xs font-medium text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.details.achievable')}:</span>
                              <p className="text-sm text-stone-700 dark:text-stone-300">{goal.achievable}</p>
                            </div>
                            <div>
                              <span className="text-xs font-medium text-stone-700 dark:text-stone-300">{t('consultant.actionPlan.details.relevant')}:</span>
                              <p className="text-sm text-stone-700 dark:text-stone-300">{goal.relevant}</p>
                            </div>
                          </div>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 text-stone-600 dark:text-stone-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors"
                        title={t('common.edit')}
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-stone-600 dark:text-stone-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors"

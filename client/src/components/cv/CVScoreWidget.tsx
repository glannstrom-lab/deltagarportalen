      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-stone-800">CV-poäng</p>
              <p className="text-sm text-stone-700">{total}/{max} poäng</p>
            </div>
          </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
      ...
      <div 
        ...
        className={`p-4 ${isCollapsed ? 'cursor-pointer hover:bg-stone-50' : ''}`}
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-stone-800">CV-poäng</h3>
              <p className="text-sm text-stone-700">{total}/{max} poäng</p>
            </div>
          </div>
          {isCollapsed && <ChevronUp className="w-5 h-5 text-stone-600" />}
        </div>
      ...
      <div className="border-t border-stone-100">
        {sections.map((section) => (
          <div key={section.name} className="p-3 border-b border-stone-50 last:border-b-0">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-stone-700">{section.name}</span>
              <span className={`text-sm font-semibold ${
                section.score === section.max ? 'text-green-600' : 'text-stone-700'
              }`}>
            ...
            {section.tips.length > 0 && (
              <div className="space-y-1">
                {section.tips.map((tip, i) => (
                  <p key={i} className="text-xs text-stone-700 flex items-start gap-1.5">
                    {section.score === section.max ? (

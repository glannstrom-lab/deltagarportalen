...
export function VideoCall({ isOpen, onClose, participantName, isIncoming = false }: VideoCallProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 bg-stone-800/50">
        <div>
      ...
      <div className="flex-1 relative bg-stone-950 flex items-center justify-center">
        {!isConnected && (
          <div className="text-center">
            <div className="w-24 h-24 rounded-full bg-stone-700 flex items-center justify-center mx-auto mb-4 text-white text-4xl">
              {participantName[0]}
            </div>
            <p className="text-white text-lg">{isIncoming ? 'Inkommande samtal...' : 'Ansluter...'}</p>
          </div>
        )}
        <div className="absolute bottom-4 right-4 w-48 h-36 bg-stone-800 rounded-xl overflow-hidden">
          {isVideoOff ? <div className="w-full h-full flex items-center justify-center"><VideoOff size={32} className="text-stone-600" /></div> : <div className="w-full h-full bg-stone-700" />}
        </div>
      </div>

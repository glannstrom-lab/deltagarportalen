...
export function PageCard({ children, className, hover = true }: PageCardProps) {
  return (
    <div 
      className={cn(
        "bg-white dark:bg-stone-900 rounded-2xl border-2 border-stone-200 dark:border-stone-700 p-5 sm:p-6",
        hover && "hover:border-stone-300 dark:hover:border-stone-600 hover:shadow-xl hover:-translate-y-1",
...
export function PageCardLink({ children, to, className }: PageCardLinkProps) {
  return (
    <Link 
      to={to}
      className={cn(
        "block bg-white dark:bg-stone-900 rounded-2xl border-2 border-stone-200 dark:border-stone-700 p-5 sm:p-6",
        "hover:border-[var(--c-accent)] dark:hover:border-[var(--c-solid)] hover:shadow-xl hover:-translate-y-1",
        "transition-all duration-300",
        className
      )}
    >

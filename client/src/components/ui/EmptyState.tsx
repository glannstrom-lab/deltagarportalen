  ...
  return (
    <div className={cn(
      ...
      <div className="w-16 h-16 bg-stone-100 dark:bg-stone-800 rounded-full flex items-center justify-center mb-4">
        <svg className="w-8 h-8 text-stone-600 dark:text-stone-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <p className="text-sm text-stone-700 dark:text-stone-300 mt-2 max-w-xs">
          ...
          <div className="flex flex-wrap gap-2">
            {suggestions.map((suggestion) => (
              <button
                key={suggestion}
                onClick={() => onClear()}
                className="px-3 py-1.5 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-full text-sm text-stone-600 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800 hover:border-stone-300 dark:hover:border-stone-600 transition-colors"
              >
                {suggestion}
              </button>
            ))}
...
export function EmptyWidget({
  ...
  return (
    <div className={cn(
      'flex flex-col items-center justify-center',
      'text-center py-8 px-4',
      'bg-stone-50 dark:bg-stone-800 rounded-xl border border-dashed border-stone-300 dark:border-stone-600',
      className
    )}>
      <p className="text-sm font-medium text-stone-700 dark:text-stone-300">{title}</p>
      <p className="text-xs text-stone-700 dark:text-stone-300 mt-1">{description}</p>
      {action && (
        <button
          onClick={handleClick}
...
export function IllustratedEmptyState({
  ...
  const illustrations = {
    documents: (
      <svg viewBox="0 0 120 120" className="w-32 h-32 text-stone-300" fill="currentColor">
        <rect x="20" y="20" width="40" height="50" rx="4" className="text-stone-200" />
        <rect x="35" y="35" width="70" height="50" rx="4" className="text-stone-300" />
        <rect x="45" y="50" width="40" height="4" rx="2" className="text-stone-600 dark:text-stone-300" />
        <rect x="45" y="60" width="30" height="4" rx="2" className="text-stone-600 dark:text-stone-300" />
        <rect x="45" y="70" width="35" height="4" rx="2" className="text-stone-600 dark:text-stone-300" />
      </svg>
    ),
    search: (
      <svg viewBox="0 0 120 120" className="w-32 h-32 text-stone-300" fill="currentColor">
        <circle cx="55" cy="55" r="25" className="text-stone-200" stroke="currentColor" strokeWidth="4" fill="none" />
        <line x1="75" y1="75" x2="95" y2="95" stroke="currentColor" strokeWidth="4" strokeLinecap="round" />
        <circle cx="55" cy="55" r="35" className="text-stone-100" opacity="0.5" />
      </svg>
    ),
    notifications: (
      <svg viewBox="0 0 120 120" className="w-32 h-32 text-stone-300" fill="currentColor">
        <path d="M60 20c-16.6 0-30 13.4-30 30v20l-10 10h80l-10-10V50c0-16.6-13.4-30-30-30z" className="text-stone-200" />
        <circle cx="60" cy="95" r="8" className="text-stone-200" />
        <circle cx="60" cy="55" r="30" className="text-stone-100" opacity="0.5" />
      </svg>
    ),
    success: (
      <svg viewBox="0 0 120 120" className="w-32 h-32 text-stone-300" fill="currentColor">
  ...
  return (
    <div className={cn(
      ...
      <h3 className="text-lg font-semibold text-stone-800 dark:text-stone-100">{title}</h3>
      {description && (
        <p className="text-sm text-stone-700 dark:text-stone-300 mt-2 max-w-sm">
          {description}
        </p>
      )}
      {action && (

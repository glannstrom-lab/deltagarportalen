...
const categoryColors: Record<WidgetColor, { bg: string; text: string }> = {
  teal: { bg: 'bg-[var(--c-bg)]', text: 'text-[var(--c-text)]' },
    ...
    <Card className="p-3 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center justify-between mb-1.5">
        <Link to={to} className="flex items-center gap-1.5 group min-w-0 flex-1">
          ...
          <h3 className="font-medium text-stone-800 text-xs truncate group-hover:text-sky-600">
            {title}
          </h3>
        </Link>
        <div className="flex items-center shrink-0 ml-1">
          {sizeSelector}
          <Link to={to} className="p-0.5 text-stone-600 hover:text-stone-600">
            <ArrowRight size={12} />
          </Link>
        </div>
      </div>

...
export const JobSearchWidget = memo(function JobSearchWidget({
  ...
  if (size === 'mini') {
    return (
      <Link
        to="/job-search"
        className="group flex items-center gap-3 bg-white dark:bg-stone-800 p-3 rounded-xl border border-stone-200 dark:border-stone-700 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-md transition-all duration-200"
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-stone-800 dark:text-stone-100">{t('jobSearchWidget.jobs')}</p>
          <p className="text-xs text-stone-500 dark:text-stone-400">{t('jobSearchWidget.savedCount', { count: savedCount })}</p>
        </div>
        {newMatches > 0 && (
  ...
  if (size === 'medium') {
    return (
      <Link
        to="/job-search"
        className="group block bg-white dark:bg-stone-800 p-4 rounded-xl border border-stone-200 dark:border-stone-700 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200"
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-stone-800 dark:text-stone-100 text-sm">{t('jobSearchWidget.jobSearch')}</h3>
        ...
        <div className="flex items-center gap-3">
          <span className="text-2xl font-bold text-stone-800 dark:text-stone-100">{savedCount}</span>
          <span className="text-sm text-stone-500 dark:text-stone-400">{t('jobSearchWidget.savedJobs')}</span>
          {newMatches > 0 && (
  ...
  return (
    <Link
      to="/job-search"
      className="group block bg-white dark:bg-stone-800 p-5 rounded-xl border border-stone-200 dark:border-stone-700 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-lg transition-all duration-200"
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-stone-800 dark:text-stone-100">{t('jobSearchWidget.jobSearch')}</h3>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <p className="text-xs text-stone-500 dark:text-stone-400">{t('jobSearchWidget.saved')}</p>
        </div>
        <div className="p-3 bg-cyan-50 dark:bg-cyan-900/20 rounded-lg">
          <div className="flex items-center gap-2">
            <Search size={16} className="text-cyan-500 dark:text-cyan-400" />
            <span className="text-lg font-bold text-stone-800 dark:text-stone-100">{newMatches}</span>
          </div>
          <p className="text-xs text-stone-500 dark:text-stone-400">{t('jobSearchWidget.newMatches')}</p>
        </div>
      </div>
      ...
      {hasJobs && (
        <div className="space-y-2 mb-4">
          <p className="text-xs font-medium text-stone-500 dark:text-stone-400">{t('jobSearchWidget.recentlySaved')}</p>
          {sampleJobs.slice(0, 2).map(job => (
            <div key={job.id} className="flex items-center gap-3 p-2 bg-stone-50 dark:bg-stone-700/50 rounded-lg">
              <div className="w-8 h-8 bg-white dark:bg-stone-700 rounded-lg flex items-center justify-center border border-stone-100 dark:border-stone-600">
                <Briefcase size={14} className="text-stone-600" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-stone-700 dark:text-stone-200 truncate">{job.title}</p>
      ...
      <div className="flex gap-2">
        ...
        {hasJobs && (
          <span className="flex items-center gap-1.5 px-3 py-1.5 bg-stone-100 dark:bg-stone-700 text-stone-600 dark:text-stone-300 rounded-lg text-xs font-medium">

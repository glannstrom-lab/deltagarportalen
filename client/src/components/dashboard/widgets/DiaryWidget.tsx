...
const eventTypeConfig: Record<string, { icon: typeof Calendar; color: string; bg: string; labelKey: string }> = {
  ...
  default: { icon: Calendar, color: 'text-stone-600 dark:text-stone-400', bg: 'bg-stone-100 dark:bg-stone-800', labelKey: 'diaryWidget.eventTypes.event' },
}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          {upcomingEvents.length === 0 ? (
            <span className="text-xs text-stone-700 dark:text-stone-300">{t('diaryWidget.noEvents')}</span>
          ) : (
            <div className="flex items-center gap-1">
              <span className="text-sm font-bold text-stone-800 dark:text-stone-100">{upcomingEvents.length}</span>
    ...
    >
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-stone-800 dark:text-stone-100">{upcomingEvents.length}</p>
            <p className="text-xs text-stone-700 dark:text-stone-300">{t('diaryWidget.upcomingEvents')}</p>
          </div>
          {eventsThisWeek > 0 && (
            <div className="ml-auto text-right">
              <p className="text-sm font-bold text-rose-600">{eventsThisWeek}</p>
              <p className="text-xs text-stone-700 dark:text-stone-300">{t('diaryWidget.thisWeek')}</p>
            </div>
          )}
        </div>
        ...
        {upcomingEvents.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-semibold text-stone-600 dark:text-stone-400 uppercase tracking-wider">{t('diaryWidget.nextUp')}</p>
            {upcomingEvents.slice(0, 2).map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="flex items-center gap-3 p-2.5 rounded-xl bg-stone-50 dark:bg-stone-800 hover:bg-rose-50 border border-transparent hover:border-rose-200 transition-all cursor-pointer group"
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {event.time && (
                        <>
                          ...
                          <span className="text-xs text-stone-700 dark:text-stone-300">{event.time}</span>
                        </>
                      )}
                    </div>
                  </div>
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="p-3 bg-stone-50 dark:bg-stone-800 rounded-xl text-center">
            <p className="text-2xl font-bold text-stone-700 dark:text-stone-300">{eventsThisWeek}</p>
            <p className="text-xs text-stone-700 dark:text-stone-300">{t('diaryWidget.thisWeek')}</p>
          </div>
        ...
        {upcomingEvents.length > 0 ? (
            ...
            <div className="space-y-2">
              {upcomingEvents.slice(0, 4).map((event, index) => {
                ...
                return (
                  <div
                    key={event.id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-stone-50 dark:bg-stone-800 hover:bg-rose-50 border border-stone-100 dark:border-stone-700 hover:border-rose-200 transition-all cursor-pointer group"
                  >
                    {/* Date column */}
                    <div className="w-14 text-center shrink-0">
                      <p className="text-xs font-bold text-rose-600">{getRelativeDate(event.date)}</p>
                      {event.time && (
                        <p className="text-xs text-stone-700 dark:text-stone-300">{event.time}</p>
                      )}
                    </div>
          ...
          <div className="p-6 bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl border border-rose-100 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              <span className="px-3 py-1.5 bg-white rounded-lg text-xs text-stone-600 dark:text-stone-400 border border-rose-100">

...
const CATEGORIES = {
  content: { label: 'Innehåll', color: 'brand' },
...
export default function VisibilityTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-brand-50 to-sky-50 dark:from-brand-900/30 dark:to-sky-900/30 border-brand-100 dark:border-brand-900">
        <div className="flex items-start gap-4">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-brand-900 dark:text-brand-400">{completedCount}/{VISIBILITY_STRATEGIES.length}</p>
      ...
      <Card className="border-brand-200 dark:border-brand-900 bg-gradient-to-br from-brand-50/50 to-sky-50/50 dark:from-brand-900/30 dark:to-sky-900/30">
        <div className="flex items-start gap-3">
          <Lightbulb className="w-5 h-5 text-brand-900 dark:text-brand-400 shrink-0 mt-0.5" />
          <div className="flex-1">
            <p className="font-medium text-brand-900 dark:text-brand-100">Idé för nästa inlägg</p>
            <p className="text-brand-900 dark:text-brand-300 mt-1 text-lg">{contentIdea}</p>
            <div className="flex gap-2 mt-3">
              ...
              <Link to="/linkedin-optimizer">
                <Button variant="ghost" size="sm" className="text-brand-900 dark:text-brand-400">
      ...
      <Card className="bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            <Calendar className="w-5 h-5 text-brand-900 dark:text-brand-400" />
            Innehållskalender
          </h3>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isCurrentDay ? "border-brand-300 dark:border-brand-900 bg-brand-50 dark:bg-brand-900/30" : "border-stone-200 dark:border-stone-600"
                )}
              >
                <p className={cn(
                  "text-xs font-medium mb-1",
                  isCurrentDay ? "text-brand-900 dark:text-brand-300" : "text-gray-600 dark:text-gray-400"
                )}>
                ...
                {dayItems.map((item) => (
                  <div
                    ...
                    className={cn(
                      ...
                      item.platform === 'blog' && "bg-brand-100 dark:bg-brand-900/50 text-brand-900 dark:text-brand-300",
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={cn(
            ...
            !selectedCategory ? "bg-brand-900 dark:bg-brand-900 text-white" : "bg-stone-100 dark:bg-stone-700 text-gray-600 dark:text-gray-300 hover:bg-stone-200 dark:hover:bg-stone-600"
          )}
        >
            ...
            <button
              ...
              className={cn(
                ...
                selectedCategory === key
                  ? cn(
                    cat.color === 'brand' && "bg-brand-900 dark:bg-brand-900 text-white",
      ...
      <Card className="bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700">
        <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-brand-900 dark:text-brand-400" />
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-3">
                      ...
                      <span className={cn(
                        ...
                        strategy.impact === 'high' && "bg-brand-100 dark:bg-brand-900/50 text-brand-900 dark:text-brand-300"
                      )}>
      ...
      <Card className="bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            <div key={idx} className="flex items-start gap-2 p-3 bg-brand-50 dark:bg-brand-900/30 rounded-lg">
              <CheckCircle className="w-4 h-4 text-brand-900 dark:text-brand-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-brand-900 dark:text-brand-100 text-sm">{tip.title}</p>
                <p className="text-xs text-brand-900 dark:text-brand-300">{tip.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-4 pt-4 border-t border-stone-100 dark:border-stone-700">
          <Link
            ...
            className="inline-flex items-center gap-1 text-sm font-medium text-brand-900 dark:text-brand-400 hover:text-brand-900 dark:hover:text-brand-300"
          >
      ...
      <Card className="bg-brand-50 dark:bg-brand-900/30 border-brand-100 dark:border-brand-900">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
          {[
            ...
            <div key={day} className="p-3 bg-white dark:bg-stone-800 rounded-lg border border-brand-100 dark:border-brand-900">
              <p className="font-medium text-brand-900 dark:text-brand-200 text-sm">{day}</p>
              <p className="text-xs text-brand-900 dark:text-brand-300 mt-1">{task}</p>
            </div>
          ))}
        </div>
      </Card>

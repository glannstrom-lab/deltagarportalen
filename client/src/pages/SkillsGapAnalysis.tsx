      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="p-6 bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-stone-700 flex items-center justify-center">
              <CheckCircle className="w-5 h-5 text-slate-600 dark:text-stone-400" />
            </div>
            <h2 className="font-semibold text-slate-800 dark:text-stone-100">{t('skillsGapAnalysis.currentProfile.title')}</h2>
          </div>
          <textarea
            value={cvText}
          ...
          />
          <p className="text-xs text-slate-600 dark:text-stone-400 mt-2">
            ...
            <div className="space-y-4 mb-6">
              ...
              {analys.skills.map((skill) => (
                <div key={skill.name} className="bg-white dark:bg-stone-800 p-4 rounded-lg border border-teal-100 dark:border-teal-800">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        ...
                        <span className={`text-xs px-2 py-1 rounded ${
                          ...
                          {skill.level === 'beginnare' ? 'Borja' : skill.level === 'intermediate' ? 'Mellanhand' : 'Expert'}
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium text-slate-700 dark:text-stone-300">{skill.current}% - {skill.required}%</div>
                      <div className="text-xs text-slate-600 dark:text-stone-400">Luckegap: {skill.required - skill.current}%</div>
                    </div>
                  </div>
                  ...
                  {/* Larresurser */}
                  <div className="mt-2">
                    <p className="text-xs font-medium text-slate-600 dark:text-stone-400 mb-1">Resurser:</p>

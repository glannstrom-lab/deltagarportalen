...
/**
 * Översikt — clean rebuild (oversikt-sketch-v2.html + team feedback round 2).
 *
 * Five blocks:
 *   1. Hero — varm hälsning + datum (förslag 10: trimmade frasen efter datumet)
 *   2. En idé för idag — kandidatlista; "Visa annat förslag" roterar (förslag 1)
 *      + outline-style primary CTA (förslag 12)
 *   3. Status-rad — 3 KPIer; svenskt textdatum, 18px ikoner (förslag 2, 8)
 *   4. Senaste aktivitet — kronologisk feed; klickbara rader (förslag 14),
 *      konsulent-rader (förslag 13), exakt datum för >30d (förslag 4),
 *      "Visa allt →" (förslag 17)
 *   5. Hubs — 4 länkkort med stärkt hover (förslag 6)
 */
...
/** "26 april" / "26 april 2025" om annat år */
...
/**
 * Förslag 4: relativ tid tills 30d, sen exakt datum (för att inte visa
 * "1 år sen" som inte säger något användbart).
 */
...
export default function HubOverview() {
  ...
  const cvStatus = useMemo(() => {
    ...
    return {
      value: 'Uppdaterat',
      // Förslag 2: textdatum istället för 2026-04-28
  ...
  // ---------- Nästa steg (kandidatlista — förslag 1: Visa annat förslag roterar) ----------
  const nextStepCandidates = useMemo<NextStepCandidate[]>(() => {
    ...
    // Fallback om allt är gjort: utforska
    candidates.push({
  ...
  const activity = useMemo(() => {
    ...
    // Förslag 13: konsulent-rader
    const upcoming = summary?.minVardag?.upcomingEvents ?? []
    const nextMeeting = upcoming.find((e) => e.type === 'meeting' || e.type === 'consultant')
      ?? upcoming[0]
    if (nextMeeting) {
      items.push({
  ...
  return (
    <PageLayout
      ...
      showTabs={false}
    >
      {/* 1. Hero — förslag 10: avskalad utan extra-fras */}
      <header aria-labelledby="hero-greeting">
        <h2
          id="hero-greeting"
          className="text-[30px] font-bold text-[var(--stone-900)] leading-tight tracking-tight m-0"
        >
      ...
      {/* 2. En idé för idag — outline CTA + Visa annat förslag */}
      {nextStep && (
        <section
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <h3
              id="nextstep-heading"
              className="text-[20px] font-semibold text-[var(--stone-900)] tracking-tight m-0 mb-1.5"
            >
              {nextStep.heading}
            </h3>
            ...
            {/* Förslag 7: ta bort italic, använd em-dash som avskiljare */}
            <p className="text-[11px] text-[var(--stone-500)] mt-2.5 m-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {canRotate && (
              <button
                ...
                className="md:inline-flex hidden items-center justify-center w-10 h-10 rounded-[10px] bg-white/60 hover:bg-white border border-[var(--c-accent)] text-[var(--c-text)]"
              >
                <RefreshCw size={16} />
              </button>
            )}
            {/* Förslag 12: outline-style primary CTA */}
            <Link
              to={nextStep.ctaHref}
              className="inline-flex items-center gap-2 bg-white border border-[var(--c-solid)] text-[var(--c-text)] px-5 py-3 rounded-[10px] text-[14px] font-semibold no-underline hover:bg-[var(--c-bg)] whitespace-nowrap"
            >
              {nextStep.ctaLabel}
            </Link>
          </div>
      ...
      {/* 3. Status-rad — förslag 8: 18px ikoner */}
      ...
      {/* 4. Senaste aktivitet — förslag 5: byt hint till Visa allt-länk */}
          ...
          {activity.length === 0 ? (
            ...
            <ul className="list-none m-0 p-0">
              {activity.map((item, i) => (
                <li
                  key={item.key}
                  className={i < activity.length - 1 ? 'border-b border-[var(--stone-100)]' : ''}
                >
                  {/* Förslag 14: hela raden klickbar om href finns */}
                  {item.href ? (
                    <Link
                      to={item.href}
      ...
      {/* 5. Hubs — förslag 6: större padding + starkare hover */}
...
function ActivityRow({
  ...
  item: { icon: typeof Heart; domain: Domain; label: React.ReactNode; iso: string }
}) {
  return (
    <>
      <span
...
function HubCard({
  ...
  // Förslag 6: större padding + hover-border --c-solid (mer markant)
  return (
    <Link
      to={to}
      className="block bg-[var(--surface)] border border-[var(--stone-200)] rounded-[14px] p-[20px_22px] no-underline transition-[border-color,box-shadow,transform] duration-150 hover:border-[var(--c-solid)] hover:shadow-md hover:-translate-y-px"
    >
      <span
        aria-hidden="true"

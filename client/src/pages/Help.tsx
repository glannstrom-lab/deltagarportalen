        ...
        {quickLinkDefs.map((item) => {
          ...
          return (
            <Link
              key={item.titleKey}
              to={item.link}
              className="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-4 hover:shadow-md hover:border-[var(--c-accent)] transition-all text-center"
            >
              <div className="w-10 h-10 bg-[var(--c-bg)] rounded-lg flex items-center justify-center mx-auto mb-2">
        ...
        {faqCategoryDefs.map((category) => {
          ...
          return (
            <section key={category.titleKey} className="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 overflow-hidden">
              <div className="px-6 py-4 bg-[var(--c-bg)] border-b border-[var(--c-accent)] flex items-center gap-3">
                <Icon className="w-5 h-5 text-[var(--c-text)]" />
                <h2 className="font-semibold text-[var(--c-text)]">{t(category.titleKey)}</h2>
              </div>
                ...
                {category.items.map((item, index) => (
                  <div key={index} className="p-6">
                    ...
                    {item.link && (
                      <Link
                        to={item.link}
                        className="inline-flex items-center gap-1 text-sm text-[var(--c-text)] hover:text-[var(--c-solid)] font-medium transition-colors"
                      >
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-3 justify-center md:justify-start">
              <Link
                to="/diary"
                className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--c-solid)] hover:brightness-110 text-white rounded-lg font-medium transition-colors"
              >
          ...
          <div className="w-16 h-16 bg-white dark:bg-stone-700 rounded-2xl flex items-center justify-center shadow-sm">
            <MessageCircle className="w-8 h-8 text-[var(--c-solid)]" />
          </div>
        </div>
      </section>

...
const hanterare = async (req, res) => {
  ...
  for (const n of kandidater) {
    ...
    if (profilSvar.error || prefSvar.error) {
      utfall.fel++;
      console.error('[aktivitet-mejl] uppslag misslyckades för notis', n.id, (profilSvar.error || prefSvar.error).message);
      continue;
    }
    ...
    try {
      ...
      if (markeringsfel) {
        utfall.ejMarkerade++;
        console.error('[aktivitet-mejl] mail_sent kunde inte sättas för notis', n.id, markeringsfel.message);
      }
    } catch (e) {
      utfall.fel++;

...
export function PageTabs({ tabs, className, collapsible = true, variant = 'minimal' }: PageTabsProps) {
  ...
  // En renderfunktion, inte en komponent: en komponent definierad i renderingen
  // monteras om vid varje rendering (react-hooks/static-components).
  const renderMobileScrollTabs = () => (
    <nav
...
export function PageHeader({ title, description, tabs, tabVariant = 'minimal', actions, stats, className }: PageHeaderProps) {
  ...
  return (
    <div className={cn(
      'bg-[var(--header-bg)] rounded-2xl border border-stone-300 dark:border-stone-700 shadow overflow-hidden',
      className
    )}>
      {/* Header content */}
      ...
      {/* Tabs separated from title row by a hairline */}
      {hasTabs && (

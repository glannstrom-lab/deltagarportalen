-- Fix: Add ALL missing columns to achievements table
...
-- Add xp_reward column if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Add requirement_type column if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Add requirement_value column if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE achievements ADD COLUMN icon TEXT DEFAULT 'award';
  END IF;
END $$;
...
-- Update existing rows with generated key if null
UPDATE achievements SET key = LOWER(REPLACE(REPLACE(name, ' ', '_'), '-', '_')) WHERE key IS NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    -- First make sure all keys are not null
    UPDATE achievements SET key = 'achievement_' || id WHERE key IS NULL;
...
-- Clear existing achievements and insert fresh ones

-- Fix: OUT-parameter "participant_id" krockade med kolumnen
-- consultant_participants.participant_id, vilket gav
-- "column reference 'participant_id' is ambiguous" i smart-match-grenen.
--
-- Lösning: byt namn på OUT-parametern till linked_participant_id.
-- API:n uppdateras tillsammans.
...
COMMENT ON FUNCTION sta_bulk_smart_add IS
  'CSV/Excel-import: per rad försök koppla befintlig Jobin-user direkt, annars skapa invitation. Returnerar linked|invited|error per rad.';

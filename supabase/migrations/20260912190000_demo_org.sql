-- KM12 (8): Demokonto för kommuner — "Demokommun" med påhittade personer (2026-09-12)
--
-- En kommun som läser /for-arbetsmarknadsenheter/ ska kunna logga in i konsulentvyn
-- direkt, med ett delat konto vars uppgifter står på sidan. Allt i demot är fiktivt:
-- deltagarna heter Exempel/Demo/Fiktiv, har @example.com-adresser och kan inte logga in.
--
-- Skydd:
--   * organizations.is_demo = true (ny kolumn) — send-invite-email nekar medlemmar i en
--     demo-org (ingen kan skicka riktiga mejl från demot), och klienten visar en banner.
--   * organizations.ai_enabled = false — org-brytaren (pass 8) stänger AI för alla
--     deltagare kopplade till demokonsulenten; profiles.ai_enabled = false dessutom.
--   * reset_demo_org() återställer demot till utgångsläget varje natt 01:00 UTC
--     (pg_cron 'demo-reset'): allt som demobesökare skapat raderas, sedan seedas om.
--
-- Fasta id:n (så återställningen är idempotent):
--   org        22222222-2222-4222-8222-222222222222
--   konsulent  22222222-2222-4222-8222-000000000001  demo@jobin.se
--   deltagare  22222222-2222-4222-8222-000000000002 … 006
--
-- Lösenordet sätts via seed_demo_org(p_password) vid första körningen och byts aldrig
-- av reset (reset rör inte auth.users för de fasta id:n). Det står på B2B-sidan.
--
...
COMMENT ON COLUMN public.organizations.is_demo IS
  'Demoorganisation med påhittade personer (KM12). Medlemmar kan inte skicka mejl; klienten visar banner; nattlig återställning.';

-- ============================================================================
-- Seed: skapar eller återställer demodatan. Idempotent.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.seed_demo_org(p_password text DEFAULT NULL)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR r IN SELECT * FROM (VALUES
      (v_kons, 'demo@jobin.se',                'Demo',   'Konsulent', true),
      (v_p1,   'anna.exempel@example.com',     'Anna',   'Exempel',   false),
      (v_p2,   'omar.demo@example.com',        'Omar',   'Demo',      false),
      (v_p3,   'lisa.fiktiv@example.com',      'Lisa',   'Fiktiv',    false),
      (v_p4,   'erik.testsson@example.com',    'Erik',   'Testsson',  false),
      (v_p5,   'fatima.exempel@example.com',   'Fatima', 'Exempel',   false)
    ) AS t(id, email, fnamn, enamn, kan_logga_in)
  LOOP
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = r.id) THEN
      ...
      VALUES (r.id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', r.email,
        -- Deltagarna får ett slumpat lösenord som ingen känner till: de ska aldrig logga in.
        crypt(CASE WHEN r.kan_logga_in THEN coalesce(p_password, gen_random_uuid()::text) ELSE gen_random_uuid()::text END, gen_salt('bf')),
        now(), '{"provider":"email","providers":["email"]}'::jsonb,
        jsonb_build_object('first_name', r.fnamn, 'last_name', r.enamn, 'email_verified', true),
        now(), now(), '', '', '', '', '', '', '', '');
      -- auth.identities.email är en genererad kolumn (ur identity_data) — skicka inte den
      ...
      VALUES (gen_random_uuid(), r.id, r.id::text, 'email',
        ...
        NULL, now(), now());
    ELSIF r.kan_logga_in AND p_password IS NOT NULL THEN
      UPDATE auth.users SET encrypted_password = crypt(p_password, gen_salt('bf')) WHERE id = r.id;
    END IF;
    -- Profilen (skapad av triggern) sätts alltid om
    ...
    UPDATE profiles SET
      ...
      role = CASE WHEN r.kan_logga_in THEN 'CONSULTANT' ELSE 'USER' END,
      -- App.tsx vaktar konsulentvyn på active_role/roles, inte på role (triggern
      -- sätter {USER}/USER — utan raden nedan landar demokontot på deltagarens Översikt)
      roles = CASE WHEN r.kan_logga_in THEN ARRAY['CONSULTANT'] ELSE ARRAY['USER'] END,
      active_role = CASE WHEN r.kan_logga_in THEN 'CONSULTANT' ELSE 'USER' END,
      consultant_id = CASE WHEN r.kan_logga_in THEN NULL ELSE v_kons END,
      status = 'ACTIVE', ai_enabled = false, avatar_url = NULL, phone = NULL,
      terms_accepted_at = now(), privacy_accepted_at = now(),
      -- Välkomstguiden (OnboardingFlow) visas när onboarding_completed = false och
      -- 'Hoppa över' navigerar till deltagarens Översikt — bort för demokontona.
      onboarding_completed = true
    WHERE id = r.id;
  END LOOP;

  -- Deltagarnas profildetaljer (ort, läge) — det konsulentvyn visar
  ...
  -- Medlemskap: demokonsulenten är chef så caseload/organisation syns
  ...
  -- CV:n (formen som prod har: skills som objekt)
  ...
  -- Sparade jobb / ansökningar (pipeline)
  ...
  -- Aktivitetskrav: en mall, en plan för Anna med pass i förra och denna vecka
...
-- ============================================================================
-- Reset: raderar allt demobesökare skapat och seedar om. Rör inte auth.users
-- för de fasta id:n (lösenordet består). Extra konton som skapats via demot
-- (t.ex. inbjudningar som ändå slunkit igenom) raderas.
-- ============================================================================
...
DECLARE
  ...
  v_extra uuid[];
BEGIN
  -- Konton som demot skapat utöver de fasta: kopplade till demokonsulenten eller
  -- medlemmar i demo-orgen utan att vara i listan.
  ...
  -- Allt som rör demokonsulenten och demodeltagarna
  ...
  RETURN seed_demo_org(NULL);
END;
$$;

REVOKE ALL ON FUNCTION public.seed_demo_org(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.reset_demo_org() FROM PUBLIC, anon, authenticated;

-- Nattlig återställning
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'demo-reset') THEN
    PERFORM cron.schedule('demo-reset', '0 1 * * *', $job$ SELECT public.reset_demo_org(); $job$);
  END IF;
END $$;
